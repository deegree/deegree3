...
/**
 * The <code>WPVSStressTest</code> class generates the requests for the WPVSClient. The current command-line
 * parameters are:
 * <ul>
...
public class WPVSStressTest {
    ...
    /**
     * The <code>WPVSSender</code> class generates the requests to deegree3 WPVS in a thread
     * 
     * @author <a href="mailto:ionita@lat-lon.de">Andrei Ionita</a>
     * 
     * @author last edited by: $Author: ionita $
    ...
    private class WPVSSender implements Runnable {
        ...
        private URL getGetViewURL() {
            ...
            // including a valid BBOX because it is required (although the actual values are not currently evaluated --
            // 26 Jun 2009)
    ...
    private static Options initOptions() {
        ...
        // Optional parameters
        opt = new Option( "datasets", true, "specify a list of datasets in "
                          + "comma-separated format; by default value all datasets are used" );
        ...
        opt = new Option( "pitch", true, "specify a Pitch value or interval "
                          + "(using an underscore '_') between -90 and 90;"
        ...
        opt = new Option( "distance", true, "specify a distance to the point of interest;"
                          + " an integer value or an interval (separated by underscore) are "
                          + "valid inputs" );
        opt.setRequired( false );
        opts.addOption( opt );
        ...
        // currently the Bounding Box is not evaluated; any valid description (xMin<xMax && yMin<yMax)
        // yields an image with having the maximum BBox coverage
        // opt = new Option( "bbox", true, "specify a fixed Bounding Box - in the format \"xLow, yLow, xHigh, yHigh\" "
        // );
        // opt.setRequired( false );
        // opts.addOption( opt );

        opt = new Option( "poi", true, "specify a point-of-interest; valid inputs are"
                          + " 'x, y, z, crs', in which x, y and z can be fixed "
                          + "floating-point values or intervals specified using an underscore"
                          + " e.g. xMin_xMax; the crs value can be omitted " );
        opt.setRequired( false );
        opts.addOption( opt );

        opt = new Option( "scale", true, "specify a fixed scale for all tests; "
                          + "by default scale values will be randomized " );
        opt.setRequired( false );
        opts.addOption( opt );
    ...
    /**
     * @param args  mandatory arguments: -capabilities, -requests, -threads. To see the others, provide the argument 'help'.
     * @throws Exception    when the execution is aborted on the base of a wrong argument (or help was invoked),
     */
    ...
    private static void checkFloatOrInterval( String arg, String name ) throws Exception {
        if ( !arg.contains( "_" ) ) {
            ...
            if ( argInterval.length != 2 ) {
                String msg = "ERROR: The " + name + " interval should be two floating point numbers separated by an underscore ('_') ";
                LOG.error( msg );
    ...
    private static void checkIntegerOrInterval( String arg, String name ) throws Exception {
        if ( !arg.contains( "_" ) ) {
            ...
            String[] argInterval = arg.split( "_" );
            if ( argInterval.length != 2 ) {
                String msg = "ERROR: The " + name + " interval should be two integers separated by an underscore ('_') ";
                LOG.error( msg );
                throw new Exception( msg );
            }
    ...
    private static void checkIntegerOrBoundInterval( String arg, String name, int lowEnd, int highEnd ) throws Exception {
        if ( !arg.contains( "_" ) ) {
            ...
            if ( arg1 < lowEnd || arg1 > highEnd ) {
                String msg = "ERROR: The " + name + " interval values should be " + "between " + lowEnd + " and " + highEnd;
                LOG.error( msg );
                throw new Exception( msg );
            }
