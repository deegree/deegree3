...
<package
  ...
  <test name="csw:level1.3">
    ...
    <code>
      ...
      <call-test name="csw:CorrectnessOfResult.GetRecords-DefaultMaxRecords">
        <with-param name="csw.GetRecordById.soap.url" select="$csw.GetRecordById.soap.url"/>
      </call-test>
  ...
  <test name="csw:CorrectnessOfResult.GetRecords-DefaultMaxRecords">
    <param name="csw.GetRecordById.soap.url"/>
    <assertion>
      Search for records whitout any filter and without restricting the number of records to be returned.
      The response of the GetRecords request must satisfy the applicable assertions:
      1. the filter request is understood by the server and no exception concerning the request is thrown
      a. the response includes 10 ‘brief’ metadata entries returned in the http://www.isotc211.org/2005/gmd format
    </assertion>
    <comment>Pass if the response of the GetRecordById request (sent via
      HTTP/SOAP/POST/XML) holds the relevant assertions.</comment>
    <code>
      ...
      <xsl:if test="not(count($response//gmd:MD_Metadata) &gt; 9)">
        <message>FAILURE: the second assertion failed</message>
        <fail/>
      </xsl:if>
