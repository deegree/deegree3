...
/**
 * The change of coordinates from one CRS to another CRS based on different datum is 'currently'
 * only possible via a coordinate <code>Transformation</code>.
 * <p>
 * The transformation parameters could only be derived empirically by a set of points common to both
 * coordinate reference systems it means by identical points. Choice, allocation, number and the
 * quality of coordinates of the points affect extensive the results and the accuracy. Therefore
 * different realizations for transformations from one datum to another exist.
 * </p>
 * 
...
public abstract class CRSTransformation {
     ...
     * 
     * @param from
     *            containing the value (id or name) of the 'from' coourdinateSystem
     * @param to
     *            containing the value (id or name) of the 'to' coourdinateSystem
     * @return a following string "FROM_fromValue_TO_toValue".
     */
    protected static String createFromTo( String from, String to ) {
        return new StringBuilder( "FROM_" ).append( from ).append( "_TO_" ).append( to ).toString();
    }

    /**
     * Do a transformation, e.g. the incoming data comes from the sourceCRS and must be transformed
     * to the targetCRS.
     * 
     * @param srcPts
     *            the points which must be transformed, expected are following values either,
     *            long_1, lat_1, height_1, long_2, lat_2, height_2. or long_1, lat_1, long_2, lat_2
    ...
    /**
     * Wraps the incoming coordinates into a List<Point3d> and calls the {@link #doTransform(List)}.
     * The source array will be read according to the dimension of the source CRS
     * {@link #getSourceDimension()} and the target coordinates will be put according to the
     * dimension of the targetCRS {@link #getTargetDimension()}. If the sourceDim &lt; 2 or &gt; 3
     * a transformation exception will be thrown.
     * 
     * @param srcCoords
     *            the array holding the source ('original') coordinates.
     * @param startPositionSrc
     *            the position to start reading the coordinates from the source array (0 is the
     *            first).
     * @param destCoords
     *            the array which will receive the transformed coordinates.
     * @param startPositionDest
     *            the index of the destCoords array to put the results, if the result will exceed
     *            the array.length, the array will be enlarged to hold the transformed coordinates.
     * @param lastCoord
    ...
    /**
     * @return true if this transformation doesn't transform the incoming points. (e.g. is the id.
     *         matrix)
     */
    ...
    /**
     * This method flags the transformation about it's state. If this transformation was inverse
     * calling this method will result in a forward transformation and vice versa.
     */
    public void inverse() {
        isInverse = !isInverse;
    }
    ...
    /**
     * @return a representation of this transformations name, including the 'Forward' or 'Inverse'
     *         modifier.
     */
    ...
    /**
     * Checks if this transformation is the inverse of the other transformation, which means,
     * this.sourceCRS equals other.targetCRS && this.targetCRS == other.sourceCRS. If Both
     * transformations are identity this method also returns true.
     * 
     * @param other
     *            the transformation to check
     * @return true if this and the other transformation are eachothers inverse.
     */
    ...
    /**
     * @param sb
     *            to add the transformation chain to, if <code>null</code> a new StringBuilder
     *            will be created.
    ...
    private int outputTransform( int level, StringBuilder sb, CRSTransformation t ) {
