...
public class SimpleRaster extends AbstractRaster {
    ...
    /**
     * Returns a scaled subset of the raster.
     * 
     * @param x
     *            min x coordinate
     * @param y
     ...
     *            max y coordinate
     * @param resolution
     *            resolution of the subset
     * @return scaled subset with different resolution
     */
    public SimpleRaster getSubset( double x, double y, double x2, double y2, double resolution ) {
        Envelope env = getGeometryFactory().createEnvelope( new double[] { x, y }, new double[] { x2, y2 },
                                                            getRasterEnvelope().getDelta(), null );
        return getSubset( env, resolution );
    }

    /**
     * Returns a scaled subset of the raster.
     * 
     * The subset is scaled to the output size.
     * 
     * @param x
     *            min x coordinate
     * @param y
     ...
     * @param outHeight
     *            height in pixel
     * @return scaled subset with different raster size (resolution)
     */
    public SimpleRaster getSubset( double x, double y, double x2, double y2, int outWidth, int outHeight ) {
        Envelope env = getGeometryFactory().createEnvelope( new double[] { x, y }, new double[] { x2, y2 },
                                                            getRasterEnvelope().getDelta(), null );
        return getSubset( env, outWidth, outHeight );
    }
     ...
     * 
     * @param env
     *            envelope for the subset
     * @param resolution
     *            resolution of the subset
     * @return scaled subset with different resolution
     */
    public SimpleRaster getSubset( Envelope env, double resolution ) {
        // retain sign
        double xRes = resolution * Math.signum( getRasterEnvelope().getXRes() );
        double yRes = resolution * Math.signum( getRasterEnvelope().getYRes() );

        RasterEnvelope outRasterEnv = getRasterEnvelope().createScaledEnvelope( env, xRes, yRes );
        int[] size = outRasterEnv.getSize( env );
        return getSubset( env, size[0], size[1] );
    }
