...
/**
 * Provides access to the {@link FeatureType} hierarchy defined in a GML schema
 * document.
 * <p>
 * Note that the generated {@link ApplicationSchema} only contains user-defined
 * feature types, i.e. all feature base types from the GML namespace (e.g.
 * <code>gml:_Feature</code> or <code>gml:FeatureCollection</code>) are ignored.
 * This follows the idea that working with {@link ApplicationSchema} objects
 * should not involve GML (and GML-version) specific details (such as the
 * mentioned GML feature types).
 * </p>
 * 
 ...
 * 
 * @version $Revision$, $Date: 2010-03-15 15:09:10 +0100 (Mo, 15. Mär
 *          2010) $
 */
public class ApplicationSchemaXSDDecoder {
	 ...
	 *            optional hints (key: prefix, value: namespaces) for generating
	 *            'nice' qualified feature type and property type names, may be
	 *            null
	 * @param schemaUrls
	...
	public ApplicationSchemaXSDDecoder(GMLVersion gmlVersion, Map<String, String> namespaceHints, String... schemaUrls)
		...
		for (XSElementDeclaration elementDecl : featureElementDecls) {
			...
			if (substitutionElement != null) {
				QName substitutionElementName = createQName(substitutionElement.getNamespace(), substitutionElement
						.getName());
	...
	private PropertyType<?> buildPropertyType(XSElementDeclaration elementDecl, int minOccurs, int maxOccurs) {
		...
		if (gmlNs.equals(elementDecl.getNamespace())
				&& ("boundedBy".equals(elementDecl.getName()) || "name".equals(elementDecl.getName())
						|| "description".equals(elementDecl.getName())
						|| "metaDataProperty".equals(elementDecl.getName())
			...
			case XSTypeDefinition.SIMPLE_TYPE: {
				...
				pt = new SimplePropertyType<Object>(ptName, minOccurs, maxOccurs,
						getPrimitiveType((XSSimpleType) typeDef), elementDecl.getAbstract(), ptSubstitutions,
				...
				if (typeDef.getName() != null) {
					...
					if (typeName.equals(new QName(gmlVersion.getNamespace(), "CodeType"))) {
						...
						pt = new MeasurePropertyType(ptName, minOccurs, maxOccurs, elementDecl.getAbstract(),
							...
							.valueOf("{http://www.xplanung.de/xplangml}XP_VariableGeometrieType"))
							|| typeName.equals(QName
									.valueOf("{http://www.xplanung.de/xplangml/3/0}XP_FlaechengeometrieType"))
							|| typeName.equals(QName
									.valueOf("{http://www.xplanung.de/xplangml/3/0}XP_LiniengeometrieType"))
							|| typeName.equals(QName
									.valueOf("{http://www.xplanung.de/xplangml/3/0}XP_PunktgeometrieType"))
							|| typeName.equals(QName
									.valueOf("{http://www.xplanung.de/xplangml/3/0}XP_VariableGeometrieType"))) {
						// TODO remove xplan hack!!!
						pt = new GeometryPropertyType(ptName, minOccurs, maxOccurs,
								GeometryPropertyType.GeometryType.GEOMETRY,
								GeometryPropertyType.CoordinateDimension.DIM_2, elementDecl.getAbstract(),
								ptSubstitutions, INLINE);
					} else {
	...
	private String getCodeListId(XSElementDeclaration elementDecl) {
		...
		if (annotations.getLength() > 0) {
			...
			codeListId = adapter.getNodeAsString(adapter.getRootElement(), new XPath(
					"xs:appinfo/adv:referenzierteCodeList/text()", nsContext), null);
			if (codeListId != null) {
	 ...
	 * @param minOccurs
	 * @param maxOccurs
	 * @return corresponding {@link FeaturePropertyType} or null, if declaration
	 *         does not define a feature property
	 */
					...
					case XSConstants.ELEMENT_DECLARATION: {
						...
						if (ftNameToFtElement.get(elementName) != null) {
							...
							if (gmlNs.equals(elementName.getNamespaceURI())) {
								pt = new FeaturePropertyType(ptName, minOccurs, maxOccurs, null, elementDecl2
										.getAbstract(), ptSubstitutions, ValueRepresentation.BOTH);
							} else {
								pt = new FeaturePropertyType(ptName, minOccurs, maxOccurs, elementName, elementDecl2
										.getAbstract(), ptSubstitutions, ValueRepresentation.BOTH);
							}
	...
	private FeaturePropertyType buildFeaturePropertyTypeXGml(XSElementDeclaration elementDecl,
		...
		if (annotations.getLength() > 0) {
			...
			NamespaceContext nsContext = new NamespaceContext();
			nsContext.addNamespace("xs", CommonNamespaces.XSNS);
			QName refElement = adapter.getNodeAsQName(adapter.getRootElement(), new XPath(
					"xs:appinfo[@source='urn:x-gml:targetElement']/text()", nsContext), null);
			if (refElement != null) {
	...
	private FeaturePropertyType buildFeaturePropertyTypeAdv(XSElementDeclaration elementDecl,
		...
		if (annotations.getLength() > 0) {
			...
			nsContext.addNamespace("adv", "http://www.adv-online.de/nas");
			QName refElement = adapter.getNodeAsQName(adapter.getRootElement(), new XPath(
					"xs:appinfo/adv:referenziertesElement/text()", nsContext), null);
			if (refElement != null) {
	...
	private FeaturePropertyType buildFeaturePropertyTypeGML32(XSElementDeclaration elementDecl,
		...
		if (annotations.getLength() > 0) {
			...
			QName refElement = adapter.getNodeAsQName(adapter.getRootElement(), new XPath(
					"xs:appinfo/gml:targetElement/text()", nsContext), null);
			if (refElement != null) {
				LOG.trace("Identified a feature property (GML 3.2 style).");
				QName elementName = createQName(elementDecl.getNamespace(), elementDecl.getName());
				FeaturePropertyType pt = new FeaturePropertyType(elementName, minOccurs, maxOccurs, refElement,
						elementDecl.getAbstract(), ptSubstitutions, ValueRepresentation.REMOTE);
				featurePropertyTypes.add(pt);
				return pt;
			}
		}
	 ...
	 * @param minOccurs
	 * @param maxOccurs
	 * @return corresponding {@link GeometryPropertyType} or null, if
	 *         declaration does not define a geometry property
	 */
					...
					case XSConstants.ELEMENT_DECLARATION: {
						...
						if (geometryNameToGeometryElement.get(elementName) != null) {
							...
							return new GeometryPropertyType(ptName, minOccurs, maxOccurs, geometryType,
									CoordinateDimension.DIM_2_OR_3, elementDecl2.getAbstract(), ptSubstitutions, BOTH);
						}
					}
					case XSConstants.WILDCARD: {
						LOG.debug("Unhandled particle: WILDCARD");
	...
	/**
	 * After parsing, this method can be called to find out all referenced types
	 * that have been encountered (for debugging).
	 * 
	 * @return
	 */
