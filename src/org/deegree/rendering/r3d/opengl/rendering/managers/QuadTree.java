...
public class QuadTree<T extends Positionable> {
     ...
     * @return true if the object was inserted, false otherwise.
     */
    public boolean remove( Positionable object ) {
        if ( object != null && liesWithin( object.getPosition() ) ) {
     ...
     * @param object
     */
    private boolean removeObject( Positionable object ) {
        ...
        if ( isLeaf() ) {
            ...
            QuadTree<T> n = getChild( object.getPosition() );
            if ( n != null ) {
     ...
     *         node.
     */
    public QuadTree<T> getNode( float[] position ) {
     ...
     */
    private QuadTree<T> getChild( float[] position ) {
     ...
     */
    private QuadTree<T> getLeafNode( float[] position ) {
        if ( !isLeaf() ) {
            int index = getIndex( position );
     ...
     * @return
     */
    private QuadTree<T> createNode( int index ) {
     ...
     */
    public boolean contains( Positionable object ) {
        boolean result = false;
        if ( object != null && liesWithin( object.getPosition() ) ) {
            QuadTree<T> n = getLeafNode( object.getPosition() );
            if ( n.objects != null ) {
