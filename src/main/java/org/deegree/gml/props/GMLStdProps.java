...
/**
 * Version-agnostic representation of the standard properties that any
 * {@link GMLObject} allows for.
 * <p>
 * The following properties exist (description taken from GML 3.1.1/3.2.1
 * schemas):
 * <ul>
 * <li><b><code>gml:metaDataProperty</code></b>: Contains or refers to a
 * metadata package that contains metadata properties. Has been deprecated in
 * GML 3.2.1.</li>
 * <li><b><code>gml:description</code></b>: The value of this property is a text
 * description of the object. gml:description uses gml:StringOrRefType as its
 * content model, so it may contain a simple text string content, or carry a
 * reference to an external description. The use of gml:description to reference
 * an external description has been deprecated and replaced by the
 * gml:descriptionReference property.</li>
 * <li><b><code>gml:descriptionReference</code></b>: The value of this property
 * is a remote text description of the object. The xlink:href attribute of the
 * gml:descriptionReference property references the external description.</li>
 * <li><b><code>gml:identifier</code></b>: Often, a special identifier is
 * assigned to an object by the maintaining authority with the intention that it
 * is used in references to the object For such cases, the codeSpace shall be
 * provided. That identifier is usually unique either globally or within an
 * application domain. gml:identifier is a pre-defined property for such
 * identifiers.</li>
 * <li><b><code>gml:name</code></b>: The gml:name property provides a label or
 * identifier for the object, commonly a descriptive name. An object may have
 * several names, typically assigned by different authorities. gml:name uses the
 * gml:CodeType content model. The authority for a name is indicated by the
 * value of its (optional) codeSpace attribute. The name may or may not be
 * unique, as determined by the rules of the organization responsible for the
 * codeSpace. In common usage there will be one name per authority, so a
 * processing application may select the name from its preferred codeSpace.</li>
 * </ul>
 * </p>
 * <p>
 * <table border="1">
...
public class GMLStdProps {
	...
	static {
		PT_DESCRIPTION_GML2 = new SimplePropertyType(new QName(GMLNS,
				"description"), 0, 1, STRING, false, null);
		PT_NAME_GML2 = new SimplePropertyType(new QName(GMLNS, "name"), 0, 1,
				STRING, false, null);

		// TODO correct this (should be a MetaDataPropertyType)
		PT_META_DATA_PROPERTY_GML31 = new CustomPropertyType(new QName(GMLNS,
				"metaDataProperty"), 0, -1, null, false, null);
		// TODO correct this (should be a StringOrRefType)
		PT_DESCRIPTION_GML31 = new StringOrRefPropertyType(new QName(GMLNS,
				"description"), 0, 1, false, null);
		PT_NAME_GML31 = new CodePropertyType(new QName(GMLNS, "name"), 0, -1,
				false, null);

		// TODO correct this (should be a MetaDataPropertyType)
		PT_META_DATA_PROPERTY_GML32 = new CustomPropertyType(new QName(
				GML3_2_NS, "metaDataProperty"), 0, -1, null, false, null);
		// TODO correct this (should be a StringOrRefType)
		PT_DESCRIPTION_GML32 = new StringOrRefPropertyType(new QName(GML3_2_NS,
				"description"), 0, 1, false, null);
		// TODO correct this (should be a ReferenceType)
		PT_DESCRIPTION_REFERENCE_GML32 = new StringOrRefPropertyType(new QName(
				GML3_2_NS, "descriptionReference"), 0, 1, false, null);
		PT_IDENTIFIER_GML32 = new CodePropertyType(new QName(GML3_2_NS,
				"identifier"), 0, -1, false, null);
		PT_NAME_GML32 = new CodePropertyType(new QName(GML3_2_NS, "name"), 0,
				-1, false, null);
	}
	 ...
	 */
	public GMLStdProps(TypedObjectNode[] metadata, StringOrRef description,
			CodeType identifier, CodeType[] names) {
		if (metadata == null) {
