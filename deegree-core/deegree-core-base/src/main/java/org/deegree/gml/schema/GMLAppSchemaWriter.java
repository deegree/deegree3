...
/**
 * Stream-based writer for exporting {@link AppSchema} or {@link FeatureType} instances as
...
public class GMLAppSchemaWriter {
	...
	// set to "gml:FeatureAssociationType" (GML 2) or "gml:FeaturePropertyType" (GML 3.0,
	// 3.1 and 3.2)
	...
	/**
	 * Creates a new {@link GMLAppSchemaWriter} for the given GML version and optional
	 * import URL.
	 * @param version gml version that exported schemas will comply to, must not be
	 * <code>null</code>
	 * @param targetNamespace target namespace for the schema document, may be
	 * <code>null/<code>
	 *
	@param importURLs
	 *            to be imported in the generated schema document, this may also contain a URL for the gml namespace,
	 *            may be <code>null</code>
	 * @param prefixToNs keys: namespace prefixes, values: namespaces, may be
	 * <code>null</code>
	 */
	...
	public static void export(XMLStreamWriter writer, GMLSchemaInfoSet xsModel, String targetNs,
			URITranslator translator) throws XMLStreamException, IOException {
	...
	public void export(XMLStreamWriter writer, AppSchema schema) throws XMLStreamException {
	 ...
	 * @throws XMLStreamException
	 */
	public void export(XMLStreamWriter writer, List<FeatureType> fts) throws XMLStreamException {
		...
		for (String prefix : prefixesToNs.keySet()) {
			...
			// avoid double writing of the namespace (required since
			// IS_REPAIRING_NAMESPACE=FALSE)
		...
		// export element declarations and type declarations (in the target namespace)
		// that are not feature type related
		try {
	...
	private void export(XMLStreamWriter writer, FeatureType ft) throws XMLStreamException {
	...
	private void exportFeatureComplexType(XMLStreamWriter writer, FeatureType ft, FeatureType parentFt, String typeName)
		...
		// TODO check for GML 2-only properties (gml:pointProperty, ...) and export as
		// "app:gml2PointProperty" for
		// GML 3
	...
	/**
	 * Returns the XML schema type name to use for exporting the complex type for the
	 * given {@link FeatureType}.
	 * @param ft feature type, must not be <code>null</code>
	 * @return the qualified complex type name or <code>null</code> if the type should be
	 * exported anonymously
	 */
	private QName getXSTypeName(FeatureType ft) {
		...
		if (analyzer == null) {
			...
			XSElementDeclaration elDecl = analyzer.getXSModel()
				.getElementDeclaration(elName.getLocalPart(), elName.getNamespaceURI());
			if (elDecl != null) {
	...
	private void export(XMLStreamWriter writer, GeometryPropertyType pt) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, FeaturePropertyType pt) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, CustomPropertyType pt) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, ObjectPropertyType pt) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, XSTypeDefinition xsTypeDef) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, CodePropertyType pt) throws XMLStreamException {
	...
	private void export(XMLStreamWriter writer, MeasurePropertyType pt) throws XMLStreamException {
		if (version.equals(GML_2)) {
	...
	private void exportType(XMLStreamWriter writer, XSTypeDefinition type) throws XMLStreamException {
	...
	private void exportComplexType(XMLStreamWriter writer, XSComplexTypeDefinition complex) throws XMLStreamException {
		...
		switch (contentType) {
			...
			case CONTENTTYPE_ELEMENT:
				// TODO check if non-redundant restriction / extension is performed (in
				// that case, complexContent
	...
	private void exportElement(XMLStreamWriter writer, XSElementDeclaration element, int minOccurs, int maxOccurs,
		...
		if (substGroup != null) {
			writer.writeAttribute("substitutionGroup",
					getPrefixedName(new QName(substGroup.getNamespace(), substGroup.getName())));
		}
