...
public class GMLFeatureWriter extends AbstractGMLObjectWriter {
	 ...
	 * @param feature feature to be exported, must not be <code>null</code>
	 ...
	 * @param prop property to be exported, must not be <code>null</code>
	 * @throws XMLStreamException
	 * @throws UnknownCRSException
	 * @throws TransformationException
	 */
	public void export(Property prop) throws XMLStreamException, UnknownCRSException, TransformationException {
	...
	private void writeNilledElement(QName propName, Map<QName, PrimitiveValue> attributes) throws XMLStreamException {
		writeEmptyElementWithNS(propName.getNamespaceURI(), propName.getLocalPart());
		writeNilAttributes(attributes);
		endEmptyElement();
	}

	private void writeNilAttributes(Map<QName, PrimitiveValue> attributes) throws XMLStreamException {
	...
	private List<Property> augmentBoundedBy(Feature f) {
		...
		for (int i = 0; i < props.size(); i++) {
			...
			if (!gmlNs.equals(name.getNamespaceURI()) || name.getLocalPart().equals("location")) {
				// not a GML property or gml:location -> gml:boundedBy must be included
				// right before it
	...
	private void exportFeatureProperty(FeaturePropertyType pt, FeatureReference ref, GmlXlinkOptions resolveState,
			QName propName) throws XMLStreamException, UnknownCRSException, TransformationException {
	...
	private void exportFeaturePropertyByReference(QName propName, FeatureReference ref,
			boolean forceInclusionInDocument, GmlXlinkOptions resolveState) throws XMLStreamException {
	...
	private void writeAttributes(final Map<QName, PrimitiveValue> attrs) throws XMLStreamException {
		if (attrs != null) {
