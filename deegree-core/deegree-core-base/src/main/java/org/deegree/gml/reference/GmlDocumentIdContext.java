...
/**
 * Keeps track of {@link GMLObject}s inside GML instance documents, their ids and local
 * xlink references during the parsing of GML documents.
 * <p>
 * Can be used for resolving local xlink-references at the end of the parsing process of a
 * GML instance document or to access all encountered objects on any level of the
 * document.
 * </p>
 *
...
public class GmlDocumentIdContext implements GMLReferenceResolver {
	...
	/**
	 * Creates a new {@link GmlDocumentIdContext} instance for a GML document of the given
	 * version.
	 ...
	 * @param schema application schema to use for parsing external references
	 */
	...
	/**
	 * Adds a new {@link GMLObject} that has been encountered during the parsing of the
	 * GML document.
	 * @param object GML object, must not be <code>null</code> and must not be of type
	 * {@link GMLReference}
	 */
	...
	/**
	 * Adds a new {@link GMLReference} that has been encountered during the parsing of the
	 * GML document.
	 * @param ref GML reference, must not be <code>null</code>
	 */
	...
	/**
	 * Returns all {@link GMLObject} (but no {@link GMLReference} instances) that have
	 * been added.
	 * @return all gml objects that have been added before, may be empty, but never
	 * <code>null</code>
	 */
	 ...
	 * @return all gml references that have been added before, may be empty, but never
	 * <code>null</code>
	 */
	...
	public GMLObject getObject(String uri, String baseURL) {
		if (uri.startsWith("#")) {
			...
			if (!skipUrnWarning) {
				LOG.warn(
						"Unable to resolve external object reference: {}. Resolving of urn references is not implemented yet.",
						uri);
			}
		}
	 ...
	 * objects.
	 * @throws ReferenceResolvingException if a local reference cannot be resolved
	 */
