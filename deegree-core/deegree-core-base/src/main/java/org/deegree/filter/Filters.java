...
public class Filters {
	...
	/**
	 * Tries to extract a {@link BBOX} constraint from the given {@link Filter} that can
	 ...
	 * @param bbox bounding box, can be <code>null</code>
	 * @param filter filter expression, can be <code>null</code>
	 * @param propName can be <code>null</code>
	 * @return combined filter or <code>null</code> (if bbox and filter are
	 * <code>null</code>)
	 */
	...
	/**
	 * Returns all {@link ValueReference}s contained in the given {@link Filter} (taking
	 * nesting into account).
	 * @param filter filter to be traversed, must not be <code>null</code>
	 * @return {@link ValueReference}s found on any nodes of the {@link Filter}, can be
	...
	/**
	 * Returns all {@link Geometry}-values contained in the given {@link Filter} (taking
	 * nesting into account).
	 * @param filter filter to be traversed, must not be <code>null</code>
	 * @return {@link Geometry}s found on any nodes of the {@link Filter}, can be empty,
	...
	/**
	 * Sets the {@link CRS} for all geometries contained in the given {@link Filter} that
	 * do not have crs information.
	 * @param filter filter to process, must not be <code>null</code>
	 * @param crs crs to set, must not be <code>null</code>
	 */
	public static void setDefaultCRS(Filter filter, ICRS crs) {
		for (Geometry geom : getGeometries(filter)) {
			if (geom.getCoordinateSystem() == null) {
				// TODO propagate to deeper levels / change behavior of
				// setCoordinateSystem()
	...
	/**
	 * Tries to repair an expression's broken namespace bindings using the hints in the
	 * map.
	 * @param e
	 * @param bindings binds local names to qualified names
	 * @param validNames a set of valid qnames
	...
	public static Expression repair(Expression e, Map<String, QName> bindings, Set<QName> validNames) {
		switch (e.getType()) {
			...
			case FUNCTION:
				// workaround seems to produce errors, so function expressions are not
				// fixed now
				return e;
			...
			case VALUE_REFERENCE:
				...
				if (name != null && !validNames.contains(name)) {
					if (!bindings.containsKey(name.getLocalPart())) {
						LOG.warn(
								"Error while trying to repair an expression: local name {} could still not be resolved to a proper qname.",
								name.getLocalPart());
						return e;
					}
