...
public class Filter110XMLDecoder {
	...
	/**
	 * Returns the object representation for the given <code>wfs:Filter</code> element
	 ...
	 * (&lt;wfs:Filter&gt;), points at the corresponding <code>END_ELEMENT</code> event
	 * (&lt;/wfs:Filter&gt;) afterwards
	 * @return corresponding {@link Filter} object
	 * @throws XMLParsingException if the element is not a valid "wfs:Filter" element
	...
	public static Filter parse(XMLStreamReader xmlStream) throws XMLParsingException, XMLStreamException {
		...
		if (xmlStream.getEventType() != START_ELEMENT) {
			throw new XMLParsingException(xmlStream,
					Messages.getMessage("FILTER_PARSER_FILTER_EMPTY", new QName(OGC_NS, "Filter")));
		}
	...
	/**
	 * Returns the object representation for the given <code>ogc:expression</code> element
	 * event that the cursor of the associated <code>XMLStreamReader</code> points at.
	 * <p>
	 ...
	 * <li>ogc:Literal</li>
	 * <li>ogc:Function</li>
	 * <li>substitution for ogc:expression (handled by {@link CustomExpression}
	 * instance)</li>
	 * </ul>
	 * </p>
	 * <p>
	 ...
	 * (&lt;ogc:expression&gt;)</li>
	 * <li>Postcondition: cursor points at the corresponding <code>END_ELEMENT</code>
	 * event (&lt;/ogc:expression&gt;)</li>
	 * </ul>
	 * </p>
	 * @param xmlStream cursor must point at the <code>START_ELEMENT</code> event
	 * (&lt;ogc:expression&gt;), points at the corresponding <code>END_ELEMENT</code>
	 * event (&lt;/ogc:expression&gt;) afterwards
	 * @return corresponding {@link Expression} object
	 * @throws XMLParsingException if the element is not a valid "ogc:expression" element
	 * @throws XMLStreamException
	 */
	...
	/**
	 * Returns the object representation for the given custom <code>ogc:expression</code>
	...
	/**
	 * Returns the object representation for the given <code>ogc:comparisonOps</code>
	 * element event that the cursor of the associated <code>XMLStreamReader</code> points
	 * at.
	 * <p>
	 * The element must be one of the following:
	 ...
	 * </p>
	 * @param xmlStream cursor must point at the <code>START_ELEMENT</code> event
	 * (&lt;ogc:comparisonOps&gt;), points at the corresponding <code>END_ELEMENT</code>
	 * event (&lt;/ogc:comparisonOps&gt;) afterwards
	 * @return corresponding {@link Expression} object
	...
	private static IdFilter parseIdFilter(XMLStreamReader xmlStream) throws XMLStreamException {
		...
		// needed to check that only one type of identifier elements occur, OGC 04-095,
		// 11.2, p.15
	...
	private static ValueReference parsePropertyName(XMLStreamReader xmlStream, boolean permitEmpty)
		...
		if (!permitEmpty && xpath.isEmpty()) {
			// TODO filter encoding guy: use whatever exception shall be used here. But
			// make sure that the
			// GetObservation100XMLAdapter gets an exception from here as the compliance
			// of the SOS hangs on it's thread
			throw new XMLParsingException(xmlStream,
					Messages.getMessage("FILTER_PARSER_PROPERTY_NAME_EMPTY", new QName(OGC_NS, "PropertyName")));
		}
		if (xpath.isEmpty()) {
			return null;
		}
	...
	private static SpatialOperator parseSpatialOperator(XMLStreamReader xmlStream) throws XMLStreamException {
		...
		try {
			switch (type) {
				case BBOX: {
					// first parameter: 'ogc:PropertyName' (can be empty / omitted
					// completely
