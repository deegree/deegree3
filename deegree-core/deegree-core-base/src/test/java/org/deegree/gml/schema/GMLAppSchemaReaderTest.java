...
public class GMLAppSchemaReaderTest {
	...
	public void testCite100GeometryFeatureCollection()
		...
		GMLAppSchemaReader adapter = new GMLAppSchemaReader(null, null, schemaURL);
		AppSchema schema = adapter.extractAppSchema();
		FeatureCollectionType ft = (FeatureCollectionType) schema
			.getFeatureType(QName.valueOf("{http://www.opengis.net/gml}_FeatureCollection"));
		ft = (FeatureCollectionType) schema
			.getFeatureType(QName.valueOf("{http://www.opengis.net/cite/geometry}GeometryFeatureCollection"));
	...
	public void testAIXMCustomGeometryDeclarations()
		...
		// gml:pos/gml:coordinates are actually part of a choice (that's why minOccurs is
		// 1)
	...
	public void testIncludedGml321BaseSchemaIncludesGmlIdCorrigendum()
			throws ClassCastException, ClassNotFoundException, InstantiationException, IllegalAccessException {
		// schemaUrl is internally redirected to copy of the schema in the
		// deegree-ogcschemas.jar
		...
		for (Object object : typeDefinition.getAttributeUses()) {
			...
			if (attrDecl.getNamespace().equals("http://www.opengis.net/gml/3.2") && attrDecl.getName().equals("id")) {
