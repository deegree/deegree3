...
public class DefaultWorkspace implements Workspace {
	...
	@Override
	public <T extends Resource> ResourceMetadata<T> getResourceMetadata(
			Class<? extends ResourceProvider<T>> providerClass, String id) {
		...
		for (ResourceMetadata<? extends Resource> metadata : mdList) {
			...
			try {
				...
				throw new ResourceInitException(
						"Unable to build resource " + metadata.getIdentifier() + ": " + ex.getLocalizedMessage(), ex);
			}
		}
	...
	@Override
	public <T extends Resource> List<ResourceIdentifier<T>> getResourcesOfType(
