...
/**
 * Responsible for building a graph network based on a list of resource metadata objects.
 * The metadata must obviously be prepared, so they knows their dependencies.
 *
...
public class ResourceGraph {
	...
	/**
	 * @param metadata a list of prepared metadata objects, never <code>null</code>
	 */
	 ...
	 * @return a single node of the dependency network, <code>null</code> if no such node
	 * exists
	 */
	...
	/**
	 * The list is sorted by initialization order, with resources that need to be
	 * initialized first at the beginning of the list.
	...
	public List<ResourceMetadata<? extends Resource>> toSortedList() {
		// sketch: first add resources without dependencies, then add resources whose
		// dependencies are met until done
