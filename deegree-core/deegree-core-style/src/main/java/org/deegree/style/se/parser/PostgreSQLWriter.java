...
public class PostgreSQLWriter {
	...
	private int write(Connection conn, Graphic graphic) throws SQLException {
	...
	private int write(Connection conn, Fill fill) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn
				.prepareStatement("insert into " + schema + ".fills (color, graphic_id) values (?, ?) returning id");
	...
	private int write(Connection conn, Stroke stroke) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement("insert into " + schema
	...
	private int write(Connection conn, Font font) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement(
					"insert into " + schema + ".fonts (family, style, bold, size) values (?, ?, ?, ?) returning id");
	...
	private int write(Connection conn, LinePlacement lineplacement) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement("insert into " + schema
	...
	private int write(Connection conn, Halo halo) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn
				.prepareStatement("insert into " + schema + ".halos (fill_id, radius) values (?, ?) returning id");
	...
	private int write(Connection conn, PointStyling styling) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn
				.prepareStatement("insert into " + schema + ".points (uom, graphic_id) values (?, ?) returning id");
	...
	private int write(Connection conn, LineStyling styling) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement("insert into " + schema
	...
	private int write(Connection conn, PolygonStyling styling) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement("insert into " + schema
	...
	private int write(Connection conn, TextStyling styling, String labelexpr) throws SQLException {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			stmt = conn.prepareStatement("insert into " + schema
	...
	private void write(Styling<?> styling, DoublePair scales, String name, String labelexpr) {
		...
		try {
			...
			stmt = conn.prepareStatement(
					"insert into " + schema + ".styles (type, fk, minscale, maxscale, name) values (?, ?, ?, ?, ?)");
		...
		for (Triple<LinkedList<Styling>, DoublePair, LinkedList<String>> p : style.getBasesWithScales()) {
			...
			for (Styling s : p.first) {
				write(s, p.second, name == null ? style.getName() : name,
						labelexprs.hasNext() ? labelexprs.next() : null);
			}
		}
	}
	...
	public static void main(String[] args) throws XMLStreamException, FactoryConfigurationError, IOException {
		Style style = new SymbologyParser(true)
			.parse(XMLInputFactory.newInstance().createXMLStreamReader(new FileInputStream(args[0])));
		...
		ResourceLocation<ConnectionProvider> loc = ConnectionProviderUtils.getSyntheticProvider("configtool",
				"jdbc:postgresql://localhost/configtool", "postgres", "postgres");
