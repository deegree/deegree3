...
public class ShapeHelper {
	...
	/**
	 * Example: calculateStarPolygon (5, 2) connects the first, third, fifth, second,
	 * fourth and again first edge in that order.
	 * @param edges the number of edges
	 * @param skip specifies, how to connect the edges
	 * @param size
	...
	public static Shape getShapeFromMark(Mark mark, double size, double rotation, boolean translate, double x,
			double y) {
		Shape shape;
	...
	public static Shape getShapeFromSvg(InputStream in, String url) {
		try {
			...
			if (SVG_TO_SHAPE_FALLBACK) {
				// TRICKY setting transform on elements interferes with the svg coordinate
				// system / viewbox
				// use only as fallback if all styles are already adapted to previous
				// scaling
				...
				// should not include root's shape in the path as it doesn't always work
				// properly
