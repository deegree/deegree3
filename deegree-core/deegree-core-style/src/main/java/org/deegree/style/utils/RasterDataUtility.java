...
/**
 * Utility class to interpret raster data in a consistent way. It should be used in raster
 * lookup operations (for example in Categorize or Interpolate). This class can read 1, 2,
 * 3 and 4-band rasters, and combines band values into a single pixel value. RGB bands are
 * combined and their average value is returned (yielding the overall gray-pixel intensity
 * of a pixel), and alpha bands are ignored.
 *
 * This class can perform contrast enhancement on-the-fly.
 *
 * If channel mappings are provided, this class can process rasters with any number of
 * bands.
 *
...
public class RasterDataUtility {
	...
	/*
	 * Contrast Enhancement: Normalize. lower and upper bounds of raster data in scope,
	 * and lower and upper bounds of datatype.
	 */
	...
	/**
	 * @param raster abstract raster data source
	 */
	...
	/**
	 * Create a RasterDataUtility object, where channels are mapped according to the
	 * RasterStyle options
	 * @param raster
	 * @param style
	 */
	...
	/**
	 * Update the contrast enhancement. Method getEnhanced(row, col, band) will apply this
	 * contrast enhancement before returning values.
	 * @param ce
	 */
	...
	public float get(int col, int row) {
		...
		switch (data.getDataType()) {
			case BYTE:
				...
				// Compensate for byte being a signed datatype. This is commonly used in
				// grayscale images, range 0-255
				if (ret < 0)
	...
	public float getEnhanced(int col, int row, int band) {
		...
		switch (data.getDataType()) {
			case BYTE:
				...
				// Compensate for byte being a signed datatype (-128...127). This is
				// commonly used in grayscale images,
				// range 0-255
				if (i < 0)
					i += -2 * Byte.MIN_VALUE;
				if (contrast != null) {
			...
			default:
				...
				throw new UnsupportedOperationException(
						"Datatype '" + data.getDataType() + "' is not suitable for gamma correction.");
		}
	...
	/**
	 * Precomputes weight factors for applying contrast enhancements to the current
	 * raster. Can apply enhancements to all channels (if <code>index</code> is less than
	 * 0) or to a particular channel (if <code>index</code> is non-negative). This
	 * function is relevant for normalize or histogram enhancements, where the operation
	 * is dependent on the input raster data. All subsequent calls to
	 * <code>getEnhanced(col,row)</code> will use this contrast enhancement.
	 *
	 * NOTE: This function overrides the effects of <code>setGamma()</code>.
	 * @param index target channel number.
	 * @param enhancement desired contrast enhancement
	 */
		...
		if (enhancement.histogram) {
			switch (data.getDataType()) {
				case BYTE:
					...
					// And precompute the histogram normalization lookup table (only for
					// BYTE data)
					for (i = 0; i < 256; i++)
