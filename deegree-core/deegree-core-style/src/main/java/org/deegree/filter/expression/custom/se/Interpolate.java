...
public class Interpolate extends AbstractCustomExpression {
	...
	private Interpolate(StringBuffer value, Continuation<StringBuffer> contn, LinkedList<Double> datas,
			...
			LinkedList<Continuation<StringBuffer>> valueContns, boolean color, Mode mode, Color fallbackColor) {
		this.value = value;
		this.contn = contn;
		this.datas = datas;
	...
	/*
	 * Cubic interpolation between y1 and y2, with fraction f. y0 and y3 are extra points,
	 * such that y0-y1-y2-y3 are ordered.
	 */
	...
	private Color interpolateColor(final int pos1, final int pos2, final double f) {
		...
		switch (mode) {
			...
			case Cubic:
				...
				// Create extra 2 points with the same slope on both sides of the input
				// points
				double r, g, b, a;
				double r1 = colorArray[pos1].getRed(), g1 = colorArray[pos1].getGreen(),
						b1 = colorArray[pos1].getBlue(), a1 = colorArray[pos1].getAlpha();

				double r2 = colorArray[pos2].getRed(), g2 = colorArray[pos2].getGreen(),
						b2 = colorArray[pos2].getBlue(), a2 = colorArray[pos2].getAlpha();

				if (pos1 == 0 || pos2 == colorArray.length - 1) {
					// Cubic interpolation needs 4 points, not just two. Interpolate for
					// each of RGBA channels.
					double aux1, aux2;
		...
		switch (mode) {
			...
			case Cubic:
				if (pos1 == 0 || pos2 == valuesArray.length - 1) {
					...
					// If we are at the first/last intervals, create 2 new points by
					// interpolating linearly, on both sides
	...
	@Override
	public Interpolate parse(XMLStreamReader in) throws XMLStreamException {
		...
		Interpolate inp = new Interpolate(value, contn, datas, dataArray, values, valuesArray, colorArray, valueContns,
				color, mode, fallbackColor);
		inp.buildLookupArrays();
		return inp;
	}

	private void parseInterpolationPoint(XMLStreamReader in, LinkedList<Double> datas, LinkedList<StringBuffer> values,
			LinkedList<Continuation<StringBuffer>> valueContns) throws XMLStreamException {
	 ...
	 */
	public static Interpolate parseSLD100(XMLStreamReader in) throws XMLStreamException {
	...
	/**
	 * Performs interpolation on a value, and returns a color built from the interpolated
	 * value. Uses binary search for optimization.
	 * @param value value
