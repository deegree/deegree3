...
/**
 * Converts a generic deegree {@link Geometry} instance (which may be anything from the
 * ISO 19107 model) into a {@link Geometry} that matches the capabilities of the
 ...
 * <li>{@link Polygon} -> {@link Polygon} (using only a patch with linear interpolated
 * boundaries)</li>
 * <li>{@link Surface} -> {@link Polygon}/{@link MultiPolygon} (one member for every
 * surface patch, each using linear interpolated boundaries)</li>
 ...
 * <li>{@link MultiPolygon} -> {@link MultiPolygon} (members using only a patch with
 * linear interpolated boundaries)</li>
 * <li>{@link MultiSurface} -> {@link MultiPolygon} (members using only a single patches
 * with linear interpolated boundaries)</li>
 * </ul>
...
public class SFSProfiler {
	...
	/**
	 * Creates a new {@link SFSProfiler} that uses the given
	 * {@link LinearizationCriterion} for linearizing non-linear curve segments / surface
	 * boundaries.
	 ...
	 * @param geometry the geometry to be simplified, must not be <code>null</code>
	 ...
	 * @param geometry the geometry to be simplified, must not be <code>null</code>
	 * @return the simplified geometry, either a {@link Polygon} or a {@link MultiPolygon}
	 * (if the input surface has multiple patches)
	 */
