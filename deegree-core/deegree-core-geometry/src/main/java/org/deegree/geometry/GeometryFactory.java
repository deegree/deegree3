...
/**
 * Augments the {@link SimpleGeometryFactory} with additional methods for building complex
 * {@link Geometry} and geometry-related objects ({@link CurveSegment}s and
 * {@link SurfacePatch}es).
 *
...
public class GeometryFactory extends SimpleGeometryFactory {
	 ...
	 * @param p1 first control point
	 * @param p2 second control point
	 * @param bulge height of the arc (multiplier for the normals)
	 * @param normal normal vector, in 2D only one coordinate is necessary
	 ...
	 * @param points control points, must contain <code>2 * k + 1</code> points (k >= 1)
	 ...
	 * coordinates directly. The control point sequence consists of the start and end
	 * points of each arc plus the bulge.
	 * @param points list of control points, must contain at least two points
	 * @param bulges heights of the arcs (multipliers for the normals)
	 ...
	 * @param degree polynomial degree of the spline
	 * @param knot1 first of the two knots that define the spline basis functions
	 * @param knot2 second of the two knots that define the spline basis functions
	 ...
	 * @param points list of control points
	 * @param degree polynomial degree of the spline
	 * @param knots sequence of distinct knots that define the spline basis functions
	 * @param isPolynomial set to true if this is a polynomial spline, otherwise it's a
	 * rational spline
	...
	/**
	 * Creates a {@link Surface} that consists of a number of {@link SurfacePatch}
	 * instances. The passed patches must touch in a topological sense to form a valid
	 * {@link Surface}.
	 * @param id identifier of the new geometry instance
	 * @param patches patches to create a surface
	 ...
	 * @param isReversed set to true, if the orientation of the base curve shall be
	 * reversed in the created geometry
	 ...
	 * @param id identifier, may be null
	 * @param crs coordinate reference system, may be null
	 * @param memberPatches patches that constitute the surface
	 ...
	 * @param referenceLocation the affine mapping that places the curve defined by the
	 * Fresnel Integrals into the coordinate reference system of this object
	 * @param scaleFactor the value for the constant in the Fresnel's integrals
	 * @param startParameter the arc length distance from the inflection point that will
	 * be the start point for this curve segment
	 * @param endParameter the arc length distance from the inflection point that will be
	 * the end point for this curve segment
	 ...
	 * @param grid the grid of control points that defines the Cone
	 ...
	 * @param grid the grid of control points that defines the Cylinder
	 ...
	 * @param grid the grid of control points that defines the Sphere
	 ...
	 * @param id identifier, may be null
	 * @param crs coordinate reference system, may be null
	 * @param members surfaces that constitute the collection
	 ...
	 * @param id identifier, may be null
	 * @param crs coordinate reference system, may be null
	 * @param members solids that constitute the collection
	 ...
	 * @param id identifier, may be null
	 * @param crs coordinate reference system, may be null
	 * @param members curves that constitute the composited curve, each curve must end at
	 * the start point of the subsequent curve in the list
	 ...
	 * must join in pairs on common boundary curves and must, when considered as a whole,
	 ...
	 * @param id identifier, may be null
	 * @param crs coordinate reference system, may be null
	 * @param memberSolids solids that constitute the composited solid, the solids must
	 * join in pairs on common boundary surfaces and which, when considered as a whole,
	 * form a single solid
	...
	public CompositeGeometry<GeometricPrimitive> createCompositeGeometry(String id, ICRS crs,
		...
		return (CompositeGeometry<GeometricPrimitive>) inspect(
				new DefaultCompositeGeometry(id, crs, pm, memberPrimitives));
	}

	/**
	 * Creates a {@link GriddedSurfacePatch} from the given list of points, each element
	 * denotes a row in the grid.
	 * @param grid containing the rows of the grid.
