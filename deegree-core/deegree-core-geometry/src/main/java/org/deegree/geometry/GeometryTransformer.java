...
/**
 *
 * Transforms a geometry defined in a {@link CRS} into a geometry defined in another
 * {@link CRS}
 *
...
public class GeometryTransformer extends Transformer {
	 ...
	 * @param targetCRS an identifier to which all other CRS's shall be transformed.
	 ...
	 * @throws IllegalArgumentException if the coordinates system of the geometry is
	 * <code>null</code>
	...
	/**
	 * transforms the coordinates of a deegree geometry to the target coordinate reference
	 * system. Optionally tests whether the given Geometry lies within the source crs'
	 * valid domain.
	 * @param geo to be transformed
	 * @param testValidDomain true if the incoming geometry should be checked against the
	 * domain of validity of the CoordinateSystem it is defined in.
	 * @return the same geometry in a different crs.
	 * @throws TransformationException if the transformation between the source and target
	 * crs cannot be created.
	 * @throws IllegalArgumentException if the coordinates system of the geometry is
	...
	/**
	 * Creates an envelope in the given CRS of the Area of Use (defined in WGS84) of the
	 * given Coordinate System.
	 * @param sourceCRS to get the area of use from.
	 * @return an Envelope with corresponding ordinates of the area of use BBox of the
	 * given Coordinate system, if the AreaOfUse was defined as -180,-90,180,90 BBox or if
	 * the envelope could not be transformed <code>null</code> will be returned.
	 */
	public static Envelope createValidDomain(ICRS sourceCRS) {
		...
		if (validDomain != null) {
			return geomFactory.createEnvelope(new double[] { validDomain[0], validDomain[1] },
					new double[] { validDomain[2], validDomain[3] }, CRSManager.getCRSRef(sourceCRS));
		}
		return null;
	}
	 ...
	 * system.
	 * @param geo to be transformed
	 * @param sourceCRS the source CRS for the geometry. overwrites the CRS of the
	 * geometry.
	 * @return the same geometry in a different crs.
	 * @throws TransformationException if the transformation between the source and target
	 * crs cannot be created.
	 * @throws IllegalArgumentException if the coordinates system of the geometry is
	 * <code>null</code>
	 * @throws UnknownCRSException if the given CRS is not found
	 */
	 ...
	 * system.
	 * @param <T>
	 * @param geom to be transformed
	 * @param sourceCRS the source CRS for the geometry. overwrites the CRS of the
	 * geometry.
	 * @param testValidArea true if the geometry should be be checked of the valid area of
	 * the source crs.
	...
	public <T extends Geometry> T transform(T geom, ICRS sourceCRS, boolean testValidArea,
			List<Transformation> toBeUsedTransformations) throws IllegalArgumentException, TransformationException {
	...
	private <T extends Geometry> T transform(T geo, Transformation trans, Envelope domainOfValidity)
		...
		if (domainOfValidity != null) {
			if (!insideValidDomain(domainOfValidity, geo)) {
				throw new OutsideCRSDomainException("Geometry (gml:id=" + (isSet(geo.getId()) ? geo.getId() : "not set")
		...
		try {
			...
			throw new TransformationException(Messages.getMessage("CRS_TRANSFORMATION_ERROR",
					geo.getCoordinateSystem().getAlias(), getTargetCRS().getCodes(), ge.getMessage()), ge);
		}
		return null;
	}

	private GeometricPrimitive transform(GeometricPrimitive geom, Transformation trans) throws TransformationException {
	...
	private MultiSolid transform(MultiSolid geom, Transformation trans) throws TransformationException {
	...
	private Solid transform(Solid solid, Transformation trans) throws TransformationException {
	...
	private CompositeSolid transform(CompositeSolid cSolid, Transformation trans) throws TransformationException {
	...
	private Geometry transform(Envelope envelope, Transformation trans) throws TransformationException {
		return transform(envelope, trans, 20);
	}

	/**
	 * Transform the geometry of an envelope. It substitutes the envelope (two points)
	 * with a polygon with <code>numPoints</code> points. The points are distributed
	 * evenly on the boundary of the envelope to provide a more accurate transformed
	 * envelope. This is useful if the transformation rotates and distorts.
	 * @param envelope
	 * @param trans
	 * @param numPoints
	 ...
	 * @throws TransformationException
	 */
	private Envelope transform(Envelope envelope, Transformation trans, int numPoints) throws TransformationException {
	...
	private LineString transform(LineString geo, Transformation trans) throws TransformationException {
		LineStringSegment segment = (LineStringSegment) geo.getCurveSegments().get(0); // only
																						// one
																						// for
																						// a
																						// line
	 ...
	 * @throws TransformationException
	 */
	private Curve transform(Curve curve, Transformation trans) throws TransformationException {
	...
	private CompositeCurve transform(CompositeCurve cCurve, Transformation trans) throws TransformationException {
	...
	private CurveSegment transform(CurveSegment segment, Transformation trans) throws TransformationException {
		...
		switch (segmentType) {
			...
			case OFFSET_CURVE:
				...
				transformedSegment = geomFactory.createOffsetCurve(ocbc, oc.getDirection(),
						(Length) oc.getDistance(null));
				break;
		}
		return transformedSegment;
	}

	private MultiLineString transform(MultiLineString geo, Transformation trans) throws TransformationException {
	 ...
	 * @throws TransformationException
	 */
	private MultiCurve<Curve> transform(MultiCurve<Curve> geo, Transformation trans) throws TransformationException {
	 ...
	 * @throws TransformationException
	 */
	private MultiPoint transform(MultiPoint geo, Transformation trans) throws TransformationException {
	...
	private MultiPolygon transform(MultiPolygon multiPolygon, Transformation trans) throws TransformationException {
	 ...
	 * @throws TransformationException
	 */
	private Points transform(Points points, Transformation trans) throws TransformationException {
		...
		for (Point point : points) {
			...
			if (Double.isNaN(point.get2())) {
				...
				// pass the 3rd coordinate if exist and dimension of source and target CRS
				// is 2
				if (trans.getSourceCRS().getDimension() == 2 && trans.getTargetCRS().getDimension() == 2) {
					tmp.z = point.get2();
				}
				result
					.add(geomFactory.createPoint(point.getId(), new double[] { tmp.x, tmp.y, tmp.z }, getTargetCRS()));
			}
		}
	 ...
	 * @throws TransformationException
	 */
	private Point transform(Point geo, Transformation trans) throws TransformationException {
		...
		if (Double.isNaN(geo.get2())) {
			...
			// pass the 3rd coordinate if exist and dimension of source and target CRS is
			// 2
			result.z = geo.get2();
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private Surface transform(Surface surface, Transformation trans) throws TransformationException {
		Surface result = null;
	...
	@SuppressWarnings("unchecked")
	private Tin transform(Tin tin, Transformation trans) throws TransformationException {
	...
	private Polygon transform(Polygon polygon, Transformation trans) throws TransformationException {
	...
	private LinearRing transform(Ring ring, Transformation trans) throws TransformationException {
		if (ring != null) {
			// TODO DefaultRing.getAsLineString currently returns an
	...
	private PolygonPatch transform(PolygonPatch patch, Transformation trans) throws TransformationException {
		...
		switch (type) {
			case POLYGON_PATCH:
				...
				List<Ring> transformedInteriorRings = new ArrayList<Ring>(
						interiorRings == null ? 0 : interiorRings.size());
				if (interiorRings != null && !interiorRings.isEmpty()) {
					for (Ring interior : interiorRings) {
						if (interior != null) {
	...
	private SurfacePatch transform(GriddedSurfacePatch patch, Transformation trans) throws TransformationException {
