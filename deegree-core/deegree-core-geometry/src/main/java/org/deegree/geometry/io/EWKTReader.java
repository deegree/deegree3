...
public class EWKTReader {
	 ...
	 * @param reader a Reader which will return a <Geometry Tagged Text> string (see the
	 ...
	 * @return the next array of <code>Coordinate</code>s in the stream, or an empty array
	 * if EMPTY is the next element returned by the stream.
	 * @throws IOException if an I/O error occurs
	 * @throws ParseException if an unexpected token was encountered
	 */
	private List<Point> getCoordinates() throws IOException, ParseException {
	...
	private List<Point> getCoordinateList(boolean openExpected) throws IOException, ParseException {
		String nextToken;
		if (openExpected) {
	 ...
	 * <tt>NaN</tt> values are handled correctly, and the case of the "NaN" token is not
	 * significant.
	 ...
	 * shell and holes do not form closed linestrings, or if an unexpected token was
	 * encountered
	 * @throws IOException if an I/O error occurs
	 */
	 ...
	 * @throws ParseException
	 */
	private ArcString readCircularStringText() throws IOException, ParseException {
	...
	/**
	 * Handles mixed line string notation - either LineString (the default) or
	 * CircularCurve. Isolated as a seperate method as I think we will need to call this
	 * from the polygon code.
	 * @return List of LineString (defined in a mixed format)
	 * @throws IOException
	 * @throws ParseException
	 */
	List<Curve> getLineStrings() throws IOException, ParseException {
	...
	/**
	 * This method will read a LineString, CircularString or CompoundCurve and return the
	 * result as a LinearRing.
	 * @return LinearRing
	 * <p>
	 ...
	 * @throws ParseException
	 */
	private Curve readCurvedLinearRingText() throws IOException, ParseException {
		String nextWord = getNextWord();
		if (nextWord.equals(L_PAREN)) {
			List<Point> coords = getCoordinateList(false);
	 ...
	 * <code>LinearRing</code> do not form a closed linestring, or if an unexpected token
	 * was encountered
	 */
	 ...
	 * @throws ParseException if an unexpected token was encountered
	 */
	private List<Point> getCoordinatesForMultiPoint() throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener();
		if (nextToken.equals(EMPTY)) {
			return emptyList();
		}
	...
	private MultiCurve<?> readMultiCurveText() throws IOException, ParseException {
	 ...
	 * @return a <code>MultiPolygon</code> specified by the next token in the stream, or
	 * if if the coordinates used to create the <code>Polygon</code> shells and holes do
	 ...
	 * @return a <code>MultiSurface</code> specified by the next token in the stream, or
	 * if if the coordinates used to create the <code>Polygon</code> shells and holes do
	 * not form closed linestrings.
	 * @throws IOException if an I/O error occurs
	 * @throws ParseException if an unexpected token was encountered
	 */
	private MultiSurface<?> readMultiSurfaceText() throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener();
		if (nextToken.equals(EMPTY)) {
	 ...
	 * @throws IOException if an I/O error occurs
	 */
	private MultiGeometry<Geometry> readGeometryCollectionText() throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener();
		if (nextToken.equals(EMPTY)) {
