...
public class CurveLinearizer {
	 ...
	 * @param geomFac geometry factory to be used for creating {@link LineString}s and
	 * {@link LineStringSegments}, must not be <code>null</code>
	 */
	 ...
	 * <p>
	 * NOTE: This method respects the semantic difference between {@link Curve} and
	 * {@link Ring} geometries: if the input is a {@link Ring}, a ring geometry will be
	 * returned.
	 * @param curve curve to be linearized, must not be <code>null</code>
	 ...
	 * {@link CurveSegment}.
	 * @param segment segment to be linearized, must not be <code>null</code>
	 * @param crit determines the interpolation quality / number of interpolation points,
	 ...
	 * {@link Arc}.
	 * <p>
	 * If the three control points <code>p0</code>, <code>p1</code> and <code>p2</code> of
	 * the arc are collinear, i.e. on a straight line, the behaviour depends on the type
	 * of {@link Arc}:
	 * <ul>
	 ...
	 * </ul>
	 * @param arc segment to be linearized, must not be <code>null</code>
	 * @param crit determines the interpolation quality / number of interpolation points,
	 * must not be <code>null</code>
	 * @return linearized version of the input segment, never <code>null</code>
	 */
	public LineStringSegment linearizeArc(Arc arc, LinearizationCriterion crit) {
		...
		if (areCollinear(arc.getPoint1(), arc.getPoint2(), arc.getPoint3())) {
			// non-standard special case full circle defined by diameter (p0 - p1, p0 ==
			// p2)
			if (arc.getPoint1().equals(arc.getPoint3())) {
				return linearizeArcString(arc, crit);
			}

			// if the points are already on a line we don't need to (and must not) apply
			// any linearization algorithm
			Points points = null;
			...
			lineSegment = geomFac.createLineStringSegment(
					interpolate(arc.getPoint1(), arc.getPoint2(), arc.getPoint3(), numPoints, arc instanceof Circle));
		}
	 ...
	 * </p>
	 * @param arcString curve segment to be linearized, must not be <code>null</code>
	 * @param crit determines the interpolation quality / number of interpolation points,
	 * must not be <code>null</code>
	 ...
	 * {@link CubicSpline}.
	 * <p>
	 * A cubic spline consists of n polynomials of degree 3: S<sub>j</sub>(x) =
	 * a<sub>j</sub> + b<sub>j</sub>*(x-x<sub>j</sub>) +
	 * c<sub>j</sub>*(x-x<sub>j</sub>)<sup>2</sup> +
	 * d<sub>j</sub>*(x-x<sub>j</sub>)<sup>3</sup>; that acts upon the interval
	 * [x<sub>j</sub>,x<sub>j+1</sub>], 0 <=j< n.
	 * <p>
	 * The algorithm for generating points on a spline defined with only control points
	 * and starting/ending tangents can be found at <a href=
	 * "http://persson.berkeley.edu/128A/lec14-2x3.pdf">http://persson.berkeley.edu/128A/lec14-2x3.pdf</a>
	 * (last visited 19/08/09)
	 * @param spline curve segment to be linearized, must not be <code>null</code>
	 * @param crit determines the interpolation quality / number of interpolation points,
	 * must not be <code>null</code>
	 * @return linearized version of the input segment, never <code>null</code>
	 */
	public LineStringSegment linearizeCubicSpline(CubicSpline spline, LinearizationCriterion crit) {
		...
		if (!ascending) {
			// interchange the elements so that they are ordered ascendingly (on the
			// X-axis)
			for (int i = 0; i <= (n / 2); i++) {
				Point aux = pts[i];
				pts[i] = pts[n - i];
		...
		if (crit instanceof NumPointsCriterion) {
			...
			if (numPoints <= 0) {
				...
				// TODO it is mathematically hard to get an expression of the numOfPoints
				// with respect to the error;
				// there would be two work-arounds as I can see them: 1) through a
				// trial-and-error procedure determine
				// how small should the sampling interval be, so that the difference in
				// value is less than the
				// given error; 2) use the mathematical expression used for the arc/circle
				// (something with Math.acos...)
		...
		// populate a list of points, so that later a LineStringSegment can be built from
		// it
	...
	private double[] interpolateSpline(int n, double[] h, double[] xcoor, double[] ycoor, double[] vectorx,
		...
		for (int i = 0; i <= numPoints - 1; i++) {
			...
			double y = a[seg] + b[seg] * (x - xcoor[seg]) + c[seg] * Math.pow(x - xcoor[seg], 2)
					+ d[seg] * Math.pow(x - xcoor[seg], 3);
	...
	private double createAngleStep(double startAngle, double endAngle, int numPoints, boolean isClockwise) {
		...
		if (isClockwise) {
			if (!isCircle) {
				...
				if (sweepAngle < 0) {
					/**
					 * Because the sweepAngle is negative and we are going cw the
					 * sweepAngle must be inverted by adding it to 2pi
					 */
			...
			if (!isCircle) {
				if (sweepAngle < 0) {
					/**
					 * Because sweepangle is negative but we are going ccw the sweepangle
					 * must be mathematically inverted
					 */
	...
	private Points interpolateCircleByDiameter(Point p0, Point p1, int numPoints) {
		// shift the points down (to reduce the occurrence of floating point errors),
	...
	private Points interpolatePoints(double startAngle, double angleStep, double radius, int numPoints, double centerX,
			double centerY, double minOrd0, double minOrd1, Point startPointShifted, Point endPointShifted) {
	...
	/**
	 * calculate the required number of point for a circle where the points p0 and p1
	 * define the diameter
	 */
	...
	/**
	 * Finds the center of a circle/arc that is specified by three points that lie on the
	 * circle's boundary.
	 * <p>
	 * Credits go to <a href=
	 * "http://en.wikipedia.org/wiki/Circumradius#Coordinates_of_circumcenter">wikipedia</a>
	 * (visited on 13/08/09).
	 * </p>
	 ...
	 * @throws IllegalArgumentException if the points are collinear, i.e. on a single line
	 */
	...
	/**
	 * Find the midpoint between the highest and the lowest ordonate 1 axis among the 3
	 * points
	 * @param p0
	 * @param p1
	 * @param p2
	...
	/**
	 * Find the midpoint between the highest and the lowest ordonate 0 axis among the 3
	 * points
	 * @param p0
	 * @param p1
	 * @param p2
	...
	/**
	 * Returns whether the order of the given three points is clockwise or
	 * counterclockwise. Uses the (signed) area of a planar triangle to get to know about
	 * the order of the points.
	 * @param p0 first point
	 * @param p1 second point
	 * @param p2 third point
	 * @return true, if order is clockwise, otherwise false
	 * @throws IllegalArgumentException if no order can be determined, because the points
	 * are identical or collinear
	 */
	boolean isClockwise(Point p0, Point p1, Point p2) throws IllegalArgumentException {
		if (areCollinear(p0, p1, p2)) {
			throw new IllegalArgumentException("Cannot evaluate isClockwise(). The three points are collinear.");
		}
		double res = (p2.get0() - p0.get0()) * ((p2.get1() + p0.get1()) / 2)
	 ...
	 * <p>
	 * NOTE: Only this method should be used throughout the whole linearization process
	 * for testing collinearity to avoid inconsistent results (the necessary EPSILON would
	 * differ).
	 * </p>
	 * @param p0 first point, must not be <code>null</code>
	 * @param p1 second point, must not be <code>null</code>
	 * @param p2 third point, must not be <code>null</code>
