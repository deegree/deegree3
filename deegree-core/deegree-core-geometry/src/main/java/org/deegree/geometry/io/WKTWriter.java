...
public class WKTWriter {
	 ...
	 * @param flags signaling the capabilities of this writer, if <code>null</code> only
	 * some geometries can be exported (all dialects are disabled).
	 * @param formatter to use, if <code>null</code> the default decimal formatter with 5
	 * decimals is used.
	 */
	 ...
	 * @param linearizedControlPoints the linearizedControlPoints to set
	 */
	...
	/**
	 * @param geometry that has to be written
	 * @param writer that is used
	 * @throws IOException
	 */
					...
					switch (gsp.getGriddedSurfaceType()) {
						case GRIDDED_SURFACE_PATCH:
							throw new UnsupportedOperationException(
									"Handling griddedSurfacePatch is not implemented yet.");
						case CONE:
	...
	private void writeCurvePolygon(Polygon geometry, Writer writer) throws IOException {
	...
	private void writeCurvePolygonWithoutPrefix(Polygon geometry, Writer writer) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void writePolygonWithoutPrefix(Surface geometry, Writer writer) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void writeCircle(Circle curve, Writer writer) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void writeArcString(ArcString curve, Writer writer) throws IOException {
		if (flags.contains(WKTFlag.USE_SQL_MM)) {
	 ...
	 * @throws IOException
	 */
	private void writeLineStringSegment(LineStringSegment curve, Writer writer) throws IOException {
		if (flags.contains(WKTFlag.USE_SQL_MM)) {
			// write nothing
		}
		else {
	 ...
	 * @throws IOException
	 */
	private void writeArc(Arc curve, Writer writer) throws IOException {
		if (flags.contains(WKTFlag.USE_SQL_MM)) {
			writer.append("CIRCULARSTRING ");
		}
		else {
	...
	/**
	 * Export the given geometry to wkt, without using any {@link WKTFlag}s. The writing
	 * is done to a StringWriter, which should be handled with care.
	 * @param geom to export.
	 * @return a wkt String representation of the given geometry, of the emtpy string if
	 * the geometry is <code>null</code>
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Export the given geometry to wkt, without using any {@link WKTFlag}s. The writing
	 * is done using the given writer instance, if no writer instance is given
	 * (<code>null</code>) a {@link NullPointerException} will be thrown.
	 * @param geom to export, if <code>null</code> the method will return.
	 ...
	 * <p>
	 * This specification comes from the GML and is not necessary for databases but maybe
	 * necessary for an export within deegree
	 * @param sb
	 * @param geom
	 * @throws IOException
	 */
	 ...
	 * @return true if the polygon is constructed from non-linear rings, rings which are
	 * non-linear or rings with contains more than one segment
	 */
	 ...
	 * @return true if the ring is non-linear or the ring is made from more than one
	 * segment
	 */
	 ...
	 * @return true if the curve is non-linear or the curve is made from more than one
	 * segment
	 */
