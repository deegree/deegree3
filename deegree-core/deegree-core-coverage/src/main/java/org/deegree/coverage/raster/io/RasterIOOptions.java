...
public class RasterIOOptions implements Serializable {
	...
	/**
	 * This key will get the default loading policy (resulting in a RasterDataContainer)
	 * of the RasterDataFactory.
	 */
	public static final String DATA_LOADING_POLICY = "LOADING_POLICIY";

	/**
	 * This key will get the location of the origin of a raster geo reference see
	 * {@link OriginLocation}
	 */
	...
	/**
	 * This key will get the local raster cache directory to be used inside the raster
	 * cache directory for the given raster. For example the raster cache directory is
	 * /tmp/ and the LOCAL_RASTER_CACHE_DIR is set to 'some_name' all raster caches will
	 * reside in /tmp/some_name/
	 */
	...
	/**
	 * Use this key to add the origin of the raster read, this might be handy if reading
	 * rasters from an URL (Stream) and the cache can use this id to load data from cache.
	 */
	...
	/**
	 * An empty constructor, nothing is set. The loading policy is the default value taken
	 * from the {@link RasterDataContainerFactory}. Worldfile reading is on.
	 */
	...
	/**
	 * Use this constructor if you read your raster reference some place else, or if you
	 * are reading from a stream. Be aware to set the filetype of the data as well. The
	 * loading policy is the default value taken from the
	 * {@link RasterDataContainerFactory}.
	 * @param reference of the file/stream to read.
	 */
	...
	/**
	 * Set the default loading policy to one configured in the
	 * {@link RasterDataContainerFactory}
	 * @param ref
	 * @param format of the raster to read, e.g. png, jpg, tiff..., may be
	 * <code>null</code>
	 */
	...
	/**
	 * @param originLocation to be used for reading worldfiles.
	 */
	...
	/**
	 * Return a RasterIOOption object with the format set according to the given file with
	 * an optional {@link RasterGeoReference}.
	 * @param file
	 * @param envelope
	 * @return RasterIOOption proper format.
	 */
	...
	/**
	 * @return the loading policy (and thus the raster data container) of the
	 * RasterDataFactory.
	 */
	...
	/**
	 * @param geoRef the raster geo reference to use for the loaded raster.
	 */
	...
	/**
	 * Returns the no data value. A no data value should be added to the
	 * {@link RasterIOOptions} by using {@link RasterIOOptions#setNoData(byte[])}. The
	 * byte[] can be created from an array of Strings by using the
	 * {@link RasterIOOptions#createNoData(String[], DataType)}.
	 * @return the no data values for the bands of the raster or <code>null</code> if no
	 * data was specified.
	 */
	...
	/**
	 * @return the location of the origin of the read file. If not defined this method
	 * will return return {@link RasterGeoReference.OriginLocation#CENTER};
	 *
	 */
	 ...
	 * {@link RasterIOOptions#createNoData(String[], DataType)}.
	 * @param noDataValue containing byte representations of the rasters no data value.
	 */
	...
	/**
	 * Create a noData array from the given strings. Each string will be interpreted as
	 * the given type.
	 * @param bandValues String representations of each bands no data value e.g {10.0,
	 * 80.5, -100} for a 3 band float raster.
	 * @param type of the no data values.
	 * @return a byte array containing the no data values for each band, the bytes can be
	 * 'reversed' by using the {@link ByteBuffer} methods. If either the type or the array
	 * is <code>null</code> <code>null</code> will be returned.
	 * @throws NumberFormatException if one of the Strings could not be decoded.
	 */
	public static byte[] createNoData(String[] bandValues, DataType type) throws NumberFormatException {
		byte[] result = null;
	...
	public RasterIOOptions copyOf(RasterIOOptions otherOptions) {
		...
		if (otherOptions != null) {
			...
			for (String key : otherOptions.options.keySet()) {
				if (!(key.equals(DATA_LOADING_POLICY) || key.equals(READ_WLD_FILE) || key.equals(GEO_ORIGIN_LOCATION)
						|| key.equals(CRS))) {
			...
			// add( CREATE_RASTER_MISSING_CACHE_DIR, otherOptions.get(
