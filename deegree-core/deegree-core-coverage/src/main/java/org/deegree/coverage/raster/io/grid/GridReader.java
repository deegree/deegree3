...
public abstract class GridReader implements RasterReader {
	 ...
	 * @param column column id, must be in the range [0 ... #columns - 1]
	 * @param row row id, must be in the range [0 ... #rows - 1]
	...
	public AbstractRaster getTile(int columnId, int rowId) throws IOException {
		...
		RasterGeoReference tileRasterReference = RasterGeoReference.create(OriginLocation.OUTER, tileEnvelope,
				infoFile.getTileRasterWidth(), infoFile.getTileRasterHeight());

		RasterRect tileRect = getGeoReference().createRelocatedReference(OriginLocation.OUTER)
			.convertEnvelopeToRasterCRS(tileEnvelope);
	...
	protected int[] getIntersectingTiles(RasterRect rect) {
		...
		if (fRect != null) {
			...
			if ((maxCol != -1) && (maxRow != -1) && (minCol != infoFile.columns()) && (minRow != infoFile.rows())) {
	 ...
	 * @param direct if the buffer should be direct
	 * @param forCache if the buffer is used for caching mechanisms.
