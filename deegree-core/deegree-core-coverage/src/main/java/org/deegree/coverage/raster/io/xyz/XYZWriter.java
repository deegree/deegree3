...
/**
 * The <code>XYZWriter</code> writes xyz, separated text files. The separator can be
 * defined in the RasterIOOptions, by supplying a value with the key
 * {@link XYZRasterIOProvider#XYZ_SEPARATOR}, if failing, the 'space' character will be
 * used.
 *
	...
	private void write(AbstractRaster raster, BufferedWriter writer, RasterIOOptions options) throws IOException {
		if (canWrite(raster, options)) {
			...
			RasterGeoReference rasterReference = simpleRaster.getRasterReference()
				.createRelocatedReference(OriginLocation.CENTER);
