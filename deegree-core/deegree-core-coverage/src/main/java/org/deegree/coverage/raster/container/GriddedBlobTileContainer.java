...
/**
 * Concrete implementation of {@link GriddedTileContainer} that extracts the tiles from a
 * blob with a custom format. See d3_tools/RasterTreeGridifier on how to create the
 * format.
 *
...
public class GriddedBlobTileContainer extends GriddedTileContainer {
	...
	/**
	 * A gridded tile container which reads data from a deegree internal format. See
	 * d3_tools/RasterTreeGridifier on how to create the format.
	 * @param metaInfoFile encapsulating the values read from the grid meta info file.
	 * @param blobDir
	 * @throws IOException
	 */
	public GriddedBlobTileContainer(GridMetaInfoFile metaInfoFile, File blobDir) throws IOException {
		...
		// long expectedBlobSize = metaInfoFile.rows() * metaInfoFile.columns() *
		// blobReaders[0].getBytesPerTile();
		// if ( expectedBlobSize != totalSize ) {
		// String msg = "Size of grid (all blob file) (=" + totalSize + ") does not match
		// the expected size (="
		// + expectedBlobSize + ").";
	...
	/**
	 * A gridded tile container which reads data from a deegree internal format. See
	 * d3_tools/RasterTreeGridifier on how to create the format. This methods takes a
	 * single bob file instead of scanning a given directory
	 * @param blobFile to read the tiles from.
	 * @param metaInfoFile
	 * @throws IOException
	 */
	...
	/**
	 * Reads the index file, 'gridded_raster.info' from the given directory reads the
	 * world file information as well as the number of tiles.
	 * @param dir to read the gridded_raster.info
	 * @param options which will hold information on the file.
	 * @return a {@link GriddedBlobTileContainer} instantiated with the values read from
	 * the gridded_raster.info.
	 * @throws IOException
	 */
