...
public class RasterTransformer extends Transformer {
	 ...
	 * @param targetCRS a coordinate system wrapper, wrapping the crs to which all
	 * incoming coordinates shall be transformed.
	 * @throws IllegalArgumentException if the given parameter is null.
	 */
	 ...
	 *
	 * <p>
	 * This method transforms the requested envelope and returns a new raster with the
	 * requested size. The source raster can be larger than the requested envelope (like a
	 * large tiled raster), or smaller (the source raster nodata value will be used
	 * outside the source raster).
	 * </p>
	 * <p>
	 * If the dstEnvelope does not contain a CRS or the CRS is the same as the
	 * {@link AbstractRaster#getCoordinateSystem()} only interpolation will be applied. If
	 * the requested size is the same as the number of rows/columns of the source raster,
	 * only the subset of the given raster will be returned, without interpolation being
	 * applied.
	 * </p>
	...
	public AbstractRaster transform(AbstractRaster sourceRaster, Envelope dstEnvelope, int dstWidth, int dstHeight,
			InterpolationType interpolationType) throws TransformationException {
		...
		synchronized (sourceRaster) {
			...
			RasterGeoReference dstREnv = RasterGeoReference
				.create(sourceRaster.getRasterReference().getOriginLocation(), dstEnvelope, dstWidth, dstHeight);

			// use warp to calculate the correct sample positions in the source raster.
			// the warp is a cubic polynomial function created of 100 points in the
			// dstEnvelope. This function will map
	...
	private void warpTransform(WarpPolynomial warp, Interpolation interpolation, RasterData dstData) {
		...
		for (int y = 0; y < dstData.getRows(); y++) {
			// look-up the pixel positions in the source raster for every pixel in this
			// row, the srcCoords will contain
			// the x,y ([2n],[2n+1]) values in the source raster (defined in the native
			// CRS) for this row of pixels.
	...
	private WarpPolynomial createWarp(int dstWidth, int dstHeight, ICRS srcCRS, RasterGeoReference srcREnv,
			RasterGeoReference dstREnv) throws TransformationException {
		int k = 0;
	 ...
	 *
	 * <p>
	 * This method transforms the whole raster into the target CRS of this
	 * RasterTransformer. The size of the output raster will be calculated, so that the
	 * pixels keep the aspect ratio (i.e. keep square pixels).
	 * </p>
	 * <p>
	 * If the coordinate system of the source raster is <code>null</code> or equals the
	 * target crs, the source raster will be returned unaltered.
	 * </p>
