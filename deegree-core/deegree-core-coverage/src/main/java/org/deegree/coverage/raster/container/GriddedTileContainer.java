...
/**
 * Abstract base class for {@link TileContainer}s based on a rectangular grid of disjunct,
...
public abstract class GriddedTileContainer implements TileContainer {
	 ...
	 * @param infoFile containing the relevant information.
	 *
	 */
	 ...
	 * Creates a new {@link GriddedTileContainer} instances.
	 * @param location
	 * @param envelope area of the samples of the contained tiles
	 * @param rows number of rows in the cell grid
	 * @param columns number of columns in the cell grid
	 * @param tileSamplesX number of samples of each raster tile in x-direction
	 * @param tileSamplesY number of samples of each raster tile in y-direction
	 */
	protected GriddedTileContainer(OriginLocation location, Envelope envelope, int rows, int columns, int tileSamplesX,
			int tileSamplesY) {
	 ...
	 * @param rowId row id, must in the range [0 ... #rows - 1]
	 * @param columnId column id, must be in the range [0 ... #columns - 1]
	 ...
	 * @param columnId column id, must be in the range [0 ... #columns - 1]
	 * @param rowId row id, must be in the range [0 ... #rows - 1]
	...
	private int getColumnIdx(double x) {
		...
		// String msg = "Specified x coordinate (=" + x + ") is out of range [" +
		// envelope.getMin().get0() + ";"
	...
	private int getRowIdx(double y) {
		...
		// String msg = "Specified y coordinate (=" + y + ") is out of range [" +
		...
		// int rowIdx = (int) Math.floor( ( ( rows * ( envelopeHeight - dy ) ) /
		// envelopeHeight ) );
		// if ( rowIdx < 0 ) {
		// // signal outside
		// return -1;
