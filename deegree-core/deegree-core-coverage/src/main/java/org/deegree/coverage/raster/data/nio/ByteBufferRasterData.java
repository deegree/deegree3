 ...
 *
 * <p>
 * It is based on java.nio.ByteBuffer and implements common get- and set-operations on the
 * data. The different InterleaveTypes are implemented by additional subclasses.
 *
 * <p>
 * get- and set-operations are implemented naive and access all data sample-wise. For
 * efficiency subclasses should overwrite methods that access more than one sample and
 * leverage the knowledge of the internal storage format (interleaving).
 *
 * <p>
 * TODO: Only implements access to byte and float data at the moment. Copy float methods
 * for other data types and change 'Float/float' to short, int, long or double. These
 * types are supported by ByteBuffer and the according methods only differ in the name of
 * the type (eg. getFloat, getInt, getDouble,...). Opposed to the methods for bytes, which
 * lack the type in the method names (eg. only get()).
 *
 * <p>
 * Also this implementation is able to store a sub-view on another
 * {@link ByteBufferRasterData}, resp. ByteBuffer. With this feature you are able to
 * create subsets without copying the data. Though the current deegree SimpleRaster
...
public abstract class ByteBufferRasterData implements RasterData {
	 ...
	 * @param view the new view on this data
	 ...
	 * @param view the raster rectangle defining the sample domain of this raster data.
	 * @param rasterWidth width of the raster data
	 * @param rasterHeight height of the raster data
	 * @param reader to be used to read the data from a location.
	 * @param originalDataInfo containing information about this raster data object
	 * @param init true if the ByteBuffer should be initialized
	 */
	protected ByteBufferRasterData(DataView view, int rasterWidth, int rasterHeight, RasterReader reader,
		...
		this.dataAccess = new BufferAccess(reader, rasterWidth, rasterHeight, view, originalDataInfo, getPixelStride(),
				getLineStride(), getBandStride());
		if (init) {
			initByteBuffer();
		}
	}
	 ...
	 * @param rasterWidth width of the raster data
	 * @param rasterHeight height of the raster data
	 * @param reader to be used to read the data.
	 * @param originalDataInfo containing information about this raster data object
	 */
	protected ByteBufferRasterData(DataView view, int rasterWidth, int rasterHeight, RasterReader reader,
	...
	/**
	 * Use the given bytebuffer as a data source. This method should be handled with much
	 * care.
	 * @param buf to set.
	 * @param dataRect defining the width, height and position of the data.
	 */
	...
	public ByteBufferRasterData createCompatibleRasterData(int width, int height) {
		// use the offset from the view and the new width and height (shouldn't they be
		// checked for size?)
	...
	/**
	 * @return the domain of validity of the byte buffer, which need not be the view on
	 * the data.
	 */
	 ...
	 * @throws IndexOutOfBoundsException if the given rect is outside the raster
	 */
	 ...
	 * @param width The width of the rect.
	 * @param height The height of the rect.
	 * @return <code>true</code> if the given rect is inside the raster, else
	 * <code>false</code>
	 */
	...
	public double[] getDoubles(int x, int y, int width, int height, int band, double[] result) {
		...
		for (int h = 0; h < height; h++) {
			for (int w = 0; w < width; w++) {
				...
				int rOffset = (2 * h) + w;
				if (pos == -1) {// the position is outside the databuffer.
					System.arraycopy(getView().dataInfo.noDataPixel,
							getView().getBandOffset(band) * getView().dataInfo.dataSize, result, rOffset,
	...
	public ByteBufferRasterData getSubset(RasterRect sampleDomain) {
		...
		ByteBufferRasterData result = createCompatibleRasterData(new DataView(getView().x + sampleDomain.x,
				getView().y + sampleDomain.y, sampleDomain.width, sampleDomain.height, dataInfo));
		if (dataAccess.hasDataBuffer() && dataAccess.getReader() == null) {
			// the data was loaded, but no reader was available, we need a copy of the
			// data.
	...
	public ByteBufferRasterData getSubset(RasterRect sampleDomain, BandType[] bands) {
		...
		// ByteBufferRasterData result = createCompatibleRasterData( new DataView(
		// getView().x + sampleDomain.x,
		// getView().y + sampleDomain.y,
		// sampleDomain.width,
		// sampleDomain.height,
		// createRasterDataInfo( bands ), dataInfo ) );

		ByteBufferRasterData result = createCompatibleRasterData(
				new DataView(newOrigx, newOrigy, newWidth, newHeight, createRasterDataInfo(bands), dataInfo));
		result.info = info;
		// result.data = this.data.asReadOnlyBuffer();
		// result.dataAccess.setByteBuffer( getByteBuffer().asReadOnlyBuffer(), view );
		if (dataAccess.hasDataBuffer() && dataAccess.getReader() == null) {
		...
		/** old comments */
		// RASTERRECT viewRect = new RasterRect( ( getView().x + rasterRect.x ), (
		// getView().y + rasterRect.y ),
		// rasterRect.width,
		// rasterRect.height );
		// ByteBufferRasterData result = (ByteBufferRasterData)
		// createCompatibleWritableRasterData( viewRect, bands );
		// // result.data = this.data.asReadOnlyBuffer();
		// result.setSubset( 0, 0, rasterRect.width, rasterRect.height, 0, this, bands,
		// getView().x + rasterRect.x,
		// getView().y + rasterRect.y );
		return result;
	}
	 ...
	 * @param dstSize e.g. getWidth() or getHeight() of the destination raster.
	 * @param dstOrdinate e.g srcX, srcY, the location to put the data in destination
	 * raster.
	 * @param srcSize e.g. getWidth() or getHeight() of the source raster. *
	 * @param srcOrdinate e.g dstX, dstY, the location to read the data from the source
	 * raster.
	 * @param copySize e.g. width/height of rectangle to put data in this raster.
