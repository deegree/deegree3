...
/**
 * The <code>RangeSetBuilder</code> class supplies methods for building rangeset
...
public class RangeSetBuilder {
	 ...
	 * @param name of the rangeset
	 * @param label of the rangeset
	 * @param raster
	 * @return a Rangeset describing the bands of the given raster or <code>null</code> if
	 * the raster is null or it has no rasterdata info..
	 */
				...
				switch (rasterDataInfo.dataType) {
					case BYTE:
						interval = new Interval<Short, String>(new SingleValue<Short>(ValueType.Byte, (short) 0),
								new SingleValue<Short>(ValueType.Byte, (short) 255), Interval.Closure.open, semantic,
								false, null);
						break;
					case SHORT:
					case USHORT:
						interval = new Interval<Short, String>(new SingleValue<Short>(ValueType.Short, Short.MIN_VALUE),
								new SingleValue<Short>(ValueType.Short, Short.MAX_VALUE), Interval.Closure.open,
								semantic, false, null);
						break;
					case INT:
						interval = new Interval<Integer, String>(
								new SingleValue<Integer>(ValueType.Integer, Integer.MIN_VALUE),
								new SingleValue<Integer>(ValueType.Integer, Integer.MAX_VALUE), Interval.Closure.open,
								semantic, false, null);
						break;
					case FLOAT:
						interval = new Interval<Float, String>(new SingleValue<Float>(ValueType.Float, Float.MIN_VALUE),
								new SingleValue<Float>(ValueType.Float, Float.MAX_VALUE), Interval.Closure.open,
								semantic, false, null);
						break;
					case DOUBLE:
						interval = new Interval<Double, String>(
								new SingleValue<Double>(ValueType.Double, Double.MIN_VALUE),
								new SingleValue<Double>(ValueType.Double, Double.MAX_VALUE), Interval.Closure.open,
								semantic, false, null);
						break;
					case UNDEFINED:
						interval = new Interval<String, String>(new SingleValue<String>(ValueType.String, "min"),
								new SingleValue<String>(ValueType.String, "max"), Interval.Closure.open, semantic,
								false, null);
						break;
				}
