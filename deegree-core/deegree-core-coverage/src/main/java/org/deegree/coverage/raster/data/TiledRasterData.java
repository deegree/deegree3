...
/**
 * The <code>TiledRasterData</code> is a grid of raster data, wrapping all pixel
...
public class TiledRasterData implements RasterData {
	...
	/**
	 * @param reader to be used for the tiles.
	 * @param options
	 *
	 */
	protected TiledRasterData(GridReader reader, RasterIOOptions options) {
		...
		for (int row = 0; row < rows; ++row) {
			for (int col = 0; col < columns; ++col) {
				TileOffsetReader r = new TileOffsetReader(reader,
						new RasterRect(col * tileWidth, row * tileHeight, tileWidth, tileHeight));
				tiles[(row * columns) + col] = RasterDataFactory.createRasterData(
						new RasterRect(0, 0, tileWidth, tileHeight), reader.getRasterDataInfo(), r, false, options);
			}
		}
	...
	/**
	 * Returns the min column, row and max column row of the given rect. The rectangle
	 * will be cut off to fit the data. If the rect does not intersect the data,
	...
	/**
	 * @param rasterCoord normally the y.
	...
	/**
	 * @param rasterCoord in raster coordinates normally x.
					...
					if (intersectionWithTile != null) {
						...
						if (tile != null) {
							...
							final RasterRect tReq = new RasterRect((intersectionWithTile.x - widthOffset),
									(intersectionWithTile.y - heightOffset), intersectionWithTile.width,
							...
							for (int yResult = 0; yResult < intersectionWithTile.height; ++yResult) {
								...
								System.arraycopy(tileResult, yResult * tReq.width, resultValues,
										(yResult * requestedRect.width) + offsetInResult, intersectionWithTile.width);
							}
						}
					}
				}
				...
				for (int col = interSectingTiles[0]; col < interSectingTiles[2]; ++col) {
					...
					if (intersectionWithTile != null) {
						...
						if (tile != null) {
							// make sure it exists.
							// get the result
							final byte[] tileValues = new byte[intersectionWithTile.width
									* intersectionWithTile.height];
							...
							for (int yResult = 0; yResult < intersectionWithTile.height; ++yResult) {
								// one band, so row copying is available
								System.arraycopy(values, (yResult * requestedRect.width) + offsetInResult, tileValues,
										yResult * intersectionWithTile.width, intersectionWithTile.width);
							}
	...
	private static class MappedTile {
		...
		/**
		 * @param rasterData
		 * @param mappedX x coordinate in the given tile
		 * @param mappedY y coordinate in the given tile
		 */
