...
public abstract class AbstractRaster extends AbstractCoverage {
	 ...
	 * @param envelope The envelope of the raster.
	 */
	...
	/**
	 * Extends current RasterReference with rasterReference. Useful for extending the
	 * raster, e.g. adding a tile.
	 * @param rasterReference The raster envelope to add to the current.
	 */
	 ...
	 * @param envelope The envelope to check for.
	 */
	protected void checkBounds(Envelope envelope) {
		// rb: following checks are unnecessary because the creation of envelopes should
		// check it.
	 ...
	 * @param subsetEnv envelope of the sub raster, may not be <code>null</code>
	 ...
	 * Returns a subset of the raster, note this is a view on the given raster.
	 * @param subsetEnv envelope of the sub raster, may not be <code>null</code>
	 * @param bands to use for the given sub raster, if <code>null</code> the bands of the
	...
	/**
	 * Returns a subset of the raster, note this is a view on the given raster.
	 * @param subsetEnv envelope of the sub raster
	 * @param bands to use for the given sub raster, if <code>null</code> the bands of the
	 * instance shall be used.
	 * @param targetOrigin the origin location of the target sub raster, if
	 * <code>null</code> the origin location of the instance shall be used.
	 * @return subset of the raster
	 */
	...
	public AbstractRaster getSubRaster(double x, double y, double x2, double y2) {
		// what about the precision model? Formerly: getRasterReference().getDelta() was
		// used
	...
	/**
	 * Sets the raster with data from source. Source must overlap the raster (within the
	 * envelope).
	 * @param source data to copy
	 * @param env Envelope with the destination area
	 */
	 ...
	 * @param x left boundary
	 * @param y upper boundary
	 * @param dstBand selected destination band
	 * @param source data to copy
	 */
	...
	/**
	 * Returns the AbstractRaster as a SimpleRaster. The data gets cropped (TiledRaster)
	 * or merged (MultiRange) if necessary.
	...
	/**
	 * Returns available information on the raster data. This method should be called with
	 * care, it may under circumstances, cause massive loading of raster data.
