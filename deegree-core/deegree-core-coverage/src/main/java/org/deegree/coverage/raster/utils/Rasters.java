...
public class Rasters {
	 ...
	 * @param rasterSide either the width or the height of the entire raster.
	 * @param numberOfTiles of the entire raster.
	...
	/**
	 * Calculates the number of tiles fitting the given preferred size and the largest of
	 * the given raster sides. Determination is based on 1.5 times the given tile size, so
	 * if the largest entire raster side (for example the width) is 1600 and the preferred
	 * tile size is 1000 this method will return 2, if however the largest side would have
	 * been 1400, this method would return 1.
	 * @param rasterWidth of the entire raster
	 * @param rasterHeight of the entire raster
	 * @param preferredTileSize in pixels of the largest side.
	 * @return the number of tiles which will fit the preferred size. Determination is
	 * based on the largest given side.
	 */
	public final static int calcApproxTiles(final int rasterWidth, final int rasterHeight,
			final int preferredTileSize) {
	 ...
	 * @param srcRect the rectangle specifying the layout of the data in the source
	 * buffer.
	 * @param destRect the rectangle specifying the layout of the data in the result
	 * buffer.
	 * @param srcBuffer containing the data fitting the srcRect
	 * @param destBuffer which will hold the result
	 * @param sampleSize size in bytes of a sample (depends on the band size and the
	 * datatype)
	 * @throws IOException
	 */
	public static void copyValuesFromTile(RasterRect srcRect, RasterRect destRect, ByteBuffer srcBuffer,
			ByteBuffer destBuffer, int sampleSize) throws IOException {
