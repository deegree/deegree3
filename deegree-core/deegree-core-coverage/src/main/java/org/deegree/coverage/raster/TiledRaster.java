...
public class TiledRaster extends AbstractRaster {
	 ...
	 * @param tileContainer wraps all tiles
	 */
	 ...
	 * @param env of the subraster
	 * @param bands defining the bands of the sub raster
	 * @param targetLocation the new origin's location definition.
	 * @return a subraster of the size of the given envelope, having the given bands and
	...
	public TiledRaster getSubRaster(Envelope env, BandType[] bands, OriginLocation targetLocation) {
		...
		if (tiles == null || tiles.isEmpty()) {
			// a tiledraster with no simple rasters should return the a simple raster with
			// no data values. To do this we
			// need the raster data info. If it is present supply the result with it. If
			// not we can only throw an
			// exception.
		...
		// throw new IndexOutOfBoundsException( "no intersection between TiledRaster and
		// requested subset" );
		// }
		return result;
	}
