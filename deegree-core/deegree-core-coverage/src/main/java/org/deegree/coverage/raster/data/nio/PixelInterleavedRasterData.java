...
public class PixelInterleavedRasterData extends ByteBufferRasterData {
	...
	/**
	 * Creates a new PixelInterleavedRasterData with given size, number of bands and data
	 * type, backed with no data.
	 * @param sampleDomain the raster rectangle defining the sample domain of this raster
	 * data.
	 * @param rasterWidth width of the underlying raster data.
	 * @param rasterHeight height of the underlying raster data.
	 * @param dataInfo containing information about the underlying raster.
	 */
	 ...
	 * @param rasterWidth width of the underlying raster data.
	 * @param rasterHeight height of the underlying raster data.
	 * @param reader to be used for reading the data, may be <code>null<code>
	 * @param dataInfo containing information about the underlying raster.
	 */
	...
	/**
	 * @param view the raster rectangle defining the sample domain of this raster data as
	 * well as the data info of the view.
	 * @param rasterWidth width of the underlying raster data.
	 * @param rasterHeight height of the underlying raster data.
	 * @param reader to be used for reading the data, may be <code>null<code>
	 * @param dataInfo containing information about the underlying raster.
	 * @param init true if a new buffer should be initialized
	 */
	...
	public void setSubset(int dstX, int dstY, int width, int height, RasterData srcRaster, int srcX, int srcY) {
		...
		if (srcRaster instanceof PixelInterleavedRasterData && getView().dataInfo.bands == dataInfo.bands) {
			// calculate if the getWidth || getWidth methods would exceed the actual
			// rasterWidth
			...
			// if the number of pixels from the source raster x position to requested
			// width > raster width, snap the new
			...
			for (int i = 0; i < subHeight; i++) {
				...
				// order of .position and .get calls is significant, if bytebuffer is
				// identical
