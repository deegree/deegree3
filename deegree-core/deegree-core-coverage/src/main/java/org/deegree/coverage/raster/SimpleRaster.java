...
public class SimpleRaster extends AbstractRaster {
	 ...
	 * @param envelope The envelope of the new raster.
	 * @param rasterReference The raster envelope of the new raster.
	 */
	 ...
	 * @param raster content for the SimpleRaster
	 * @param envelope The envelope of the new raster.
	 * @param rasterReference The raster envelope of the new raster.
	 */
	...
	// */
	// public SimpleRaster( RasterDataContainer rasterDataContainer, Envelope envelope,
	// RasterGeoReference
	// rasterReference ) {
	...
	/**
	 * Creates a new empty writable SimpleRaster with same size, DataType and
	 * InterleaveType.
	 * @return new empty SimpleRaster
	 */
	...
	/**
	 * Creates a new empty SimpleRaster with same DataType and InterleaveType. Size is
	 * determined by the given envelope.
	...
	public SimpleRaster getSubRaster(Envelope envelope, BandType[] bands, OriginLocation targetLocation) {
		// rb: testing for envelope equality can lead to a memory leak, because the memory
		// can not be freed.
		...
		// rb: don't need to get a readonly raster data, because it will be filled with
		// data later.
	 ...
	 * @param band Number of the selected band.
	 * @return A copy of the selected band.
	 */
	public SimpleRaster getBand(int band) {
		return new SimpleRaster(
				getRasterData().getSubset(new RasterRect(0, 0, getColumns(), getRows()), new BandType[] {}),
				getEnvelope(), getRasterReference(), metadata);
	}
	...
	public void setSubRaster(Envelope env, AbstractRaster source) {
		...
		// source.getSubset( env ) already returns a copy, no need for
