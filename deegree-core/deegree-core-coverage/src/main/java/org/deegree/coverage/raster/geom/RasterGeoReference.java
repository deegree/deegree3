...
/**
 * The <code>RasterGeoReference</code> defines methods for transformations between a
 * raster crs and a world crs. For this purpose the origin of the upper left raster grid
 * and the size in world coordinate units of a raster grid (pixel) must be specified.
 *
 * Alternatively an angle between the x axis and a rectangular base axis (as well as y
 * rotation between the y axis and the rectangular basis) may be defined.
 *
 * This implementation is world crs axis order aware. See
 * http://wiki.deegree.org/deegreeWiki/deegree3/RasterInnerOuterIssue for a discussion on
...
public class RasterGeoReference {
	...
	/**
	 * The <code>OriginLocation</code> defines the mapping location of the world origin to
	...
	/**
	 * Constructs a raster reference with given resolutions, origin and rotations. The
	 * origin maps to the pixel location given by location.
	...
	public RasterGeoReference(OriginLocation location, double resolutionX, double resolutionY, double rotationX,
		...
		// define a delta to which calculations will be correct. It is dependent on the
		// highest resolution. (smallest
		// value).
	...
	/**
	 * Constructs a raster reference with given resolutions and origin. The origin maps to
	...
	/**
	 * Constructs a raster reference with given resolutions and origin. The origin maps to
	 * the pixel location given by location. No world CRS is defined.
	 * @param location of the origin on the upper left pixel.
	 * @param resolutionX the resolution of one pixel on the x axis in the raster
	 * @param resolutionY the resolution of one pixel on the y axis in the raster
	 * @param origin0 ordinate of the origin of the first axis defined in the world crs
	...
	/**
	 * Create a raster reference which has it's origin at the min[0] and max[1] of the
	 * given Envelope. The resolution will be determined by getting the easting axis/width
	 * and northing-axis/height. If no CRS is available from the Envelope axisorder XY is
	 * assumed.
	 * @param location of the origin can be center or outer
	 * @param envelope to get the appropriate values from.
	 * @param width of the underlying raster needed to calculate the resolution.
	 * @param height of the underlying raster needed to calculate the resolution.
	 * @return a new RasterGeoReference of <code>null</code> if the given envelope is
	 * <code>null</code>
	 */
	...
	/**
	 * Create a raster reference which has it's origin at the min[0] and max[1] of the
	 * given Envelope. If no CRS is available from the Envelope axisorder XY is assumed.
	 * @param location of the origin can be center or outer
	 * @param envelope to get the appropriate values from.
	 * @param resolution0 of the first axis.
	 * @param resolution1 of the second axis.
	 * @return a new RasterGeoReference of <code>null</code> if the given envelope is
	 * <code>null</code>
	 */
	...
	/**
	 * Return the raster coordinate denoted by the given world coordinate. This method is
	 * CENTER and OUTER aware.
	 * @param worldX x position in the world coordinate system, for which raster
	 * coordinates should be calculated.
	 * @param worldY y position in the world coordinate system, for which raster
	 * coordinates should be calculated.
	...
	/**
	 * Return the world coordinate denoted by the given raster position. This method is
	 * CENTER and OUTER aware.
	 * @param rasterX x position in the raster for which world coordinates should be
	 * calculated.
	 * @param rasterY y position in the raster for which world coordinates should be
	 * calculated.
	...
	public double[] getWorldCoordinate(double rasterX, double rasterY) {
		...
		if (location == CENTER) {
			// if the origin is located on the center of the pixel raster coordinates
			// should be substracted 0.5, or
	 ...
	 * in world coordinates is defined over min/max (lower left/upper right), whereas a
	 * RasterRect is defined as min(upperleft) with a width and height.
	...
	public RasterRect convertEnvelopeToRasterCRS(Envelope envelope) {
		...
		if (envelope != null) {
			...
			// finding easting and northing ordinates is not necessary because the
			// transform is aligned with the defined
			// coordinatesystem.
			...
			// floor the unrounded min, ceil the unrounded max, this equals the outer
			// representation, for example the
			// point 1.1, 3.1 must result in a span of 1-4. (Draw it out :-) ) e.g a width
			// of 3
	 ...
	 * @param width in raster coordinates
	 * @param height in raster coordinates
	 ...
	 * @param rasterRect defining the x,y raster coordinates (as integers) as well as the
	 ...
	 *
	 * The calculation considers the origin and resolution of the raster.
	 * @param targetLocation of the origin, specifies if the the newly created envelope
	 * should consider the origin located at the OUTER or CENTER of a pixel.
	 * @param rasterRect defining the x,y raster coordinates (as integers) as well as the
	 * width and height of the raster.
	 * @param crs the coordinate system for the envelope
	 * @return the calculated envelope
	...
	public Envelope getEnvelope(OriginLocation targetLocation, RasterRect rasterRect, ICRS crs) {
		// if the targetlocation must be center, we add half a pixel, because we need to
		// get the world coordinate of the
		// center of the pixel.
		...
		// if ( location == CENTER ) {
		// // if the targetlocation must be center, we add half a pixel, because we need
		...
		// // this location is center, the target location is outer, subtract 0.5 pixel
		// from the width|height.
		// // tw -= 0.5;
		// // th -= 0.5;
		// pixelAddUp -= 0.5;
		// } else {
		// // this location is outer, the target location is center, add 0.5 pixel to the
		// width|height
		// // tw += 0.5;
		// // th += 0.5;
		// pixelAddUp += 0.5;
		...
		// if ( location != targetLocation ) {
		// if ( targetLocation == OUTER ) {
		// // this location is center, the target location is outer, subtract 0.5 res to
		// the origin.
		// origin[0] -= resX * 0.5;
		// origin[1] -= resY * 0.5;
		// } else {
		// // this location is outer, the target location is center, add 0.5 resolution to
		// the origin
		// origin[0] += resX * 0.5;
		// origin[1] += resY * 0.5;
		// }
	...
	/**
	 * Merge two Raster references. Returns a new RasterReference where the upper-left
	 * corner is set to the values of the smallest upper and smallest left ordinate. The
	 * resolution is set to the minimum value (i.e. the highest resolution [unit/pixel]).
	 * Some assumptions are made (not checked):
	 * <ul>
	 * <li>The pixel location (center/outer) of the origin are equal, if not, the location
	 * of the first reference will be used (translated origin of second)</li>
	...
	public static RasterGeoReference merger(RasterGeoReference geoRef1, RasterGeoReference geoRef2) {
		...
		if (geoRef1.location != geoRef2.location) {
			...
			geoRef2Copy = new RasterGeoReference(geoRef1.location, geoRef2.getResolutionX(), geoRef2.getResolutionY(),
					geoRef2.getRotationX(), geoRef2.getRotationY(), orig[0], orig[1], geoRef2.crs);
		}
	...
	/**
	 * Returns new RasterGeoReference with the origin set to the min[0],max[1] of the
	 * envelope but other values are taken from this instance. Attention, the resulting
	 * origin is snapped to the location (center/outer) of the underlying grid, so the
	 * min[0] and max[1] values are only approximations to the new origin!
	 * @param envelope to get the origin from.
	 * @return new RasterGeoReference or <code>null</code> if the envelope is
	 * <code>null</code>
	 */
	...
	/**
	 * Returns new RasterGeoReference with the origin set to the min[0],max[1] of the
	 * envelope and the OriginLocation to the given one. Other values are taken from this
	 * instance. Attention, the resulting origin is snapped to the location (center/outer)
	 * of the underlying grid, so the min[0] and max[1] values are only approximations to
	 * the new origin!
	...
	public RasterGeoReference createRelocatedReference(OriginLocation targetLocation, Envelope envelope) {
		if (envelope != null) {
			...
			// take the 'upper' left raster position as the new origin, thus add a half
			// pixel.
			...
			// if ( tLoc != location ) {
			// if ( tLoc == OUTER ) {
			// // this location is center, the above subtracted 0.5 will get the raster
			// location of the centered
	 ...
	 * Other values are taken from this instance.
	 * @param targetLocation of the new reference
	 * @return new RasterGeoReference or this if the target location is <code>null</code>
	 * or equals this one.
	 */
	public RasterGeoReference createRelocatedReference(OriginLocation targetLocation) {
		...
		return new RasterGeoReference(targetLocation, this.getResolutionX(), this.getResolutionY(), this.getRotationX(),
				this.getRotationY(), newOrigin[0], newOrigin[1], this.crs);
	}

	/**
	 * Relocates the given minimum and maximum points of the given envelope to the target
	 * origin location definition. This method does nothing if the given location equals
	 * this {@link RasterGeoReference}'s origin location. This method effectively adds or
	 * subtracts half a resolution of the ordinates of the given Envelope. Different CRS's
	 * are supported.
	 * @param targetLocation the preferred location of the origin. *
	 * @param envelope to relocate.
	 * @return a new Envelope which is aligned with the target location or
	 * <code>null</code> if the envelope is <code>null</code>
	 */
	...
	public double getResolutionX() {
		...
		/*
		 * if the shear is null, the rotation matrix was not instantiated, just return the
		 * scale value.
		 */
	...
	/**
	 * A dirty hacking method for getting rid of any imprecision errors in the floating
	 * point representation of the given double value. As it is the
	 * {@link AffineTransform} creates lots of these imprecision errors, resulting in the
	 * Math.floor/round/ceil functions being of by one more often than you think!!!
	...
	/**
	 * First get rid of any rounding errors before passing the value to
