...
/**
 * The <code>AxisSubset</code> class represents the subset defined on one of the axis of
 * the coverage.
 *
...
public class AxisSubset {
	...
	/**
	 * @param name
	 * @param label may be <code>null</code>, in this case the name will be returned as
	 * label.
	 * @param intervals
	 * @param singleValues
	 */
	...
	/**
	 * @param other
	 * @param convert if true the intervals and singlevalues of this instance will be
	 * converted to the type of the given axis subset if their names match and if this
	 * type is unknown (void).
	 * @return true if this {@link AxisSubset} matches the given AxisSubset, e.g. if the
	 * names are equal and the axis values have matching parameters in the given one.
	 */
	...
	/**
	 * @param otherValues
	 * @param convert if true the given intervals will be converted to the type they match
	 * if they were of type void.
	 * @return true if the given singleValues match these of the given single values, the
	...
	private boolean checkSingles(List<SingleValue<?>> otherValues, List<Interval<?, ?>> otherIntervals) {
		...
		if (otherValues != null && !otherValues.isEmpty()) {
			for (SingleValue<?> sv : singleValues) {
				// rb: iterate over all values, if one of them mismatches this method will
				// return false.
				if (sv != null) {
					// if the value == null, the default value must be taken into account,
					// therefore no validity check
					// can be done.
					if (sv.value != null) {
			...
			for (SingleValue<?> sv : singleValues) {
				...
				if (sv != null) {
					...
					if (sv.value != null) {
						...
						while (iterator.hasNext() && !result) {
							// rb: type checking is done, but the compiler can not resolve
							// it.
								...
								if (min.type != sv.type) {
									if (min.type == ValueType.Void) {
										tmpInter = Interval.createFromStrings(sv.type.toString(),
												tmpInter.getMin().value.toString(), tmpInter.getMax().value.toString(),
												tmpInter.getClosure(), tmpInter.getSemantic(), tmpInter.isAtomic(),
												tmpInter.getSpacing());
									}
	...
	/**
	 * Convert the types of the intervals and single values of this axis subset to the
	 * given type, if and only if the type of this axis is {@link ValueType#Void}, if the
	 * conversion fails, the old types will not be changed.
	 * @param newType to convert the types of this axis to.
	 */
		...
		if (newType != null && newType != ValueType.Void && type == ValueType.Void) {
			...
			if (intervals != null && !intervals.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (origInter != null) {
						Interval<?, ?> converted = Interval.createFromStrings(type, origInter.getMin().value.toString(),
								origInter.getMax().value.toString(), origInter.getClosure(), origInter.getSemantic(),
								origInter.isAtomic(), origInter.getSpacing());
						if (converted != null) {
	 ...
	 * @return the determined type of this axis ranges, if the type could not be
	 * determined it will be {@link ValueType#Void}
	 */
