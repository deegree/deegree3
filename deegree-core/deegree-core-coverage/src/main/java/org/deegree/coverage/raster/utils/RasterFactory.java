...
/**
 * This class reads and writes raster files. The actual raster loading and writing is
 * handled by {@link RasterReader} and {@link RasterWriter} implementations.
 *
...
public class RasterFactory {
	 ...
	 * @param in
	 * @param options map with options for the raster writer
	 * @return the loaded raster as an AbstractRaster
	 * @throws IOException
	 */
	...
	/**
	 * Find all RasterIOLoaders and retrieve all the (image) formats they support on
	 * writing.
	...
	/**
	 * Creates a simple raster from a given {@link BufferedImage} and sets the geo
	 * reference to the given envelope.
	 * @param image to get as a raster
	 * @param envelope of the raster
	 * @param originLocation the mapped location of the world coordinate origin on the
	 * upper left raster coordinate.
	 ...
	 * @param originLocation the mapped location of the world coordinate origin on the
	 * upper left raster coordinate.
	 * @param options with information about the image and resulting raster.
	 * @return a geo referenced AbstractRaster.
	 */
	public static AbstractRaster createRasterFromImage(RenderedImage image, Envelope envelope,
			RasterGeoReference.OriginLocation originLocation, RasterIOOptions options) {
	...
	/**
	 * Creates a buffered image from a given {@link AbstractRaster}, note creating an
	 * image might result in a incorrect view of the raster.
	 * @param raster to create the image from
	 ...
	 * @param sourceRaster the source RasterData
	...
	public static BufferedImage rasterDataToImage(RasterData sourceRaster) {
		...
		switch (type) {
			case BYTE:
				...
				if (bands == 1) {
					...
					throw new UnsupportedOperationException(
							"Could not save raster with bands: " + Arrays.toString(bandTypes));
				}
			...
			case FLOAT:
				...
				// System.arraycopy( floatValues, 0, dataBuffer, ( ( y * width ) + x ) *
				// bands, bands );
				//
				// }
				// }
				// set data does not work.. it will round to int :-)
				SampleModel fm = new BandedSampleModel(DataBuffer.TYPE_FLOAT, raster.getColumns(), raster.getRows(),
						bands);
	...
	/**
	 * Creates Raster data from the given RenderedImage image. The options can be used to
	 * modify the outcome of the raster data image.
	 * @param img
	 * @param options which can hold information about the image read, may be
	 * <code>null</code>
	 * @return the rasterdata object from the image or <code>null</code> if the given img
	 * is <code>null</code>
	 */
	...
	/**
	 * Creates Raster data from the given buffered image. The options can be used to
	 * modify the outcome of the raster data image.
	 * @param img
	 * @param options which can hold information about the image read, may be
	 * <code>null</code>
	 * @param byteBuffer to fill
	 * @return the rasterdata object from the image or <code>null</code> if the given img
	 * is <code>null</code>
	 */
	public static ByteBufferRasterData rasterDataFromImage(RenderedImage img, RasterIOOptions options,
		...
		if (img != null) {
			...
			if (img instanceof BufferedImage) {
				// buffered images pack the pixels into the given datatype, so if datatype
				// is int, the
				// org.deegree.coverage.raster.data datatype is still Byte (4 bytes in an
				// int value).
			...
			// CacheRasterReader reader = new CacheRasterReader( byteBuffer, width,
			// height, null, rdi, geoRef );
			byteBuffer = null;
			// result = new PixelInterleavedRasterData( new RasterRect( 0, 0, width,
			// height ), width, height, reader,
			// rdi );
	...
	/**
	 * Create a byte buffer from the given raster, the Bytebuffer will be filled row
	 * order, that is, pixel(0,0);(0,1)
	 * @param imageRaster
	 * @param x
	 * @param y
	 * @param width
	 ...
	 */
	public static ByteBuffer rasterToByteBuffer(Raster imageRaster, int x, int y, int width, int height, DataType type,
			ByteBuffer byteBuffer) {
		if (byteBuffer == null) {
		...
		switch (type) {
			case BYTE:
				// rb: use the databuffer directly, to make sure no pixel 'inversion' is
				// done.
				// byteBuffer.put( ( (DataBufferByte) imageRaster.getDataBuffer()
				// ).getBankData()[0] );
	...
	private static void copyIntValues(int[] pixels, ColorModel cm, ByteBuffer byteBuffer, RasterDataInfo rdi) {
		...
		for (int i = 0; i < pixels.length; ++i) {
			...
			byteBuffer.put(res);
			// System.out.println( "pixel(" + pixels.length + "): " + i + "| color: " + (
			// res[0] & 0xff ) + ","
			// + ( res[1] & 0xff ) + "," + ( res[2] & 0xff ) );
			// System.out.println( "pos(" + byteBuffer.position() + "): " + i + "| color:
			// " + ( res[0] & 0xff ) + ","
			// + ( res[1] & 0xff ) + "," + ( res[2] & 0xff ) );
		}
	}
	...
	/**
	 * @param img to get the bands for.
	 */
	 ...
	 * {@link RasterFactory#rasterDataFromImage(RenderedImage, RasterIOOptions, ByteBuffer)}
	 * method without any options. *
	 * @param img
	 * @return the ByteBufferRasterData object from the image or <code>null</code> if the
	 * given img is <code>null</code>
	 */
	...
	/**
	 * Creates a new Raster data object from the given world envelope, a raster reference
	 * and the data info object (holding information about type, size etc...). If any of
	 * the parameters are <code>null</code> null will be returned.
	 * @param rdi
	 * @param worldEnvelope describing the raster data.
	 * @param rasterGeoReference the raster geo reference defining the resolution of the
	 * raster.
	 ...
	 * @param rasterGeoReference the raster geo reference defining the resolution of the
	 * raster.
	 * @param reader to lazily instantiate the data from, may be <code>null</code>, if the
	 * raster should not be backed by any data.
	 * @param addToCache true if the raster data of the given raster should be added to
	 * the cache.
	 * @param options containing information about the raster caching.
	 * @return a raster data object according to the given parameters.
	 */
	public static SimpleRaster createEmptyRaster(RasterDataInfo rdi, Envelope worldEnvelope,
			RasterGeoReference rasterGeoReference, RasterReader reader, boolean addToCache, RasterIOOptions options) {
		...
		if (rdi != null && rasterGeoReference != null && worldEnvelope != null) {
			...
			// case BAND:
			// data = new BandInterleavedRasterData( rasterRect, rasterRect.width,
			// rasterRect.height, reader, rdi );
			// break;
			// case LINE:
			// data = new LineInterleavedRasterData( rasterRect, rasterRect.width,
			// rasterRect.height, reader, rdi );
			// break;
			// case PIXEL:
			// data = new PixelInterleavedRasterData( rasterRect, rasterRect.width,
			// rasterRect.height, reader, rdi );
			// break;
			// }
