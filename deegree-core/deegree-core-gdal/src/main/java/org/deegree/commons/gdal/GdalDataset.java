...
public class GdalDataset implements KeyedResource {
	...
	private DefaultEnvelope switchYValues(Envelope envelope) {
		...
		Point min = new DefaultPoint(null, crs, null,
				new double[] { envelope.getMin().get0(), envelope.getMax().get1() });
		Point max = new DefaultPoint(null, crs, null,
				new double[] { envelope.getMax().get0(), envelope.getMin().get1() });
		return new DefaultEnvelope(min, max);
	}
