...
public class FeatureInfoGmlWriter implements FeatureInfoSerializer {
	 ...
	 */
	/*
	 * public FeatureInfoGmlWriter( GMLVersion version ) { gmlNs = version.getNamespace();
	 * if ( !version.equals( GML_2 ) ) { fidAttr = new QName( gmlNs, "id" ); gmlNull =
	 * "Null"; } else { fidAttr = new QName( "", "fid" ); gmlNull = "null"; } }
	 */

	/**
	 * Writes the given feature collection as a <code>wfs:FeatureCollection</code>
	 * element.
	 * @param fc feature collection, must not be <code>null</code>
	 * @param gmlWriter gml writer to use for exporting features, must not be
	 * <code>null</code>
	...
	public void serialize(FeatureInfoParams params, FeatureInfoContext context) throws IOException, XMLStreamException {
		...
		try {
			// for more than just quick 'hacky' schemaLocation attributes one should use a
			// proper WFS
			...
			export(gmlVersion, params.getFeatureCollection(), gmlWriter, ns == null ? params.getSchemaLocation() : null,
					bindings);
		}
		catch (Throwable e) {
