...
/**
 * Defines a matrix based on {@link GMatrix}. Also suplies some methods to create a 'swap'
...
public class Matrix extends GMatrix {
	 ...
	 * <code>numRow</code>&nbsp;&times;&nbsp;<code>numCol</code>. Elements on the diagonal
	 ...
	 * initialized to the values in the <code>matrix</code> array. The array values are
	 * copied in one row at a time in row major fashion. The array should be exactly
	 * <code>numRow*numCol</code> in length. Note that because row and column numbering
	 * begins with zero, <code>row</code> and <code>numCol</code> will be one larger than
	 ...
	 * @throws IllegalArgumentException if the specified matrix is not regular (i.e. if
	...
	/**
	 * Construct an affine transform changing axis order. The resulting affine transform
	 * will convert incoming coordinates into the given destination Axis. For example if
	 * source axis are given with (NORTH,WEST) and destination axis as (EAST,NORTH)
	 * assuming the axis use the same units, the resulted matrix will look like:<br/>
	 * <code>
	 *  &nbsp;0,&nbsp;1,&nbsp;0<br/>
	 * -1,&nbsp;0,&nbsp;0<br/>
	 *  &nbsp;0,&nbsp;0,&nbsp;1<br/>
	 *  </code> Axis orientation can be inverted only. Rotating axis (e.g. from
	 * NORTH,WEST, to NORTH,DOWN, ) is not supported.
	 ...
	 * @throws IllegalArgumentException if the affine transform can't be created for some
	 * other reason.
	 */
	public Matrix(final IAxis[] srcAxis, final IAxis[] dstAxis) {
		...
		/*
		 * Map source axis to destination axis. If no axis is moved (for example if the
		 * user want to transform (NORTH,EAST) to (SOUTH,EAST)), then source and
		 * destination index will be equal. If some axis are moved (for example if the
		 * user want to transform (NORTH,EAST) to (EAST,NORTH)), then ordinates at index
		...
		for (int srcIndex = 0; srcIndex < dimension; srcIndex++) {
			...
			for (int dstIndex = 0; dstIndex < dimension; dstIndex++) {
				...
				if (sourceAxisDirection == Math.abs(dstAxeDirection)) {
					if (hasFound) {
						throw new IllegalArgumentException("Following axis are colinear: " + srcAxis[srcIndex].getName()
								+ " dstAxe: " + dstAxis[dstIndex].getName());
					}
					hasFound = true;
					// row, column, value
			...
			if (!hasFound) {
				throw new IllegalArgumentException(
						"No appropriate transformation axis found for srcAxis: " + srcAxis[srcIndex].getName());
			}
		}
	...
	/**
	 * Returns <code>true</code> if this matrix is an affine transform. A transform is
	 * affine if the matrix is square and last row contains only zeros, except in the last
	 * column which contains 1.
	...
	/**
	 * Copies the first 2x3 values into an affine transform object. If not enough values
	 * are available, an identity transform is returned.
	 * @return an affine transform for this matrix. or an identity if this matrix has not
	 * sufficient values.
	 *
	 */
	public final Matrix3d toAffineTransform() {
		...
		return new Matrix3d(getElement(0, 0), getElement(0, 1), getElement(0, 2), getElement(1, 0), getElement(1, 1),
				getElement(1, 2), 0, 0, 1);
	}
	...
	/**
	 * @return an affine transform between two coordinate systems. Only units and axis
	 * order (e.g. transforming from (NORTH,WEST) to (EAST,NORTH)) are taken in account.
	 * Other attributes (especially the datum) must be checked before invoking this
	 * method.
	 * @param sourceCRS The source coordinate system.
	 * @param targetCRS The target coordinate system.
	 * @throws TransformationException if some error occurs.
	 */
	...
	/**
	 * @return an affine transform between two geographic coordinate systems. Only units,
	 * axis order (e.g. transforming from (NORTH,WEST) to (EAST,NORTH)) and prime meridian
	 * are taken in account. Other attributes (especially the datum) must be checked
	...
	public static Matrix swapAndRotateGeoAxis(final IGeographicCRS sourceCRS, final IGeographicCRS targetCRS)
		...
		if (!sourceCRS.getGeodeticDatum().getPrimeMeridian().equals(targetCRS.getGeodeticDatum().getPrimeMeridian())) {
			...
			for (int i = 0; i < targetAxis.length; ++i) {
				// Find longitude, and apply a translation if prime meridians are
				// different.
				...
				if (Axis.AO_WEST == Math.abs(orientation)) {
					...
					final double targetLongitude = targetCRS.getGeodeticDatum()
						.getPrimeMeridian()
						.getLongitudeAsRadian();
					if (Math.abs(sourceLongitude - targetLongitude) > EPS11) {
						...
						// add the translation to the matrix translate element of this
						// axis
	...
	/**
	 * Creates a matrix, with which incoming values will be transformed to a standardized
	 * form. This means, to radians and meters.
	 * @param sourceCRS to create the matrix for.
	 * @param invert the values. Using the inverted scale, i.e. going from standard to crs
	 * specific.
	 * @return the standardized matrix.
	 * @throws TransformationException if the unit of one of the axis could not be
	 * transformed to one of the base units.
	 */
	public static Matrix toStdValues(ICRS sourceCRS, boolean invert) throws TransformationException {
				...
				if (!(Unit.RADIAN.equals(targetUnit) || Unit.METRE.equals(targetUnit))) {
					if (!(targetUnit.canConvert(Unit.RADIAN) || targetUnit.canConvert(Unit.METRE))) {
						throw new TransformationException(
								Messages.getMessage("CRS_TRANSFORMATION_NO_APLLICABLE_UNIT", targetUnit));
					}
					// lazy instantiation
					if (result == null) {
