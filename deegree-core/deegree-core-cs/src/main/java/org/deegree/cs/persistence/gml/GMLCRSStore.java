...
/**
 * The <code>GMLCRSProvider</code> is a provider for a GML 3.2 backend, this may be a
 * dictionary or a database.
 *
 * Note: not all of the GML3.2. features are implemented yet, but the basics
...
public class GMLCRSStore extends AbstractCRSStore {
	...
	/**
	 * @param rootElement containing a gml:CRS dom representation.
	 * @return a {@link CRS} instance initialized with values from the given XML-OM
	 * gml:CRS fragment or <code>null</code> if the given root element is
	 * <code>null</code>
	 * @throws CRSConfigurationException if something went wrong.
	 */
	protected ICRS parseCoordinateSystem(OMElement rootElement) throws CRSConfigurationException {
		if (rootElement == null) {
	 ...
	 */
	public Transformation parseTransformation(OMElement rootElement) throws CRSConfigurationException {
		try {
	...
	/**
	 * Parses some of the gml 3.2 transformation constructs. Currently only helmert
	 * transformations are supported.
	 * @param rootElement
	 * @param sourceCRS to be used as the source crs, if <code>null</code> the values from
	 * the given transformation will be parsed.
	 * @param targetCRS to be used as the target crs, if <code>null</code> the values from
	 * the given transformation will be parsed.
	...
	public Transformation parseGMLTransformation(OMElement rootElement, ICRS sourceCRS, ICRS targetCRS)
		...
		if (result == null) {
			...
			if (target == null) {
				...
				// rb: if the wgs 84 was referenced, use the default implementation, maybe
			...
			switch (transform) {
				...
				case GEOGRAPHIC_GEOCENTRIC:
					LOG.warn(
							"The mapping of gml:Transformation to Geographic/Geocentic transformations is not necessary.");
	 ...
	 * @param parameterValues the list of values, the Object must be a {@link Double}
	 * (denoting a the rotation/translation/ppm of the helmert.)
	 * @param source to go from
			...
			if (paramValue != null && (paramValue.second instanceof Pair<?, ?>)) {
				...
				if (second != null) {
					...
					if (!Double.isNaN(value)) {
						...
						if (paramID != null) {
							SupportedTransformationParameters paramType = SupportedTransformationParameters
								.fromCodes(paramID.getCodes());
							IUnit unit = second.first;
							// If a unit was given, convert the value to the internally
							// used
							// unit.
	 ...
	 * @param id of the transformation.
	 * @param parameterValues the list of values, the Object must be a String (denoting a
	 * gridshift file url.)
	 * @param source to go from
	 * @param target to go to
	 * @return an {@link NTv2Transformation} if a file was given, <code>null</code>
	 * otherwise.
	 */
	...
	/**
	 * @param rootElement which is a subtype of gml:IdentifiedObject and
	 * gml:DefinitionType or gml:AbstractCRSType
	 * @return the {@link CRSIdentifiable} instance, its values are filled with the values
	 * of the given gml instance.
	 * @throws XMLParsingException if the given rootElement could not be parsed.
	 */
	public CRSIdentifiable parseIdentifiedObject(OMElement rootElement) throws XMLParsingException {
		if (rootElement == null) {
			return null;
		}
		...
		// List<Element> domainsOfValidity = XMLTools.getElements( rootElement, PRE +
		// "domainOfValidity", nsContext );
		List<OMElement> domainsOfValidity = adapter.getElements(rootElement,
				new XPath(PRE + "domainOfValidity", nsContext));
		if (domainsOfValidity != null && domainsOfValidity.size() > 0) {
			// <domainOfValidity><gmd:geographicElement><gmd:EX_GeographicBoundingBox>
			OMElement elem = adapter.getElement(domainsOfValidity.get(0), new XPath(
					GMD_PRE + "EX_Extent/" + GMD_PRE + "geographicElement/" + GMD_PRE + "EX_GeographicBoundingBox",
					nsContext));
			if (elem != null) {
				double w = adapter.getNodeAsDouble(elem,
						new XPath(GMD_PRE + "westBoundLongitude/" + GCO_PRE + "Decimal", nsContext), -180);
				double e = adapter.getNodeAsDouble(elem,
						new XPath(GMD_PRE + "eastBoundLongitude/" + GCO_PRE + "Decimal", nsContext), 180);
				double s = adapter.getNodeAsDouble(elem,
						new XPath(GMD_PRE + "southBoundLatitude/" + GCO_PRE + "Decimal", nsContext), -90);
				double n = adapter.getNodeAsDouble(elem,
						new XPath(GMD_PRE + "northBoundLatitude/" + GCO_PRE + "Decimal", nsContext), 90);
		...
		// String[] scopes = XMLTools.getNodesAsStrings( rootElement, PRE + "scope",
		// nsContext );
	...
	/**
	 * This methods parses the given element and maps it onto a {@link CompoundCRS}.
	 * Currently only gml:CompoundCRS 's consisting of following combination is supported:
	 * <ul>
	 * <li>Projected CRS with VerticalCRS</li>
	 * </ul>
	 *
	 * Geographic crs with a height axis can be mapped in a {@link CompoundCRS} by calling
	 * the {@link #parseGeodeticCRS(OMElement)}
	 * @param rootElement containing a gml:CompoundCRS dom representation.
	 * @return a {@link CompoundCRS} instance initialized with values from the given
	 * XML-OM gml:CompoundCRS fragment.
	 * @throws XMLParsingException
	 * @throws IOException
	 */
	protected ICompoundCRS parseCompoundCRS(OMElement rootElement) throws XMLParsingException, IOException {
		if (rootElement == null) {
			LOG.debug("The given crs root element is null, returning nothing");
			return null;
		}
		...
		List<OMElement> compRefSysProp = adapter.getRequiredElements(rootElement,
				new XPath(PRE + "componentReferenceSystem", nsContext));
		...
		if ("ProjectedCRS".equals(crsElement1.getLocalName())) {
			...
			throw new XMLParsingException(adapter, crsElement1,
					"Currently only Compoundcrs's with the ProjectedCRS and VerticalCRS combination are supported, following elements were found:"
							+ crsElement1.getLocalName() + " and " + crsElement2.getLocalName() + ".");
		}
	...
	/**
	 * @param rootElement containing a gml:ProjectedCRS dom representation.
	 * @return a {@link ProjectedCRS} instance initialized with values from the given
	 * XML-OM gml:ProjectedCRS fragment or <code>null</code> if the given root element is
	 * <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException if a retrieval of an xlink of one of the subelements failed.
	 */
	protected ICRS parseProjectedCRS(OMElement rootElement) throws XMLParsingException, IOException {
		if (rootElement == null) {
			LOG.debug("The given crs root element is null, returning nothing");
			return null;
		}
		...
		OMElement baseGEOCRSElementProperty = adapter.getRequiredElement(rootElement,
				new XPath(PRE + "baseGeodeticCRS", nsContext));

		ICRS parsedBaseCRS = parseGeodeticCRS(
				getRequiredXlinkedElement(baseGEOCRSElementProperty, PRE + "GeodeticCRS"));
		...
		OMElement cartesianCSProperty = adapter.getRequiredElement(rootElement,
				new XPath(PRE + "cartesianCS", nsContext));
		...
		OMElement conversionElementProperty = adapter.getRequiredElement(rootElement,
				new XPath(PRE + "conversion", nsContext));

		IProjection projection = parseProjection(
				getRequiredXlinkedElement(conversionElementProperty, PRE + "Conversion"));
	...
	/**
	 * @param rootElement containing a gml:GeodeticCRS dom representation.
	 * @return a {@link CRS} instance initialized with values from the given XML-OM
	 * gml:GeodeticCRS fragment or <code>null</code> if the given root element is
	 * <code>null</code>. Note the result may be a {@link CompoundCRS}, a
	 * {@link GeographicCRS} or a {@link GeocentricCRS}, depending of the definition of
	 * the CS type.
	 * @throws XMLParsingException
	 * @throws IOException
	 */
	protected ICRS parseGeodeticCRS(OMElement rootElement) throws XMLParsingException, IOException {
		if (rootElement == null) {
			LOG.debug("The given crs root element is null, returning nothing");
			return null;
		}
		...
		OMElement datumElementProp = adapter.getRequiredElement(rootElement,
				new XPath(PRE + "geodeticDatum", nsContext));
		...
		if (axis != null) {
			if ("ellipsoidalCS".equals(csTypeProp.getLocalName())) {
				if (axis.length == 2) {
					...
					result = new CompoundCRS(axis[2], new GeographicCRS(datum, new IAxis[] { axis[0], axis[1] }, id), 0,
							id);
				}
			}
			else {
	...
	/**
	 * @param rootElement containing a gml:GeodeticDatum dom representation.
	 * @return a {@link GeodeticDatum} instance initialized with values from the given
	 * XML-OM fragment or <code>null</code> if the given root element is <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException if a retrieval of an xlink of one of the subelements failed.
	 */
	protected IGeodeticDatum parseDatum(OMElement rootElement) throws IOException, XMLParsingException {
		if (rootElement == null) {
		...
		if (result == null) {
			OMElement pmElementProp = adapter.getRequiredElement(rootElement,
					new XPath(PRE + "primeMeridian", nsContext));
			...
			OMElement ellipsoidElementProp = adapter.getRequiredElement(rootElement,
					new XPath(PRE + "ellipsoid", nsContext));
	...
	/**
	 * For the ellipsoidal and cartesian cs Types, this method also checks the consistency
	 * of axis (radian, radian, [metre] ) or (metre, metre, [metre] ). If the conditions
	 * are not met, an xml parsing exception will be thrown as well.
	 * @param rootElement containing a (Ellipsoidal, Spherical, Cartesian) CS type dom
	 * representation.
	 * @return a {@link Axis} array instance initialized with values from the given XML-OM
	 * fragment or <code>null</code> if the given root element is <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException if a retrieval of an xlink of one of the subelements failed.
	 */
	protected IAxis[] parseAxisFromCSType(OMElement rootElement) throws XMLParsingException, IOException {
		if (rootElement == null) {
		...
		for (int i = 0; i < axisProps.size(); i++) {
			...
			if (a == null) {
				throw new XMLParsingException(adapter, axisElement,
						"Axis: " + i + " of the CS Type is null, this may not be.");
			}
			axis[i] = a;
		}
		if ("cartesianCS".equalsIgnoreCase(rootElement.getLocalName())) {
			for (int i = 0; i < axis.length; ++i) {
				if (!axis[i].getUnits().canConvert(Unit.METRE)) {
					throw new XMLParsingException(adapter, rootElement,
							"The units of all axis of a (cartesian) cs must be convertable to metres. Axis " + i
									+ " is not: " + axis[i]);
				}
			}
		}
			...
			if (axis[0].getUnits() == null) {
				...
				if (!(axis[0].getUnits().canConvert(Unit.RADIAN) && axis[1].getUnits().canConvert(Unit.RADIAN))) {
					throw new XMLParsingException(adapter, rootElement,
							"The axis of the geodetic (Geographic) crs are not consistent: " + axis[0] + ", "
									+ axis[1]);
				}
				if (axis.length == 3) {
	...
	/**
	 * @param rootElement containing an gml:CoordinateSystemAxis type dom representation.
	 * @return an {@link Axis} instance initialized with values from the given XML-OM
	 * fragment or <code>null</code> if the given root element is <code>null</code> if the
	 * axis could not be mapped it's orientation will be {@link Axis#AO_OTHER}
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 */
	protected IAxis parseAxis(OMElement rootElement) throws XMLParsingException {
		if (rootElement == null) {
	...
	/**
	 * @param rootElement containing a gml:Ellipsoid dom representation.
	 * @return a {@link Ellipsoid} instance initialized with values from the given XML-OM
	 * fragment or <code>null</code> if the given root element is <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 *
	 */
	protected IEllipsoid parseEllipsoid(OMElement rootElement) throws XMLParsingException {
		if (rootElement == null) {
		...
		if (result == null) {
			...
			OMElement semiMajorAxisElem = adapter.getRequiredElement(rootElement,
					new XPath(PRE + "semiMajorAxis", nsContext));
			...
			OMElement otherParam = adapter.getRequiredElement(rootElement,
					new XPath(PRE + "secondDefiningParameter/" + PRE + "SecondDefiningParameter", nsContext));
			...
			if (type == 2) {
				...
				if (secondUnit != null) {
					if (!secondUnit.canConvert(unit)) {
						throw new XMLParsingException(adapter, param,
								"Ellispoid axis can only contain comparable unit, supplied are: " + unit + " and "
										+ secondUnit + " which are not convertable.");
					}
	...
	/**
	 * @param rootElement to create the pm from.
	 * @return {@link PrimeMeridian#GREENWICH} or the appropriate pm if a longitude is
	 * defined.
	 * @throws XMLParsingException
	 */
	protected IPrimeMeridian parsePrimeMeridian(OMElement rootElement) throws XMLParsingException {
		if (rootElement == null) {
		...
		// }
		if (result == null) {
			OMElement gwLongitudeElem = adapter.getRequiredElement(rootElement,
					new XPath(PRE + "greenwichLongitude", nsContext));
	...
	/**
	 * @param rootElement containing a gml:VerticalCRS dom representation.
	 * @return a {@link VerticalCRS} instance initialized with values from the given
	 * XML-OM fragment or <code>null</code> if the given root element is <code>null</code>
	 * @throws IOException
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 *
	 */
	protected IVerticalCRS parseVerticalCRS(OMElement rootElement) throws XMLParsingException, IOException {
		if (rootElement == null) {
		...
		OMElement verticalDatumProp = adapter.getRequiredElement(rootElement,
				new XPath(PRE + "verticalDatum", nsContext));
	...
	/**
	 * @param rootElement containing a gml:VerticalDatum dom representation.
	 * @return a {@link VerticalDatum} instance initialized with values from the given
	 * XML-OM fragment or <code>null</code> if the given root element is <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 *
	 */
	protected IVerticalDatum parseVerticalDatum(OMElement rootElement) throws XMLParsingException {
		if (rootElement == null) {
	...
	/**
	 * For now this method actually wraps all information in a
	 * gml:AbstractGeneralConversionType (or a derived subtype) into an CRSIdentifiable
	 * Object (used for the Projections).
	 * @param rootElement a gml:GeneralConversion element
	 * @return a Projection (Conversion) containing the mapped values from the given
	 * gml:Conversion XML-OM-representation.
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException
	 */
	protected IProjection parseProjection(OMElement rootElement) throws XMLParsingException, IOException {
		...
		if (result == null) {
			...
			double falseNorthing = 0, falseEasting = 0, scale = 1, firstParallelLatitude = 0,
					secondParallelLatitude = 0, trueScaleLatitude = 0;
				...
				if (paramValue != null && (paramValue.second instanceof Pair<?, ?>)) {
					...
					if (second != null) {
						...
						if (!Double.isNaN(value)) {
							CRSResource paramID = paramValue.first;
							if (paramID != null) {
								SupportedProjectionParameters paramType = SupportedProjectionParameters
									.fromCodes(paramID.getCodes());
								IUnit unit = second.first;
								// If a unit was given, convert the value to the
			...
			switch (projection) {
				case TRANSVERSE_MERCATOR:
					...
					result = new TransverseMercator(northernHemisphere, falseNorthing, falseEasting, naturalOrigin,
							units, scale, id);
					break;
	...
	/**
	 * @param rootElement which should contain a list of parameter Value properties.
	 * @return a list of Pairs containing the parsed OperationParamter and the value as a
	 * double, converted to the units defined in the value element, or the empty list if
	 * the rootElement is <code>null</code> or no parameterValues were found.
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException
	 */
	...
	/**
	 * @param rootElement containing a parameter Value property.
	 * @return a Pair containing the parsed OperationParamter and the value as a double or
	 * null if the rootElement is <code>null</code>
	 * @throws XMLParsingException if the dom tree is not consistent or a required element
	 * is missing.
	 * @throws IOException
	 */
	protected Pair<CRSIdentifiable, Object> parseParameterValue(OMElement rootElement)
		...
		OMElement operationParameterProp = adapter.getRequiredElement(paramValue,
				new XPath(PRE + "operationParameter", nsContext));
		...
		if (valueElem == null) {
			LOG.debug(
					"No gml:value found in the gml:Conversion/gml:parameterValue/gml:ParameterValue/ node, trying gml:integerValue instead.");
			valueElem = adapter.getElement(paramValue, new XPath(PRE + "integerValue", nsContext));
			if (valueElem == null) {
				LOG.debug(
						"No gml:integerValue found in the gml:Conversion/gml:parameterValue/gml:ParameterValue/ node, trying gml:fileValue instead.");
				valueElem = adapter.getElement(paramValue, new XPath(PRE + "valueFile", nsContext));
				if (valueElem == null) {
					LOG.debug(
							"Neither found a gml:integerValue in the gml:Conversion/gml:parameterValue/gml:ParameterValue/ node, ignoring this parameter value.");
				}
				else {
	...
	/**
	 * Returns the unit defined by the uomAttribute given of the given element. This
	 * method will use a 'colon' heuristic to determine if the given uom is actually an
	 * urn (and thus represents an xlink-type). This will then be resolved and mapped onto
	 * an unit.
	 * @param elementContainingUOMAttribute an element containing the 'uom' attribute
	 * which will be mapped onto a known unit.
	 * @return the mapped {@link Unit} or <code>null</code> if the given uomAttribute is
	 * empty or <code>null</code>, or no appropriate mapping could be found.
	 * @throws XMLParsingException
	 */
	protected IUnit parseUnitOfMeasure(OMElement elementContainingUOMAttribute) throws XMLParsingException {
	...
	/**
	 * convenience method to retrieve a given required element either by resolving a
	 * optional xlink or by evaluating the required element denoted by the xpath.
	 * @param propertyElement to resolve an xlink from.
	 * @param alternativeXPath denoting a path to the required node starting from the
	 * given propertyElement.
	 * @return the dom-element in the xlink:href attribute of the given propertyElement or
	 * the required alternativeXPath element.
	 * @throws XMLParsingException if the given propertyElement is <code>null</code> or
	 * the resulting xml dom-tree could not be parsed or the alternative xpath does not
	 * result in an Element.
	 * @throws IOException if the xlink could not be properly resolved
	 */
	...
	/**
	 * Retrieves the xlink:href of the given rootElement and use the XLinkResolver to
	 * resolve the xlink if it was given.
	 * @param rootElement to retrieve and resolve
	 * @return the resolved xlink:href attribute as an XML-OM element or <code>null</code>
	 * if the xlink could not be resolved (or was not given) or the rootElement is null.
	 * @throws IOException
	 */
	protected OMElement retrieveAndResolveXLink(OMElement rootElement) throws IOException {
		if (rootElement == null) {
	...
	/**
	 * Find an xlink:href attribute and return it's value, if not found, the empty String
	 * will be returned.
	 * @param rootElement to get the attribute from.
	 * @return the trimmed xlink:href attribute value or the empty String if not found or
	 * the rootElement is null;
	 */
