...
/**
 * The <code>PROJ4CRSProvider</code> class is capable of parsing the nad/epsg file and use
 * it as a backend for crs's. This class also adds following identifiers to the
 * coordinatesystems.
 * <ul>
...
public class PROJ4CRSStore extends AbstractCRSStore {
	...
	public PROJ4CRSStore(DSTransform prefTransformType) {
		...
		version = cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "-" + cal.get(Calendar.DAY_OF_MONTH)
	...
	/**
	 * Creates a projected crs from given params. Because proj4 doesn't define some
	 * axisorder xy is always assumed.
	 * @param projectionName to give the geographic crs (+e.g. proj=tmerc) or
	 * <code>null</code>
	 * @param params containing datum info.
	 * @return a geographic crs
	 * @throws CRSConfigurationException if an exception occurs while creating the datum.
	 */
	private ProjectedCRS createProjectedCRS(String projectionName, Map<String, String> params)
		...
		if ("3068".equals(id) || "31466".equals(id) || "31467".equals(id) || "31468".equals(id) || "31469".equals(id)) {
			geoID = EPSG_PRE + "4314";
		}
		else {
			geographicCRSCount++;
		...
		return new ProjectedCRS(projection, underlyingCRS,
				new Axis[] { new Axis(projection.getUnits(), "x", Axis.AO_EAST),
						new Axis(projection.getUnits(), "y", Axis.AO_NORTH) },
				codes, names, versions, descriptions, areasOfUse);
	}

	/**
	 * Creates a geographic crs with given identifier, if the identifier is empty or
	 * <code>null</code> the params.getIdentifier will be used. Because proj4 doesn't
	 * define some axisorder xy is always assumed.
	 * @param identifier to give the geographic crs (+proj=longlat) or <code>null</code>
	 * @param params containing datum info.
	 * @param projectedID of the projected crs (only if identifier is not null).
	 * @return a geographic crs
	 * @throws CRSConfigurationException if an exception occurs while creating the datum.
	 */
	private GeographicCRS createGeographicCRS(String identifier, String projectedID, Map<String, String> params)
		...
		if (tmpIdentifier == null || "".equals(tmpIdentifier.trim())) {
			...
			// if the id was not set, we create a geocrs, which means that no projectedID
			// has been
	...
	/**
	 * Create a datum by resolving the 'datum' param or creating it from the given
	 * ellipsoid/primemeridan parameters.
	 * @param params to create the datum from
	 * @return a datum
	 * @throws CRSConfigurationException if one of the datums necessities( ellipsoid,
	 * primemeridan) could not be created.
	 */
	private GeodeticDatum createDatum(Map<String, String> params, String identifier) throws CRSConfigurationException {
		...
		if (tmpValue != null && !"".equals(tmpValue.trim())) {
			...
			if (result == null) {
				throw new CRSConfigurationException(Messages.getMessage("CRS_CONFIG_PROJ4_UNKNOWN_DATUM",
						params.get(EPSG_PRE + "identifier"), tmpValue));
			}
		}
		else {
	 ...
	 * @param params to get the 'pm' parameter from.
	 * @return a mapped primemeridian or the Greenwich if no pm parameter was found.
	 * @throws CRSConfigurationException if the pm-parameter could not be mapped.
	 */
	private PrimeMeridian createPrimeMeridian(Map<String, String> params) throws CRSConfigurationException {
	...
	/**
	 * Tries to create an ellips from a predefined mapping, or from axis, eccentricities
	 * etc. if defined.
	 * @param params to create the ellipsoid from
	 * @return an (mapped) ellipsoid.
	 * @throws CRSConfigurationException if no mapping was found or the semimajor axis was
	 * not defined.
	 */
	private Ellipsoid createEllipsoid(Map<String, String> params) throws CRSConfigurationException {
		...
		if (tmpValue != null && !"".equals(tmpValue.trim())) {
			...
			/**
			 * These parameters (R_A, R_V, R_a, R_g, R_h, R_lat_a, R_lat_g) were not used
			 * in the epsg file, maybe they are of future value?. <code>
			...
			// if ( tmpValue != null && Boolean.getBoolean( tmpValue ) ) {
			// semiMajorAxis = 2. * semiMajorAxis * semiMinorAxis / ( semiMajorAxis +
			// semiMinorAxis
			// );
			...
			// semiMajorAxis *= .5 * ( 1. - eccentricitySquared + tmp ) / ( tmp *
			// Math.sqrt( tmp )
			// );
			// eccentricitySquared = 0.;
			// } else {
			...
			if (Double.isNaN(semiMajorAxis)) {
				throw new CRSConfigurationException(Messages.getMessage("CRS_CONFIG_PROJ4_ELLIPSOID_WITHOUT_SEMIMAJOR",
						params.get(EPSG_PRE + "identifier")));
			}
			...
			if (!Double.isNaN(eccentricitySquared)) {
				...
				result = new Ellipsoid(semiMajorAxis, eccentricity, Unit.METRE, CRSCodeType.valueOf(id), name, version,
				...
				result = new Ellipsoid(Unit.METRE, semiMajorAxis, semiMinorAxis, CRSCodeType.valueOf(id), name, version,
						description, areaOfUse);
			}
			else {
				LOG.debug("Only a semimajor defined, assuming a sphere (instead of an ellipsoid) is to be created.");
				result = new Ellipsoid(Unit.METRE, semiMajorAxis, semiMajorAxis, CRSCodeType.valueOf(id), name, version,
						description, areaOfUse);
			}
		}
		return result;
	...
	/**
	 * @param datumName of the datum to map.
	 * @param definedEllipsoid the ellipsoid to use (gotten from 'ellps' param) or
	 * <code>null</code> if a predefined ellipsoid should be used.
	...
	private GeodeticDatum getPredefinedDatum(String datumName, String definedEllipsoid, String crsID)
		...
		if (datumName != null && !"".equals(datumName.trim())) {
			...
			if ("GGRS87".equalsIgnoreCase(datumName)) {
				...
				confInfo = new Helmert(446.448, -125.157, 542.060, 0.1502, 0.2470, 0.8421, -20.4894,
						GeographicCRS.WGS84, GeographicCRS.WGS84, codes, null, null,
						new String[] {
				...
				if (definedEllipsoid == null || "".equals(definedEllipsoid) || "airy".equals(definedEllipsoid.trim())) {
				...
				confInfo = new Helmert(-263.0, 6.0, 431.0, 0, 0, 0, 0, GeographicCRS.WGS84, GeographicCRS.WGS84, codes,
						null, null, new String[] { "Derived at 5 stations." }, new String[] { "Tunisia" });
				...
				confInfo = new Helmert(482.530, -130.596, 564.557, -1.042, -0.214, -0.631, 8.15, GeographicCRS.WGS84,
						GeographicCRS.WGS84, codes, null, null, new String[] { "no epsg code was found" }, null);
				...
				confInfo = new Helmert(59.47, -5.04, 187.44, 0.47, -0.1, 1.024, -4.5993, GeographicCRS.WGS84,
						GeographicCRS.WGS84, codes, new String[] { "NZGD49 to WGS 84 (2)" },
						new String[] { "OSG-Nzl 4m" },
						new String[] {
								"hese parameter values are taken from NZGD49 to NZGD2000 (4) (code 1701) and assume that NZGD2000 and WGS 84 are coincident to within the accuracy of the transformation. For improved accuracy use NZGD49 to WGS 84 (4) (code 1670)." },
						new String[] { "New Zealand" });
				datumIDs = getPredefinedIDs("6272");
				datumNames = new String[] { "New Zealand Geodetic Datum 1949" };
				if (definedEllipsoid == null || "".equals(definedEllipsoid) || "intl".equals(definedEllipsoid.trim())) {
				...
				if ((crsID != null && !"".equals(crsID)) && "3068".equals(crsID) || "4314".equals(crsID)
					...
					confInfo = new Helmert(598.1, 73.7, 418.2, 0.202, 0.045, -2.455, 6.7, GeographicCRS.WGS84,
							GeographicCRS.WGS84, codes, new String[] { "DHDN to WGS 84" },
							...
							new String[] {
									"Germany - states of former West Germany - Baden-Wurtemberg, Bayern, Hessen, Niedersachsen, Nordrhein-Westfalen, Rheinland-Pfalz, Saarland, Schleswig-Holstein." });
					datumIDs = getPredefinedIDs("6314");
					datumNames = new String[] { "Deutsches Hauptdreiecksnetz" };
					datumVersions = new String[] { "2006-06-12" };
					datumDescriptions = new String[] {
							"Fundamental point: Rauenberg. Latitude: 52 deg 27 min 12.021 sec N; Longitude: 13 deg 22 min 04.928 sec E (of Greenwich). This station was destroyed in 1910 and the station at Potsdam substituted as the fundamental point." };
					datumAOU = new String[] {
							"Germany - states of former West Germany - Baden-Wurtemberg, Bayern, Hessen, Niedersachsen, Nordrhein-Westfalen, Rheinland-Pfalz, Saarland, Schleswig-Holstein." };
				}
				else {
					...
					confInfo = new Helmert(606.0, 23.0, 413.0, 0, 0, 0, 0, GeographicCRS.WGS84, GeographicCRS.WGS84,
							codes, new String[] { "RD/83 to WGS 84 (1)" }, new String[] { "OGP-Deu BeTA2007" },
							new String[] {
									"These parameter values are taken from DHDN to ETRS89 (8) (code 15948) as RD/83 and ETRS89 may be considered equivalent to DHDN and WGS 84 respectively within the accuracy of the transformation." },
	...
	/**
	 * This method was adopted from the com.jhlabs.map.proj.Projection Factory#initizalize
	 * method.
	 * @param projName name of the projection
	 * @return the deegree understandable String or <code>null</code> if the projName
	 * could not be mapped.
	 * @throws CRSConfigurationException if the projName could not be mapped.
	 */
	private Projection createProjection(String projName, Map<String, String> params) throws CRSConfigurationException {
		...
		if (projName != null && !"".equals(projName)) {
			...
			else if ("imw_p".equals(projName)) {// "Internation Map of the World
												// Polyconic"
			}
			...
			else if ("mbt_fps".equals(projName)) {// "McBryde-Thomas Flat-Pole Sine (No.
													// 2)"
			}
			...
			if (result == null) {
				throw new CRSConfigurationException(
						Messages.getMessage("CRS_CONFIG_PROJ4_UNKNOWN_PROJECTION", projName));
			}
		}
		return result;
	}
	...
	/**
	 * @param params the values to get the units or to_meter from.
	 * @return a unit create from the +unit parameter or Unit.METRE if not found.
	 * @throws CRSConfigurationException if the given unit parameter could not be mapped
	 * to a valid deegree-crs unit.
	 */
	private Unit createUnit(Map<String, String> params) throws CRSConfigurationException {
	 ...
	 * @param ellipsoidName defined in the proj lib
	 * @return an ellipsoid with an id and name or <code>null</code> if the ellipsoidName
	 * was null or empty.
	 * @throws CRSConfigurationException if the given name could not be mapped.
	 */
	private Ellipsoid getPredefinedEllipsoid(String ellipsoidName) throws CRSConfigurationException {
		if (ellipsoidName != null && !"".equals(ellipsoidName.trim())) {
			...
			if ("APL4.9".equalsIgnoreCase(ellipsoidName)) {
				...
				throw new CRSConfigurationException(
						Messages.getMessage("CRS_CONFIG_PROJ4_UNKNOWN_ELLIPSOID", ellipsoidName));
			}
			// String[] ids = new String[] { id };
			...
			if (!ellipsoidName.equals(id)) {
				ids = new CRSCodeType[] { CRSCodeType.valueOf(EPSG_PRE + id), CRSCodeType.valueOf(OGC_URN + id),
						CRSCodeType.valueOf(OPENGIS_URL + id), CRSCodeType.valueOf(OPENGIS_URN + id) };
			}
			Ellipsoid ellips;
			if (Double.isNaN(semiMinorAxis)) {
				...
				ellips = new Ellipsoid(Unit.METRE, semiMajorAxis, semiMinorAxis, ids, new String[] { name }, null, null,
						null);
			}
			return ellips;
		}
	...
	/**
	 * Helper method to parse day, month second formats. With a little help from
	 * com.jhlabs.map.AngleFormat
	 * @param text to pe parsed into degrees (or radians).
	 * @param toDegrees if the given text is in degrees
	 * @return a
	 */
	...
	/**
	 * Converts angle information to radians. With a little help from
	 * com.jhlabs.map.MapMath. For negative angles, d should be negative, m & s positive.
	 * @param d days
	 * @param m months
	 * @param s seconds.
	...
	/**
	 * Converts angle information to degrees. With a little help from
	 * com.jhlabs.map.MapMath. For negative angles, d should be negative, m & s positive.
	 * @param d days
	 * @param m minutes
	 * @param s seconds.
	...
	public CRSResource getCRSResource(CRSCodeType code) throws CRSConfigurationException {
	...
	protected CRS parseCoordinateSystem(Map<String, String> crsDefinition) throws CRSConfigurationException {
		...
		if ("longlat".equals(crsType)) {
			// the geo-crs should find it's own id and has no parent projected crs (null,
			// null).
	...
	// @Override
	// public Transformation parseTransformation( Map<String, String>
	// transformationDefinition )
	// throws CRSConfigurationException {
	// throw new UnsupportedOperationException(
	// "Parsing of transformation parameters is not applicable for proj4 configuration
	// files yet." );
	// }
