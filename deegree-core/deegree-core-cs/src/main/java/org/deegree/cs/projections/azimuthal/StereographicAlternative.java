...
/**
 * <code>StereographicAlternative</code> projection may be imagined to be a projection of
 * the earth's surface onto a plane in contact with the earth at a single tangent point
 * from the opposite end of the diameter through that tangent point.
 * <p>
 * An alternative approach is given by Snyder {@link StereographicAzimuthal}, where,
 * instead of defining a single conformal sphere at the origin point, the conformal
 * latitude at each point on the ellipsoid is computed. The conformal longitude is then
 * always equivalent to the geodetic longitude. This approach is a valid alternative to
 * the above, but gives slightly different results away from the origin point. It is
 * therefore considered by EPSG to be a different projection method. Hence this
 * implementation.
 * </p>
 * <p>
 * This projection is best known in its polar form and is frequently used for mapping
 * polar areas where it complements the Universal Transverse Mercator used for lower
 * latitudes. Its spherical form has also been widely used by the US Geological Survey for
 * planetary mapping and the mapping at small scale of continental hydrocarbon provinces.
 * In its transverse or oblique ellipsoidal forms it is useful for mapping limited areas
 * centered on the point where the plane of the projection is regarded as tangential to
 * the ellipsoid., e.g. the Netherlands. The tangent point is the origin of the projected
 * coordinate system and the meridian through it is regarded as the central meridian. In
 * order to reduce the scale error at the extremities of the projection area it is usual
 * to introduce a scale factor of less than unity at the origin such that a unit scale
 * factor applies on a near circle centered at the origin and some distance from it.
 * </p>
 *
 * <p>
 * The coordinate transformation from geographical to projected coordinates is executed
 * via the distance and azimuth of the point from the center point or origin. For a sphere
 * the formulas are relatively simple. For the ellipsoid the same formulas are used but
 * with auxiliary latitudes, known as conformal latitudes, substituted for the geodetic
 * latitudes of the spherical formulas for the origin and the point .
 * </p>
 * <quote>from <a href=
 * "http://www.posc.org/Epicentre.2_2/DataModel/ExamplesofUsage/eu_cs34j.html">http://www.posc.org/</a></quote>
 *
 * <p>
 * Determinations of oblique projections on an ellipsoid can be difficult to solve and
 * result in long, complex computations. Because conformal transformations can be made
 * multiple time without loss of the conformal property a method of determining oblique
 * projections involves conformal transformation of the elliptical coordinates to
 * coordinates on a conformal sphere. The transformed coordinates can now be
 * translated/rotated on the sphere and then converted to planar coordinates with a
 * conformal spherical projection. is Ce/2 1 − e sin φ 2 arctan K tanC (π/4 + φ/2) − π/2
 * (3.6) χ = 1 + e sin φ λc = Cλ (3.7) √ 1 − e2 Rc = (3.8) 1 − e2 sin2 φ0
 * </p>
 * From the <a href=
 * "http://members.verizon.net/~gerald.evenden/proj4/manual.pdf">libproj4-manual</a> by
 * Gerald I. Evenden
 *
...
public class StereographicAlternative extends AzimuthalProjection {
	...
	/**
	 * To determine the inverse solution, geographic coordinates from Gaussian sphere
	 * coordinates, execute with the initial value of φi−1 = χ and φi−1 iteratively
	 * replaced by φ until |φ − φi−1 | is less than an acceptable error value. (taken from
	 * the proj-lib manual.
	 * @param centralGeographicLatitude
	 */
	private synchronized Point2d pj_inv_gauss(IGeographicCRS geographicCRS, Point2d slp,
			double centralGeographicLatitude, double K) throws ProjectionException {
		...
		for (; i > 0; --i) {
			elp.y = 2. * Math.atan(
					num * srat(getEccentricity(geographicCRS) * Math.sin(slp.y), -.5 * getEccentricity(geographicCRS)))
					- HALFPI;
	...
	/**
	 * The conformal transformation of ellipsoid coordinates (φ, λ) to conformal sphere
	 * coordinates (χ, λ_c ), where λ is relative to the longitude of projection origin,
	 * R_c is radius of the conformal sphere. χ_0 is the latitude on the conformal sphere
	 * at the central geographic latitude of the projection.
	 */
	private synchronized Point2d pj_gauss(IGeographicCRS geographicCRS, double lambda, double phi,
		...
		slp.y = 2. * Math.atan(K * Math.pow(Math.tan(.5 * phi + QUARTERPI), centralGeographicLatitude)
	...
	private synchronized Map<PARAMS, Double> calculateParameters(IGeographicCRS geographicCRS) {
		...
		double cosc0 = 0;
		/**
		 * Two times the radius of the conformal sphere, which is denoted by Gerald I.
		 * Evenden as R_c
		 */
		double R2;
		/**
		 * THe latitude of on the conformal sphere which is denoted by Gerald I. Evenden
		 * as Chi_0
		 */
		double latitudeOnCS;
		/**
		 * The exponent of the calculation of conformal latitude Chi in Gerald I. Evenden
		 * (3.6)
		 */
		double clExponent;
		/**
		...
		double K;
		/**
		 * The central geographic latitude of the projection on the conformal sphere,
		 * denoted by Gerald I. Evenden (3.9) with C
		 */
		...
		double cosPhiSquare = getCosphi0() * getCosphi0();// Math.cos(
		...
		K = Math.tan(.5 * latitudeOnCS + QUARTERPI)
				/ (Math.pow(Math.tan(.5 * getProjectionLatitude() + QUARTERPI), centralGeographicLatitude)
						* srat(getEccentricity(geographicCRS) * sinPhi0, clExponent));
