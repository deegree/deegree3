...
/**
 * Three kinds of <code>CoordinateSystem</code>s (in this class abbreviated with CRS) are
 * supported in this lib.
 * <ul>
 * <li>Geographic CRS: A position (on the ellipsoid) is given in Lattitude / Longitude
 * (Polar Cooridnates) given in rad° min''sec. The order of the position's coordinates are
 * to be contrued to the axis order of the CRS. These lat/lon coordinates are to be
 * tranformed to x,y,z values to define their location on the underlying datum.</li>
 * <li>GeoCentric CRS: A position (on the ellipsoid) is given in x, y, z (cartesian)
 * coordinates with the same units defined as the ones in the underlying datum. The order
 * of the position's coordinates are to be contrued to the axis order of the datum.</li>
 * <li>Projected CRS: The position (on the map) is given in a 2D-tuple in pre-defined
 * units. The Axis of the CRS are defined (through a transformation) for an underlying
 * Datum, which can have it's own axis with their own units. The order of the position's
 * coordinates are to be contrued to the axis order of the CRS</li>
 * </ul>
 *
 * Summarizing it can be said, that each CRS has following features
 * <ul>
 * <li>A reference code (an casesensitive String identifying this CRS, for example
 * 'EPGS:4326' or 'urn:ogc:def:crs:OGC:2:84' or 'luref')</li>
...
public abstract class CRS extends CRSIdentifiable implements ICRS {
	...
	/**
	 * @param datum of this coordinate system.
	 * @param axisOrder the axisorder of this coordinate system.
	 * @param identity
	 */
	 ...
	 * @param datum of this coordinate system.
	 * @param axisOrder the axisorder of this coordinate system.
	 * @param codes of this coordinate system.
	 * @param names
	 * @param versions
	 * @param descriptions
	 * @param areasOfUse
	...
	/**
	 * @param transformations to use instead of the helmert transformation(s).
	 * @param datum of this crs
	 * @param axisOrder
	 * @param identity
	 */
	...
	/**
	 * Checks if the given axis match this.axisOrder[] in length, but flipped x/y
	 * ([0]/[1]) order.
	 * @param otherAxis the axis to check
		...
		synchronized (LOCK) {
			if (this.validDomain == null) {
				...
				try {
					...
					try {
						// rb: lookup the default WGS84 in the registry, it may be, that
						// the axis are swapped.
