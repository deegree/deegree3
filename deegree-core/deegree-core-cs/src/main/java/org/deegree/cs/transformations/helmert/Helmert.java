...
/**
 * Parameters for a geographic transformation into another datum. The Bursa Wolf
 * parameters should be applied to geocentric coordinates, where the X axis points towards
 * the Greenwich Prime Meridian, the Y axis points East, and the Z axis points North.
 *
...
public class Helmert extends Transformation {
	 ...
	 * @param ex Bursa Wolf rotation in arc seconds or in radians (by setting the flag).
	 * @param ey Bursa Wolf rotation in arc seconds or in radians (by setting the flag).
	 * @param ez Bursa Wolf rotation in arc seconds or in radians (by setting the flag).
	 * @param ppm Bursa Wolf scaling in parts per million.
	 * @param sourceCRS of this helmert transformation
	 * @param targetCRS of this helmert transformation
	 * @param identifiable object containing all relevant id.
	 * @param inRadians true if the rotation parameters are in radians
	 */
	...
	public Helmert(ICRS sourceCRS, ICRS targetCRS, CRSCodeType[] codes, String[] names, String[] versions,
		...
		this(0, 0, 0, 0, 0, 0, 0, sourceCRS, targetCRS,
	...
	/**
	 * Construct a conversion info with all parameters set to 0, the target crs is
	 * {@link GeographicCRS#WGS84}
	 * @param sourceCRS of this helmert transformation
	 * @param codes
	 */
	...
	public Helmert(double dx, double dy, double dz, double ex, double ey, double ez, double ppm, ICRS sourceCRS,
			...
			String[] areaOfUses) {
		this(dx, dy, dz, ex, ey, ez, ppm, sourceCRS, targetCRS,
				new CRSIdentifiable(codes, names, versions, descriptions, areaOfUses));
	}
	...
	/**
	 * Returns an affine transformation also known as the "Helmert" transformation. The
	 * matrix representation of this transformation (also known as "Bursa Wolf" formula)
	 * is as follows:
	 *
	 * <blockquote>
	 *
	 ...
	 * </blockquote>
	 *
	 * This affine transform can be applied to transform <code>geocentric</code>
	 * coordinates from one datum into <code>geocentric</code> coordinates of an other
	 * datum. see <a href=
	 * "http://www.posc.org/Epicentre.2_2/DataModel/ExamplesofUsage/eu_cs35.html#CS3523_helmert">
	 * http://www.posc.org/Epicentre.2_2/DataModel/ExamplesofUsage/eu_cs35.html</a> for
	 * more information.
	...
	public Matrix4d getAsAffineTransform() {
		...
		// Note: (ex, ey, ez) is a rotation in arc seconds. We need to convert it into
		// radians (the
		// R factor in RS).
		...
		transformMatrix = new Matrix4d(S, -ez * RS, +ey * RS, dx, +ez * RS, S, -ex * RS, dy, -ey * RS, +ex * RS, S, dz,
				0, 0, 0, 1.);
		return new Matrix4d(transformMatrix);
	}
	...
	/**
	 * Returns the Well Know Text (WKT) for this object. The WKT is part of OpenGIS's
	 * specification and looks like <code>TOWGS84[dx, dy, dz, ex, ey, ez, ppm]</code>.
	...
	@Override
	public synchronized List<Point3d> doTransform(List<Point3d> srcPts) throws TransformationException {
	...
	public static Transformation createAxisAllignedTransformedHelmertTransformation(Helmert transform) {
		...
		if (sourceCRS != null && targetCRS != null) {
			...
			try {
				...
				// result = concatenate( createMatrixTransform( sourceCRS, alignedSource,
				// first ), result,
				...
				LOG.warn(
						"Could not create an alignment matrix for the supplied Helmert transformation, are the coordinate systems correctly defined?");
			}
		}
		return result;
	}
