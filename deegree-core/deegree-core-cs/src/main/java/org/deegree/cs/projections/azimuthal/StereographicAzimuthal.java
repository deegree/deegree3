...
import static org.deegree.cs.utilities.ProjectionUtils.conformalLatitudeInnerPart;
...
/**
 * The <code>StereographicAzimuthal</code> class allows for Stereographic Projections of
 * the Poles, equator as well as oblique. This projection has following properties (Snyder
 * p. 154):
 * <ul>
 * <li>Azimuthal</li>
 * <li>Conformal</li>
 * <li>The central meridian an a particular parallel (if shown) are straight lines.</li>
 * <li>All meridians on the polar aspects and the equator on the equatorial aspect are
 * straight lines.</li>
 * <li>All other meidians and parallels are shown as arcs of circles</li>
 * <li>A perspective projections for the sphere</li>
 * <li>Directions from the center of the projection are true (except on ellipsoidal
 * oblique and equatorial aspects)</li>
 ...
 *
 * <p>
 * Like Orthographic, the stereographic projection is a true perspective in its
 * isSpherical() form. It is the only known true perspective projection of any kind that
 * is also conformal. Its point of projection is on the the surface of the sphere at a
 * point jus opposite the oint of tangency of the plane or the center point of the
 * projection. Thus, if the north pole is the center of the map, the projection is from
 * the south-pole.
 * </p>
 * <p>
...
public class StereographicAzimuthal extends AzimuthalProjection {
	...
	/**
	 * @param trueScaleLatitude the latitude (in radians) of a circle around the
	 * projection point, which contains the true scale.
	 * @param geographicCRS
	 * @param falseNorthing
	 * @param falseEasting
	 * @param naturalOrigin
	...
	public StereographicAzimuthal(double trueScaleLatitude, double falseNorthing, double falseEasting,
		...
		super(falseNorthing, falseEasting, naturalOrigin, units, scale, true,
	...
	/**
	 * Create a {@link StereographicAzimuthal} which has a true scale latitude at
	 * MapUtils.HALFPI.
	 * @param geographicCRS
	 * @param falseNorthing
	 * @param falseEasting
	...
	/**
	 * Create a {@link StereographicAzimuthal} which has a true scale latitude at
	 * MapUtils.HALFPI. Sets the id to "Snyder-StereoGraphic"
	 * @param geographicCRS
	 * @param falseNorthing
	 * @param falseEasting
	 * @param naturalOrigin
	...
	/**
	 * Create a {@link StereographicAzimuthal} which has a scale of 1 and a true scale
	 * latitude,
	 * @param trueScaleLatitude the latitude (in radians) of a circle around the
	 * projection point, which contains the true scale.
	 * @param geographicCRS
	...
	/**
	 * Create a {@link StereographicAzimuthal} which has a scale of 1 and a true scale
	 * latitude. Sets the id to "Snyder-StereoGraphic".
	 * @param trueScaleLatitude the latitude (in radians) of a circle around the
	 * projection point, which contains the true scale.
	 * @param geographicCRS
	 * @param falseNorthing
	...
	/**
	 * Create a {@link StereographicAzimuthal} which is conformal, has a scale of 1 and a
	 * truescale latitude at pi*0.5.
	 * @param geographicCRS
	 * @param falseNorthing
	 * @param falseEasting
	 * @param naturalOrigin
	...
	/**
	 * Create a {@link StereographicAzimuthal} which is conformal, has a scale of 1 and a
	 * truescale latitude at pi*0.5. Sets the id to "Snyder-StereoGraphic".
	 * @param geographicCRS
	 * @param falseNorthing
	 * @param falseEasting
	 * @param naturalOrigin
		...
		if (isSpherical(geographicCRS)) {
			...
			switch (getMode()) {
				case OBLIQUE:
					...
					// And now lambda but instead of using the atan, use atan2. Is this
					// correct????
			...
			double sinCe = 0;
			double X = 0;
			// for oblique and equatorial projections, akm1 = first part of Snyder (p.160
			// 21-27). which is 2*scale*m_1
			...
			switch (getMode()) {
				...
				case SOUTH_POLE:
					/**
					 * akm1 can hold rho from 21-34 or 21-33 (if no truescale latitude was
					 * given) without the parameter t. Either way it must only be
					 * multiplied with t to fulfill the formula. Independent of the true
					 * scale latitude, the value of akm1 must therefore only be inverted
					 * to calculate Snyder (p.162 21-39 or 21-40).
					 */
					double t = rho * 1. / akm1;
		...
		if (isSpherical(geographicCRS)) {
			...
			switch (getMode()) {
				case OBLIQUE:
					...
					// sinPhi0 and cosPhi0 where set to the conformal latitude of the
					// natural origin.
				...
				case SOUTH_POLE:
					...
					sinPhi = -sinPhi;
				case NORTH_POLE:
					// akm1 holds the rho's from 21-33 or 21-34, but without the t
					// (==halfColatitude of the conformal
					// latitude).
	...
	private synchronized Map<PARAMS, Double> calculateParameters(IGeographicCRS geographicCRS) {
		 ...
		 * <ul>
		 * <li>for Oblique projection it is the first Part (2*a*k_0*m_1, hence it's name)
		 * of A (p.160 21-27)</li>
		 ...
		 * <ul>
		 * <li>For oblique and equatorial: 2*scale, which is compliant with 2*k_0 from
		 * Snyder (p.157 21-4)</li>
		...
		if (!isSpherical(geographicCRS)) {
			...
			switch (getMode()) {
				...
				case SOUTH_POLE:
					 ...
					 */
					// If true scale or known scale factor k_0 is to occur at the pole,
					// the following applies:
					if (Double.isNaN(trueScaleLatitude) || Math.abs(Math.abs(trueScaleLatitude) - HALFPI) < EPS10) {
						// Snyder (p.161 21-33) akm1 = rho.
						akm1 = 2. * getScaleFactor(geographicCRS) / Math
							.sqrt(Math.pow(1 + getEccentricity(geographicCRS), 1 + getEccentricity(geographicCRS))
									* Math.pow(1 - getEccentricity(geographicCRS), 1 - getEccentricity(geographicCRS)));

					}
					else {
						// For true scale along the circle represented by
						// trueScaleLatitude (phi_c in snyder)..
				...
				case OBLIQUE:
					...
					// Calculate the ConformalLatitude for this ellipsoid Snyder (p.160
					// 3-1) the X
					conformalLatitude = (2. * Math
						.atan(conformalLatitudeInnerPart(getProjectionLatitude(), tmp, getEccentricity(geographicCRS))))
							- HALFPI;
					...
					tmp *= getEccentricity(geographicCRS);
					// the first part (2a*k_0*m) of the largeA in Snyder (p.160 21-27) is
					// stored in akm1 it still must be
