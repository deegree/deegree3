...
/**
 * The <code>Transformation</code> class supplies the most basic method interface for any
 * given transformation.
 *
 * The change of coordinates from one CRS to another CRS based on different datum is
 * 'currently' only possible via a coordinate <code>Transformation</code>.
 * <p>
 * The derivation of transformation parameters can be done empirically or analytically.
 * <p>
 * The quality (accuracy) of an empirical derivation strongly depends on the chosen
 * reference points, there allocation, and their number. Therefore different realizations
 * for transformations from one datum to another exist. *
 * </p>
 * <p>
...
public abstract class Transformation extends CRSIdentifiable {
	...
	/**
	 * Do a transformation, e.g. the incoming data will be transformed into other
	 * coordinates.
	 * @param srcPts the points which must be transformed, expected are following values
	 * either, long_1, lat_1, height_1, long_2, lat_2, height_2. or long_1, lat_1, long_2,
	 * lat_2
	 * @return the transformed points
	 * @throws TransformationException if a transform could not be calculated.
	 */
	public abstract List<Point3d> doTransform(final List<Point3d> srcPts) throws TransformationException;

	/**
	 * @return true if this transformation doesn't transform the incoming points. (e.g. is
	 * the id. matrix)
	 */
	 ...
	 * @param source containing the value (id or name) of the 'src' coourdinateSystem
	 * @param dest containing the value (id or name) of the 'dest' coourdinateSystem
	...
	/**
	 * Wraps the incoming coordinates into a List<Point3d> and calls the
	 * {@link #doTransform(List)}. The source array will be read according to the
	 * dimension of the source CRS {@link #getSourceDimension()} and the target
	 * coordinates will be put according to the dimension of the targetCRS
	 * {@link #getTargetDimension()}. If the sourceDim &lt; 2 or &gt; 3 a transformation
	 * exception will be thrown.
	 * @param srcOrdinates the array holding the source ('original') coordinates.
	 * @param startPositionSrc the position to start reading the coordinates from the
	 * source array (0 is the first).
	 * @param destOrdinates the array which will receive the transformed coordinates.
	 * @param startPositionDest the index of the destCoords array to put the results, if
	 * the result will exceed the array.length, the array will be enlarged to hold the
	 * transformed coordinates.
	 * @param length the number of source ordinates to transform
	 * @throws TransformationException If the sourceDim &lt; 2 or soureDim &gt 3;
	 * @throws IllegalArgumentException if
	 * <ul>
	 * <li>the srcCoords is null</li>
	 * <li>the startPositionSrc &gt; srcCoords.length</li>
	 * <li>the lastCoord &gt; startPositionSrc</li>
	 * <li>the number of source coordinates are not congruent with the source
	 * dimension</li>
	 * <li>the lastCoord &lt; startCoordSrc</li>
	 * <li>the source or target dimension &lt; 2 or &gt; 3</li>
	 * </ul>
	 */
	public void doTransform(double[] srcOrdinates, int startPositionSrc, double[] destOrdinates, int startPositionDest,
			int length) throws TransformationException {
		if (startPositionSrc < 0) {
			startPositionSrc = 0;
		}
		if (srcOrdinates == null) {
			throw new IllegalArgumentException(Messages.getMessage("CRS_PARAMETER_NOT_NULL",
					"doTransform(double[],int,double[],int,int)", "srcCoords"));
		}
	 ...
	 * @param coordinate to transform, if <code>null</code> null will be returned.
	 * @return the transformed coordinate.
	 * @throws TransformationException if the coordinate could not be transformed from the
	 * sourceCRS to the targetCRS.
	 */
	public Point3d doTransform(Point3d coordinate) throws TransformationException {
		if (coordinate == null) {
			return null;
		}
	...
	/**
	 * This method flags the transformation about it's state. If this transformation was
	 * inverse calling this method will result in a forward transformation and vice versa.
	 */
	public void inverse() {
		isInverse = !isInverse;
	}
	...
	/**
	 * @return a representation of this transformations name, including the 'Forward' or
	 * 'Inverse' modifier.
	 */
	...
	/**
	 * Checks if this transformation is the inverse of the other transformation, which
	 * means, this.sourceCRS equals other.targetCRS && this.targetCRS == other.sourceCRS.
	 * If Both transformations are identity this method also returns true.
	...
	/**
	 * @param sb to add the transformation chain to, if <code>null</code> a new
	 * StringBuilder will be created.
	 * @return the given StringBuilder (or a new instance) with the appended
	...
	/**
	 * Returns true if the source and target of this transformation equal the source and
	 * target of the given transformation.
	 * @param other transformation to check against
	 * @return true if the source and target coordinate systems of this transformation
	 * equal the source and target coordinate systems of the given transformation.
	 */
	...
	/**
	 * @param newSource to be used as the new source coordinate system.
	 */
	...
	/**
	 * @param sourceCRS from which ordinates will be transformed
	 * @param targetCRS to which ordinates will be transformed.
	 * @return true if this transformation can transform from the given source CRS to the
	 * target CRS.
	 */
	...
	public Transformation copyTransformation(CRSResource newId) {
		...
		if ("Concatenated-Transform".equals(this.getImplementationName())) {
			...
			Transformation firstT = firstTransform.copyTransformation(new CRSIdentifiable(new CRSCodeType(newFirstId)));
			...
			Transformation secondT = secondTransform
				.copyTransformation(new CRSIdentifiable(new CRSCodeType(newSecondId)));
			...
			result = new NTv2Transformation(sourceCRS, targetCRS, newId, ((NTv2Transformation) this).getGridfileRef());
		}
