...
public class TransformationParser extends DefinitionParser {
	...
	/**
	 * @param infoID to get the conversioninfo from.
	 * @return the configured wgs84 conversion info parameters.
	 * @throws CRSConfigurationException
	 */
	protected Helmert getConversionInfoForID(String infoID) throws CRSConfigurationException {
	...
	/**
	 * @param transformId to get the transformation for.
	 * @return the configured wgs84 conversion info parameters.
	 * @throws CRSConfigurationException
	 */
	public Transformation getTransformationForId(String transformId) throws CRSConfigurationException {
	 ...
	 */
	protected Transformation parseTransformation(XMLStreamReader reader) throws XMLStreamException {
		...
		if (className != null && !"".equals(className.trim())) {
			...
			if (HELM_ELEM.equals(transformName)) {
				...
				throw new CRSConfigurationException(Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "transformation",
						"definitions", "Transformation type: " + transformName
	...
	private Transformation parseLeastSquare(XMLStreamReader reader, ICRS sourceCRS, ICRS targetCRS,
			CRSResource identifiable) throws XMLStreamException {
	...
	private Transformation parseHelmert(XMLStreamReader reader, ICRS sourceCRS, ICRS targetCRS,
		...
		try {
			...
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "helmert", "definitions", e.getMessage()), e);
		}
	 ...
	 * @param sourceCRS
	 * @param targetCRS
	 * @return the (concatenated) configured transform between the source and the target
	 * crs. Calling this method is not the same as creating a new Transformation chain
	 * with the {@link TransformationFactory#createFromCoordinateSystems(CRS, ICRS)}
	 */
			...
			if (availableTransformations.containsKey(c)) {
				...
				for (Transformation t : set) {
					if (targetCRS.equals(t.getTargetCRS())) {
						...
						// found a possible hit, but it was not preferred (e.g. found NTv2
						// but Helmert is preferred.)
						hits.add(t);
					}
				}
