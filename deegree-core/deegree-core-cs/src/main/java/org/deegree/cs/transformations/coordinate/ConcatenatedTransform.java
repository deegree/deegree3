...
/**
 * The <code>ConcatenatedTransform</code> class allows the connection of two
 * transformations.
 * <p>
 * Calling inverse on this transformation will invert the whole underlying transformation
 * chain. For example, if A * (B *C)=D and D is this transformation calling D.inverse()
 * will result in (C.inverse * B.inverse) * A.inverse.
 * </p>
 *
...
public class ConcatenatedTransform extends Transformation {
	 ...
	 * transform applies two transforms, one after the other. The dimension of the output
	 * space of the first transform must match the dimension of the input space in the
	 * second transform.
	 * @param first The first transformation to apply to given points.
	 * @param second The second transformation to apply to given points.
	 ...
	 * second transform.
	 *
	 * Creates an CRSIdentifiable using the
	 * {@link Transformation#createFromTo(String, String)} method.
	 * @param first The first transformation to apply to given points.
	 * @param second The second transformation to apply to given points.
	 */
	public ConcatenatedTransform(Transformation first, Transformation second) {
		this(first, second, new CRSIdentifiable(CRSCodeType
			.valueOf(Transformation.createFromTo(first.getCode().toString(), second.getCode().toString()))));
	}
	...
	/**
	 * @return the firstTransform, which is the second transformation if this transform is
	 * inverse.
	 */
	...
	/**
	 * @return the secondTransform, which is the first transformation if this transform is
	 * inverse.
	 */
	 ...
	 * @param first The first transform to apply to points.
	 * @param second The second transform to apply to points.
	 * @param keepIdentity true if identity {@link Transformation}s should be kept in the
	 * resulting {@link ConcatenatedTransform} . Default value is <code>false</code>.
	...
	private static Transformation concatenateTransformations(Transformation first, Transformation second,
		...
		/*
		 * If one transform is the inverse of the other, just return an identitiy
		 * transform.
		 */
		...
		/*
		 * If both transforms use matrix, then we can create a single transform using the
		 * concatened matrix.
		 */
		 ...
		 * then concatenate <code>tr1</code> or <code>tr2</code> with one of step
		 * transforms.
		 */
	 ...
	 * @param keepIdentity true if identity {@link Transformation}s should be kept in the
	 * resulting {@link ConcatenatedTransform} . Default value is <code>false</code>.
	 * @return A concatenated transform, or <code>null</code> if all arguments were
	 * <code>null</code>.
	 */
	public static Transformation concatenate(final Transformation step1, final Transformation step2,
		...
		return concatenateTransformations(step1, concatenateTransformations(step2, step3, keepIdentity), keepIdentity);
	}
