...
/**
 *
 * The <code>MatrixTransform</code> class allows transformations using matrices. Although
 * technically n &times; m matrices are possible, at the moment only 2 &times; 2, 3
...
public class MatrixTransform extends Transformation {
	...
	public MatrixTransform(ICRS source, ICRS target, final GMatrix matrix) {
		this(source, target, matrix, new CRSIdentifiable(
	 ...
	 * @param target the target coordinate system.
	 * @param matrix
	 * @param transformationName the 'optional' name of the transformation, which is
	 * useful to specify the 'helmert' transformation.
	 * @param id an identifiable instance containing information about this transformation
	 */
	...
	/**
	 * @return true if this transformation holds an identity matrix (e.g. doesn't
	 * transform at all).
	 */
	@Override
		...
		for (int row = 0; row < numRow; row++) {
			for (int col = 0; col < numCol; col++) {
				double value = (matrix3D != null) ? matrix3D.getElement(row, col)
						: ((matrix4D != null) ? matrix4D.getElement(row, col) : matrix.getElement(row, col));
	...
	/**
	 * Transforms an array of floating point coordinates by this matrix. Because of the
	 * usage of a point3d some assumptions are made,
	 * <ol>
	 * <li>The result point will have the dimension of the number of rows-1.</li>
	 * <li>Only the first number of Columns -1 (=input dimension) are used from the
	 * incoming point to calculate the result.</li>
	 * <li>An output dimension > 3 (e.g. the number of rows > 4 ) results in an
	 * IllegalArgumentException.</li>
	 * <li>An input dimension > 3 (e.g. the number of columns > 4 ) results in an
	 * IllegalArgumentException.</li>
	 * <li>Only inputDimension values of the incoming point will be taken into
	 * account.</li>
	 * <li>If the input dimension == 4 the fourth value is assumed to be 1.</li>
	 * </ol>
	 *
	 * <p>
	 * For example, a square matrix of size 4&times;4. Is a three-dimensional
	 * transformation for incoming and outgoing coordinates. The transformed points
	 * <code>(x',y',z')</code> are computed as below (note that this computation is
	 ...
	 * @param m4d the matrix to use (e.g. the inverse matrix or the forward matrix.
	 ...
	 * @param m3d the matrix to use (e.g. the inverse matrix or the forward matrix).
	 * @param srcPts The array containing the source point coordinates.
	 */
	 ...
	 * @param sourceCRS used as the CRS for transforming ordinates on the matrix
	 * @param targetCRS used as the result CRS after transforming ordinates on the matrix
	 * @param matrix The matrix used to define the affine transform.
	 * @return A {@link MatrixTransform} mapping incoming ordinates given in the sourceCRS
	 * to the targetCRS.
	 * @throws TransformationException if the matrix is not affine.
	 *
	 */
	...
	/**
	 * Create a swap matrix to align the axis of the given coordinate systems and create a
	 * tranformation for the result.
	 * @param sourceCRS used as the CRS for transforming ordinates on the matrix
	 * @param targetCRS used as the result CRS after transforming ordinates on the matrix
	 * @return an axis aligned matrix transform, or <code>null</code> if the source and/or
	 * target are <code>null</code> or no transformation is needed.
	 * @throws TransformationException if the matrix could not be created or the result is
	 * not affine.
	 */
