...
public class MappingUtils {
	 ...
	 * @param operationName the name of the 'operation', normally an epsg urn is something
	 * like this: urn:ogc:def:${operationName}:EPSG::1234
	 * @param epsgCode to check
	...
	public static boolean matchEPSGString(String compare, String operationName, String epsgCode) {
		return compare != null && ((EPSG_DOUBLE + epsgCode).equalsIgnoreCase(compare)
				...
				|| (compare.toUpperCase().contains(EPSG_DOUBLE + epsgCode))
				|| (compare.toUpperCase().matches("[\\w/\\:.]*/EPSG/[\\w.]*/" + epsgCode)));

	}

	/**
	 * Iterates over all given Transformations and tries to map one of the given
	 * Transformations onto the some part of the given Transformation chain.
	 * @param userRequested a list of Transformations which should be used in the given
	 * transformation chain.
	 * @param originalChain the transformation chain which is to be replaced with some (or
	 * all) of the given transformations.
	...
	public static Transformation updateFromDefinedTransformations(List<Transformation> userRequested,
			Transformation originalChain) throws TransformationException {
	...
	/**
	 * Traverse the given original transformation chain and replace some (or all) part(s)
	 * of the transformation chain with the given transformation.
	 * @param originalChain
	 * @param tbu
	...
	private static Transformation traverseAndReplace(Transformation originalChain, Transformation tbu)
		...
		if (originalChain.contains(tbu.getSourceCRS()) && originalChain.contains(tbu.getTargetCRS())) {
			// some part of the transformation matches the given source and target of to
			// be used transformation, lets
			// find and replace it.
	...
	/**
	 * Reorganize the the given concatenated transformation so that some (or all) part(s)
	 * of the given transformation uses the given transformation.
	 * @param ct the original chain
	 * @param tbu the transformation to be used
	 * @return the transformation which uses the given tbu (if it matched).
	 * @throws TransformationException
	 */
		...
		if (!ct.getSourceCRS().equals(first.getSourceCRS())) {
			...
			if (!chain.isEmpty()) {
				if (chain.getLast() != null && chain.getLast().equals(first)) {
					// get out the last transformation (which is the first of the old
					// chain).
					chain.removeLast();
				}
				if (!chain.isEmpty()) {
					Transformation prev = chain.getFirst();
					// align the axis of source crs of the given transformation to the
					// targetcrs of the previous one.
					if (prev != null) {
		...
		if (!ct.getTargetCRS().equals(last.getTargetCRS())) {
			...
			if (!chain.isEmpty()) {
				if (chain.getFirst() != null && chain.getFirst().equals(last)) {
					// get out the first transformation (which is the last of the old
					// chain).
					chain.removeFirst();
				}
				if (!chain.isEmpty()) {
					...
					if (next != null) {
						Transformation allign = MatrixTransform.createAllignMatrixTransform(tbu.getTargetCRS(),
								next.getSourceCRS());
						// align the axis of target crs of the given transformation to the
						// source crs of the next
						// one.
						if (!TransformationFactory.isIdentity(allign)) {
							resultChain.add(allign);
						}
	...
	/**
	 * Traverse the concatenated transformation and recursively find all transformations
	 * which go from source to target crs.
	 * @param ct to traverse.
	 * @param sourceCRS starting crs
	 * @param targetCRS ending crs
	 * @param chain (stack) containing the transformations found recursively.
	 */
