...
public class ProjectionUtils {
	...
	/**
	 * The max and min of the projected word map in radians (-Math.PI, -HALFPI, TWOPI,
	 * Math.PI)
	 */
	...
	/**
	 * From the proj4 library, to determine small q which is needed to calculate the
	 * authalic (equal-areaed) latitude beta, on a sphere having the same surface area as
	 * the ellipsoid, relative to the ellipsoid. Snyder (3 -12).
	 * @param sinphi the sine of the angle between the positive z-axis and the line formed
	 * between the origin and P.
	 * @param e the eccentricity
	 * @return the q value from Snyder (3-12)
	 * @deprecated use {@link ProjectionUtils#calcQForAuthalicLatitude(double, double)}
	 * instead.
	 */
	@Deprecated
	 ...
	 * @param sinphi the sine of the angle between the positive z-axis and the line formed
	 * between the origin and P.
	 * @param eccentricity the eccentricity of the ellipsoid to map the sphere to.
	 * @return the q value from Snyder (3-12)
	 */
	public static double calcQForAuthalicLatitude(double sinphi, double eccentricity) {
		if (eccentricity >= EPS10) {
			...
			return (1 - es) * (sinphi / (1. - eAndSinphi * eAndSinphi)
					- (.5 / eccentricity) * Math.log((1. - eAndSinphi) / (1. + eAndSinphi)));
		}
		// we have a sphere.
	...
	/**
	 * Pre-Calculates the values (used for the adams? series) which will be used to
	 * calculate the phi value of an inverse projection. Snyder (3-5).
	 * @param eccentricitySquared the squared eccentricity from the ellipsoid to calculate
	 * the theta for.
	...
	/**
	 * Gets Phi from the given conformal latitude chi and the precalculated values (gotten
	 * from {@link ProjectionUtils#preCalcedThetaSeries(double)} ) of the adams? serie.
	 * From Snyder (3-5).
	 * @param chi the conformal latitude
	 * @param APA the precalculated values from the serie gotten from
	 * {@link ProjectionUtils#preCalcedThetaSeries(double)}.
	 * @return the Phi as a polarcoordinate on the ellipsoid or chi if the length of APA
	 * != 4.
	 */
	public static double calcPhiFromConformalLatitude(double chi, double[] APA) {
		...
		double tmp = chi + chi;
		return (chi + APA[0] * Math.sin(tmp) + APA[1] * Math.sin(tmp + tmp) + APA[2] * Math.sin(tmp + tmp + tmp)
				+ APA[3] * Math.sin(tmp + tmp + tmp + tmp));
	}
	...
	/**
	 * Pre-Calculates the values (used for the adams? series) which will be used to
	 * calculate the authalic latitude. Snyder (3-18).
	 * @param eccentricitySquared the squared eccentricity from the ellipsoid to calculate
	 * the authalic latitude for.
	 * @return the precalculated values.
	 * @deprecated use {@link ProjectionUtils#getAuthalicLatitudeSeriesValues(double)}
	 * instead.;
	 */
	@Deprecated
	 ...
	 * serie. From Snyder (3-18).
	 * @param beta authalic latitude.
	 * @param APA the precalculated values from the series gotten from
	 * {@link ProjectionUtils#getAuthalicLatitudeSeriesValues(double)}.
	 * @return the phi on the ellipsoid.
	 * @deprecated use
	 * {@link ProjectionUtils#calcPhiFromAuthalicLatitude(double, double[])} instead.;
	 */
	@Deprecated
	 ...
	 * @param dx of the vector
	 * @param dy of the vector
	 * @return the length
	 */
	...
	/**
	 * This method calculates the innerpart of the conformal latitude's definition (Snyder
	 * p.15 3-1). This formula is almost equal to the calculation of the half colatitude
	 * from the conformal latitude (Snyder p.108 15-9). They only differ a sign in the
	 * first term.
	 * @param phi to calculate the conformal latitude from
	 * @param sinphi the sinus of the phi.
	 * @param eccentricity of the ellipsoid to which the phi should be made conformal to.
	 * @return the value of the innerpart of the conformal latitude formula. i.e. tan(
	 * pi/4 <b>+</b> phi/2)<b>*</b>[(1-e*sin(phi))/1+e*sin(phi))]^e/2.
	 */
	 ...
	 * @param eccentricity of the ellipsoid to which the phi should be made conformal to.
	 * @return the value of the innerpart of the conformal latitude formula. i.e. tan(
	 * pi/4 <b>+</b> phi/2)*[(1-e*sin(phi))/1+e*sin(phi))]^e/2.
	...
	/**
	 * This method calculates the tangens of the half colatitude from the conformal
	 * latitude (Snyder p.108 15-9).
	 * @param phi to calculate the half of the co latitude of the conformal latitude from
	 * @param sinphi the sinus of the phi.
	 * @param eccentricity of the ellipsoid to which the phi should be made conformal to.
	 * @return the value of the tangens of half of the conformal latitude formula. i.e.
	 * tan( pi/4 <b>-</b> phi/2)<b>/</b>[(1-e*sin(phi))/1+e*sin(phi))]^e/2.
	 */
	...
	/**
	 * This method calculates the tangens of the half colatitude from the conformal
	 * latitude (Snyder p.108 15-9). This formula is almost equal to the calculation of
	 * the innerpart of the conformal latitude's definition (Snyder p.15 3-1). They only
	 * differ a sign in the first term.
	 * @param phi to calculate the half of the co latitude of the conformal latitude from
	 * @param sinphi the sinus of the phi.
	 * @param eccentricity of the ellipsoid to which the phi should be made conformal to.
	 * @return the value of the innerpart of the conformal latitude formula (given sign +
	 * or -). i.e. tan( pi/4 (+-) phi/2)*[(1-e*sin(phi))/1+e*sin(phi))]^e/2.
	...
	/**
	 * This method can be used to calculate the value of a variable called 'm' by Snyder
	 * (Snyder p.101 14-15).
	 * @param sinphi the sinus of the phi
	 * @param cosphi the cosinus of the phi
	 * @param eccentricitySquared the value eccentricity * eccentricity.
	...
	/**
	 * Copied these value from proj4, I think they are reformed to fit some rule... but I
	 * don't know which rule that is :-(
	 */
	...
	/**
	 * Pre Calculates the values for the series to calculate for a given ellipsoid with
	 * given eccentricity the distance along the meridian from the equator to a given
	 * latitude {@link #getDistanceAlongMeridian(double, double, double, double[])}.
	 * @param es the squared eccentricity of the underlying ellipsoid.
	...
	/**
	 * This method calcs for a a given ellispoid the distance along the meridian from the
	 * equator to latitude phi Snyder (p.17 3-21). It is used to calculate the rectifying
	 * latitude <i>mu</i>.
	 * @param phi the lattitude of the point in radians
	 * @param sphi the sinus of the latitude
	 * @param cphi the cosinus of the latitude
	 * @param en an array (of length 5) containing the precalculate values for this
	 * ellipsoid gotten from {@link #getRectifiyingLatitudeValues(double)}.
	 * @return the distance along the meridian from the equator to latitude phi.
	 * @deprecated Use {@link #getDistanceAlongMeridian(double,double,double,double[])}
	 * instead
	 */
	@Deprecated
	...
	/**
	 * This method calcs the distance along the meridian from the equator to latitude phi
	 * for a a given ellispoid Snyder (p.17 3-21). It is used to calculate the rectifying
	 * latitude <i>mu</i>.
	 * @param phi the lattitude of the point in radians
	 * @param sphi the sinus of the latitude
	 * @param cphi the cosinus of the latitude
	...
	/**
	 * This method calcs lattitude phi from a given distance along the meridian to the
	 * equator for a a given ellispoid and is therefore the inverse of the
	 * {@link #getDistanceAlongMeridian(double, double, double, double[])}. Phi is
	 * determined to EPS (1e-11) radians, which is about 1e-6 seconds.
	 * @param initialValue to calculate phi from, a good starting value is using the
	 * (distance along the meridian / y*scale) e.g. the scaled y value on the meridian.
	 * @param squaredEccentricity the squared eccentricity of the ellipsoid.
	 * @param en an array (of length 5) containing the precalculate values for this
	 * ellipsoid gotten from {@link #getRectifiyingLatitudeValues(double)}.
	 * @return the lattitude phi.
	 * @deprecated Use {@link #calcPhiFromMeridianDistance(double,double,double[])}
	 * instead
	 */
	@Deprecated
	 ...
	 * @param en an array (of length 5) containing the precalculate values for this
	 * ellipsoid gotten from {@link #getRectifiyingLatitudeValues(double)}.
	 * @return the lattitude phi or the best approximated value if no suitable convergence
	 * was found.
	 */
	...
	/**
	 * A helper method, which returns the acos from value or if value < -1 pi or value>1
	 * 0.
	 * @param value (in radians) from which the acos must be calculated
	...
	/**
	 * A helper method, which returns the asin from value or if value < -1 (-pi/2) or
	 * value>1 (pi/2).
	 * @param value (in radians) from which the asin must be calculated
	...
	/**
	 * A helper method modulos (pi)the given angle (in radians) until the result fits
	 * betwee -HALFPI and HALF_PI.
	...
	/**
	 * A helper method modulos (2*pi)the given angle (in radians) until the result fits
	 * betwee -PI and PI.
	 ...
	 * </code>
	 * @param inCoord to be converted to radians.
	 ...
	 * </code>
	 * @param inCoord to be converted to degrees.minsec
	 * @return the radian equivalent of the inCoord.
	 */
	 ...
	 * @param value to get the remainder from.
