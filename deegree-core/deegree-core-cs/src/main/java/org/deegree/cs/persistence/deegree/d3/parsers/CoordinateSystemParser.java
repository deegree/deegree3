...
public class CoordinateSystemParser extends DefinitionParser {
	 ...
	 * @param configURL to be used for the configuration.
	 */
	 ...
	 * @param reader to be parsed from
	 * @return an Array of axis defining their order.
	 * @throws CRSConfigurationException if a required element could not be found, or an
	 * xmlParsingException occurred, or the axisorder uses names which were not defined in
	 * the axis elements.
	 * @throws XMLStreamException
	 */
	protected Axis[] parseAxisOrder(XMLStreamReader reader) throws CRSConfigurationException, XMLStreamException {
		...
		while (new QName(CRS_NS, "Axis").equals(reader.getName())) {
			...
			try {
				...
				throw new CRSConfigurationException(
						Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "Axis", e.getMessage()), e);
			}
			nextElement(reader);
		...
		if (confAxis.isEmpty()) {
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "Axis", "No axis were parsed."));
		}
	 ...
	 * @param reader to parse the transformations for.
	 * @return the list of transformations or the empty list if no transformations were
	...
	/**
	 * @param reader from which the crs is to be created (using chached datums,
	 * conversioninfos and projections).
	 * @return a projected coordinatesystem based on the given xml-element.
	 * @throws CRSConfigurationException if a required element could not be found, or an
	 * xmlParsingException occurred.
	 * @throws XMLStreamException
	 */
	protected CRS parseProjectedCRS(XMLStreamReader reader) throws CRSConfigurationException, XMLStreamException {
		...
		// no need to get it from the cache, because the abstract provider checked it
		// already.
		...
		try {
			...
			throw new CRSConfigurationException(Messages.getMessage("CRS_CONFIG_PARSE_ERROR",
					"projectiontType or usedGeographicCRS", reader.getLocalName(), e.getMessage()), e);
		}
		...
		if (usedGeographicCRS == null || "".equals(usedGeographicCRS.trim())) {
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_REFERENCE_ID_IS_EMPTY", "UsedGeographicCRS", id.getCode()));
		}
	...
	/**
	 * @param reader positioned on the crs to be created (using cached datums,
	 * conversioninfos and projections).
	...
	/**
	 * @param reader from which the crs is to be created (using cached datums,
	 * conversioninfos and projections).
	...
	/**
	 * @param reader from which the crs is to be created.
	...
	protected CRS parseCompoundCRS(XMLStreamReader reader) throws XMLStreamException {
		...
		try {
			...
			throw new CRSConfigurationException(Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "usedCRS",
					((reader == null) ? "null" : reader.getLocalName()), e.getMessage()), e);

		}
		if (usedCRS == null || "".equals(usedCRS.trim())) {
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_REFERENCE_ID_IS_EMPTY", "usedCRS", id.getCode()));
		}
		...
		try {
			...
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "HeightAxis", e.getLocalizedMessage()), e);
		}
	...
	/**
	 * Parses the required usedDatum element from the given parentElement (probably a crs
	 * element).
	 * @param reader pointing to the required usedDatum element from.
	 * @param parentID optional for an appropriate error message.
	 * @return the Datum.
	 * @throws CRSConfigurationException if a parsing error occurred, the node was not
	 * defined or an illegal id reference (not found) was given.
	 * @throws XMLStreamException
	 */
	protected IGeodeticDatum parseReferencedGeodeticDatum(XMLStreamReader reader, String parentID)
		...
		try {
			...
			throw new CRSConfigurationException(
					Messages.getMessage("CRS_CONFIG_PARSE_ERROR", "datumID", parentID, e.getMessage()), e);
		}
	...
	public List<CRSCodeType[]> getAvailableCRSs() {
		...
		try {
			...
			XMLStreamReader reader = XMLInputFactory.newInstance()
				.createXMLStreamReader(url.toExternalForm(), confStream);
			XMLStreamUtils.nextElement(reader);
