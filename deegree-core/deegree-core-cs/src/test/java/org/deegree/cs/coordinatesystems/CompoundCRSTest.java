...
public class CompoundCRSTest extends TestCase {
	 ...
	 */
	@Test
	public void testInstantiation() throws IllegalArgumentException {

		// Source crs espg:28992
		Ellipsoid sourceEllipsoid = new Ellipsoid(6377397.155, Unit.METRE, 299.1528128,
				new CRSCodeType[] { new CRSCodeType("7004", "EPSG") });
		GeodeticDatum sourceDatum = new GeodeticDatum(sourceEllipsoid, null,
				new CRSCodeType[] { new CRSCodeType("DATUM_171") });
		GeographicCRS geoSource = new GeographicCRS(sourceDatum,
		...
		Projection sourceProj = new StereographicAlternative(463000.0, 155000.0,
				new Point2d(Math.toRadians(5.38763888888889), Math.toRadians(52.15616055555555)), Unit.METRE,
				0.9999079);
		ProjectedCRS sourceCRS = new ProjectedCRS(sourceProj, geoSource,
				new Axis[] { new Axis("x", Axis.AO_EAST), new Axis("y", Axis.AO_NORTH) },
		...
		Ellipsoid targetEllipsoid = new Ellipsoid(6378137.0, Unit.METRE, 298.257222101,
				new CRSCodeType[] { new CRSCodeType("1188", "ESPG") });
		Helmert targetWGS = new Helmert(GeographicCRS.WGS84, GeographicCRS.WGS84,
				new CRSCodeType[] { new CRSCodeType("1188", "ESPG") });
		GeodeticDatum targetDatum = new GeodeticDatum(targetEllipsoid, targetWGS,
				new CRSCodeType[] { new CRSCodeType("6258", "ESPG") });
		GeographicCRS targetGEO = new GeographicCRS(targetDatum,
				new Axis[] { new Axis("longitude", Axis.AO_EAST), new Axis("latitude", Axis.AO_NORTH) },
				new CRSCodeType[] { new CRSCodeType("4258", "ESPG") });
		Projection targetProj = new TransverseMercator(true, 0, 500000.0, new Point2d(Math.toRadians(9), 0), Unit.METRE,
				0.9996);
		ProjectedCRS targetCRS = new ProjectedCRS(targetProj, targetGEO,
				new Axis[] { new Axis("x", Axis.AO_EAST), new Axis("y", Axis.AO_NORTH) },
