 ...
 * the functionality of the Transformation factory independent of any underlying
 * configuration.
 *
...
public class TransformationAccuracyTest extends TransformationAccuracy implements CRSDefines {
	...
	public void testCompoundToCompound() throws TransformationException {
		...
		CompoundCRS sourceCRS = new CompoundCRS(heightAxis, projected_28992, 20, new CRSIdentifiable(
				new CRSCodeType[] { new CRSCodeType(projected_28992.getCode().getOriginal() + "_compound") }));

		// Target crs espg:25832
		CompoundCRS targetCRS = new CompoundCRS(heightAxis, projected_25832, 20, new CRSIdentifiable(
				new CRSCodeType[] { new CRSCodeType(projected_25832.getCode().getOriginal() + "_compound") }));
	...
	public void testCompoundToGeographic() throws TransformationException {
		...
		// reference created with coord tool from http://www.rdnap.nl/ denoting
		// (NL/Groningen/lichtboei)
		Point3d sourcePoint = new Point3d(236694.856, 583952.500, 1.307);
		Point3d targetPoint = new Point3d(6.610765, 53.235916, 42);

		doForwardAndInverse(sourceCRS, targetCRS, sourcePoint, targetPoint, EPSILON_D,
	...
	public void testCompoundToGeographicEqualOnEllips() throws TransformationException {
		...
		// EPSG:4326
		CompoundCRS sourceCRS = new CompoundCRS(heightAxis, GeographicCRS.WGS84_YX, 20,
				new CRSIdentifiable(new CRSCodeType[] { new CRSCodeType("urn:x-ogc:def:crs:EPSG:4979") }));
	...
	public void testCompoundToGeographicEqualOnEllipsNotOnAxis() throws TransformationException {
		...
		// urn:x-ogc:def:crs:EPSG:4326
		// EPSG:4326
		CompoundCRS sourceCRS = new CompoundCRS(heightAxis, GeographicCRS.WGS84_YX, 600.2,
				new CRSIdentifiable(new CRSCodeType[] { new CRSCodeType("urn:x-ogc:def:crs:EPSG:4979") }));
	...
	public void testCompoundToGeocentric() throws TransformationException {
		...
		CompoundCRS sourceCRS = new CompoundCRS(heightAxis, projected_31467, 20, new CRSIdentifiable(
				new CRSCodeType[] { new CRSCodeType(projected_31467.getCode().getOriginal() + "_compound") }));
	...
	/**
	 * Test the forward/inverse transformation from a projected crs (EPSG:31467) with yx
	 * axis order to a geographic crs (EPSG:4258).
	 * @throws TransformationException
	 */
	@Test
	...
	/**
	 * Test the forward/inverse transformation from a projected crs (EPSG:31467) based on
	 * a lat/lon geographic crs to a geographic crs (EPSG:4258).
	 * @throws TransformationException
	 */
	@Test
	...
	/**
	 * Test the forward/inverse transformation from a projected crs (EPSG:31467) to a
	 * geographic crs (EPSG:4258) which has lat/lon axis
	 * @throws TransformationException
	 */
	@Test
	...
	/**
	 * Test the forward/inverse transformation from a geographic crs (EPSG:4314) lat/lon
	 * to another geographic crs (EPSG:4258) lon/lat
	 * @throws TransformationException
	 */
	@Test
	...
	/**
	 * Test the forward/inverse transformation from a geographic crs (EPSG:4314) lat/lon
	 * to another geographic crs (EPSG:4258) lat/lon
	 * @throws TransformationException
	 */
	@Test
	...
	@Test
	public void testEpsg4647ToEpsg4326() throws IllegalArgumentException, TransformationException {
	...
	@Test
	public void testEpsg4647ToEpsg25832() throws IllegalArgumentException, TransformationException {
		ICRS sourceCRS = CRSManager.getCRSRef("epsg:4647");
