...
public class TransformationSubstitutionTest implements CRSDefines {
	...
	private final Transformation c = new NotSupportedTransformation(c1, c2, new CRSIdentifiable(new CRSCodeType("c")));

	private final Transformation e = new NotSupportedTransformation(c2, c3, new CRSIdentifiable(new CRSCodeType("e")));

	private final Transformation f = new NotSupportedTransformation(c3, c4, new CRSIdentifiable(new CRSCodeType("f")));

	private final Transformation h = new NotSupportedTransformation(c4, c5, new CRSIdentifiable(new CRSCodeType("h")));

	private final Transformation j = new NotSupportedTransformation(c5, c6, new CRSIdentifiable(new CRSCodeType("j")));

	private final Transformation k = new NotSupportedTransformation(c6, c7, new CRSIdentifiable(new CRSCodeType("k")));

	private final Transformation n = new NotSupportedTransformation(c7, c8, new CRSIdentifiable(new CRSCodeType("n")));

	private final Transformation p = new NotSupportedTransformation(c8, c9, new CRSIdentifiable(new CRSCodeType("p")));

	private final Transformation q = new NotSupportedTransformation(c9, c10, new CRSIdentifiable(new CRSCodeType("q")));
	...
	/**
	 * Tests if the Substitute of the automated transformation chain is totally replaced
	 * by the given 'single' transformation.
	...
	/**
	 * Test if the substitute of usage of the helmert transformation chain can be replaced
	 * by a {@link NTv2Transformation}.
	 * @throws IllegalArgumentException
	 * @throws TransformationException
	 */
	@Test
	public void substituteHelmertWithNTv2() throws IllegalArgumentException, TransformationException {
		...
		Transformation sub = new NTv2Transformation(geographic_4314, geographic_4258,
				new CRSIdentifiable(new CRSCodeType("urn:ogc:def:coordinateOperation:EPSG::15948")), beta2007);
	...
	public void substituteNTv2WithHelmert() throws IllegalArgumentException, TransformationException {
		...
		GeocentricTransform inv_geo = new GeocentricTransform(geographic_4258,
				new GeocentricCRS(datum_6258, axis_geocentric, new EPSGCode(14258)));
		inv_geo.inverse();
	...
	/**
	 * Test if the last three transformations of the 'a' transformation are found while
	 * replacing the first 3.
	 * @throws IllegalArgumentException
	 * @throws TransformationException
	 */
	@Test
	...
	/**
	 * Test if the first three transformations of the 'a' transformation are found while
	 * replacing the last 3.
	 * @throws IllegalArgumentException
	 * @throws TransformationException
	 */
	@Test
