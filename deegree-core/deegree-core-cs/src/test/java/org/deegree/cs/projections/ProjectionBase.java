...
/**
 * <code>ProjectionTest</code> is the base for all accuracy tests, this class doesn't
...
public class ProjectionBase {
	 ...
	 * @param origPoint to transform
	 * @param referencePoint to which the result shall be checked.
	 * @param forwardEpsilons for each axis
	 * @param projection to test.
	 * @param geographicCRS geographicCRS of the points
	 * @param inverse true if the inverse projection should be tested.
	...
	protected String doAccuracyTest(Point2d origPoint, Point2d referencePoint, Point2d forwardEpsilons,
			Projection projection, GeographicCRS geographicCRS, boolean inverse) throws ProjectionException {
		...
		// Point point = GeometryFactory.createPoint( origPoint.x, origPoint.y,
		// origPoint.z, sourceCRS );
		Point2d result = null;
		if (inverse) {
	...
	protected void doForwardAndInverse(Projection projection, GeographicCRS geographicCRS, Point2d source,
			Point2d target) throws ProjectionException {
	...
	/**
	 * Helper method to test if the projections have given values, and do not change
	 * internally.
	 * @param toBeTested
	 * @param falseNorthing
	 * @param falseEasting
	 ...
	 * @param conformal
	 * @param equalArea
	 * @param name of the projection
	 */
	protected void consistencyTest(Projection toBeTested, double falseNorthing, double falseEasting,
			Point2d naturalOrigin, Unit units, double scale, boolean conformal, boolean equalArea, String name) {
