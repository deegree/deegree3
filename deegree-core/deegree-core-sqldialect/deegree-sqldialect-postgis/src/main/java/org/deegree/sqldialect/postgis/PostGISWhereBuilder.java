...
public class PostGISWhereBuilder extends AbstractWhereBuilder {
	 ...
	 * @param defaultSortCriterion criteria to use for generating the ORDER-BY clause if
	...
	public PostGISWhereBuilder(PostGISDialect dialect, PropertyNameMapper mapper, OperatorFilter filter,
			SortProperty[] sortCrit, List<SortCriterion> defaultSortCriterion, boolean allowPartialMappings,
			boolean useLegacyPredicates) throws FilterEvaluationException, UnmappableException {
	 ...
	 * <p>
	 * NOTE: This method appends the generated argument inline, i.e. not using a
	 * <code>?</code>. This is because of a problem that has been observed with PostgreSQL
	 * 8.0; the execution of the inline version is *much* faster.
	 * </p>
		...
		switch (op.getSubType()) {
			case AFTER: {
				...
				if (isTimeInstant(parameter2)) {
					TimePosition timePosition = ((GenericTimeInstant) ((Literal<?>) parameter2).getValue())
						.getPosition();
	...
	private SQLExpression createDateExpressionFromTimePosition(TimePosition now) {
		...
		DefaultPrimitiveConverter converter = new DefaultPrimitiveConverter(new PrimitiveType(DATE_TIME), null, false);
	...
	private SQLExpression toProtoSQL(Geometry geom, ICRS targetCRS, int srid) throws FilterEvaluationException {
	...
	private SQLOperation createSqlDuring(SQLExpression valueReference, SQLExpression beginExpr, SQLExpression endExpr) {
