...
public interface SQLDialect {
	...
	/**
	 * @return the default table schema to use for the database (eg. for requesting table
	 * metadata)
	 */
	 ...
	 * @return statement to determine the coordinate dimension, the srid and the geometry
	 * type of a given column (in this order)
	 */
	 ...
	 * @param mapper provides property name mappings, must not be <code>null<code>
	 *
	@param filter
	 ...
	 * <code>null</code>
	 * @param allowPartialMappings if <code>false</code>, any unmappable expression will
	 * cause an {@link UnmappableException} to be thrown
	...
	AbstractWhereBuilder getWhereBuilder(PropertyNameMapper mapper, OperatorFilter filter, SortProperty[] sortCrit,
			List<SortCriterion> defaultSortCriteria, boolean allowPartialMappings)
	 ...
	 * {@link #getBBoxAggregateSnippet(String)} into an {@link Envelope}.
	 * @param rs
	 * @param colIdx
	 * @param crs
	...
	/**
	 * Creates a new (spatially-enabled) database using the specified administrator
	 * connection.
	 * @param adminConn administrator JDBC connection, must not be <code>null</code>
	 * @param dbName name of the database to be created, must not be <code>null</code>
	 * @throws SQLException
	 */
	...
	ResultSet getTableColumnMetadata(DatabaseMetaData md, TableName table) throws SQLException;

	/**
	 * Returns whether a transaction context is required for cursor mode to work.
	 * @return <code>true</code>, if a transaction context is required, <code>false</code>
	 * otherwise
	 */
	boolean requiresTransactionForCursorMode();

	/**
	 * Returns a <code>SELECT</code> statement for retrieving the next value in the
	 * specified DB sequence.
	 * @param sequence name of the database sequence, must not be <code>null</code>
