...
/**
 * Base class for creating SQL predicates from {@link Filter} expressions. Such an
 * expression restricts an SQL <code>ResultSet</code> to those rows that contain objects
 * that match the given filter. Also handles the creation of ORDER BY clauses.
 * <p>
 * Note that the generated WHERE and ORDER-BY expressions are sometimes not sufficient to
 * guarantee that a <code>ResultSet</code> only contains the targeted objects and/or keeps
 * the requested order. This happens when the {@link ValueReference}s used in the
 * filter/sort criteria are not mappable to columns in the database or the contained XPath
 * expressions are not mappable to an equivalent SQL expression. In these cases, one or
 * both of the methods {@link #getPostFilter()}/{@link #getPostSortCriteria()} return not
 * <code>null</code> and the objects extracted from the corresponding {@link ResultSet}
 * must be filtered/sorted in memory to guarantee the requested constraints/order.
 * </p>
 * <p>
 * TODO: Implement partial backend filtering / sorting. Currently, filtering / sorting is
 * performed completely by the database <i>or</i> by the post filter / criteria (if any
 * property name has been encountered that could not be mapped).
 * </p>
 *
...
public abstract class AbstractWhereBuilder {
	 ...
	 */
	protected void build(boolean allowPartialMappings) throws FilterEvaluationException, UnmappableException {
			...
			try {
				...
				if (preFilterBBox != null) {
					try {
						...
						LOG.warn(
								"Unable to map any filter constraints to WHERE-clause. Fallback to full memory filtering.");
		...
		if (sortCrit != null && sortCrit.length != 0) {
			try {
				...
				LOG.warn(
						"Using all sort criteria for post sorting step. Partial backend-sorting is not implemented yet. ");
	...
	/**
	 * Returns a {@link Filter} that contains all constraints from the input filter that
	 * could not be expressed in the WHERE clause.
	 * @return filter to apply on the objects from the <code>ResultSet</code>, may be
	 * <code>null</code> (no post-filtering necessary)
	 */
	...
	/**
	 * Returns the sort criteria that contains all parts from the input sort criteria that
	 * could not be expressed in the ORDER-BY clause.
	 * @return sort criteria to apply on the objects from the <code>ResultSet</code>, may
	 * be <code>null</code> (no post-sorting necessary)
	 */
	 ...
	 * <p>
	 * The returned manager may also be used for generating additional aliases that are
	 * needed for creating the final SQL statement.
	 * </p>
	...
	/**
	 * Returns the mappings of all {@link ValueReference}s from the filter / sort criteria
	 * that have been mapped to the relational model.
	 ...
	 * @param op operator to be translated, must not be <code>null</code>
	 ...
	 * {@link ValueReference}s
	 */
	protected SQLExpression toProtoSQL(Operator op) throws UnmappableException, FilterEvaluationException {
	...
	protected SQLExpression toProtoSQL(ComparisonOperator op) throws UnmappableException, FilterEvaluationException {
		...
		switch (op.getSubType()) {
			...
			case PROPERTY_IS_NIL: {
				...
				// TODO what if the xsi:nil attribute is not explicitly mapped (fallback
				// to NULL mapping?)
	 ...
	 * @param op logical operator to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 * {@link ValueReference}s
	 */
	protected SQLOperation toProtoSQL(LogicalOperator op) throws UnmappableException, FilterEvaluationException {
	 ...
	 * @param op spatial operator to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 ...
	 * @param op temporal operator to be translated, must not be <code>null</code>
	 ...
	 * @param expr expression to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 * {@link ValueReference}s
	 */
	protected SQLExpression toProtoSQL(Expression expr) throws UnmappableException, FilterEvaluationException {
	 ...
	 * @param function function to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, or <code>null</code> if function can not be
	 * mapped to an SQL function call
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 * {@link ValueReference}s
	 */
	protected SQLExpression toProtoSQL(Function function) throws UnmappableException, FilterEvaluationException {
	 ...
	 * <p>
	 * The returned literal does not have any type information. Literal types are inferred
	 * in {@link #toProtoSQL(ComparisonOperator)}.
	 * </p>
	 * @param literal literal to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 ...
	 * @param propName expression to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 * {@link ValueReference}s
	 */
	protected SQLExpression toProtoSQL(ValueReference propName) throws UnmappableException, FilterEvaluationException {
		SQLExpression sql = null;
	 ...
	 * @param sortCrits sort criteria to be translated, must not be <code>null</code>
	 * @return corresponding SQL expression, never <code>null</code>
	 * @throws UnmappableException if translation is not possible (usually due to
	 * unmappable property names)
	 * @throws FilterEvaluationException if the filter contains invalid
	 * {@link ValueReference}s
	 */
	protected SQLExpression toProtoSQL(SortProperty[] sortCrits) throws UnmappableException, FilterEvaluationException {
	...
	/**
	 * Ensures that the given {@link SQLExpression} is not an {@link SQLExpression} that
	 * is multi-valued.
	 * @param expr SQL expression, must not be <code>null</code>
	 * @throws UnmappableException
	 */
	protected void assertNotMultiValued(SQLExpression expr) throws UnmappableException {
