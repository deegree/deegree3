...
public class SDOGeometryConverterExampleTests {
	...
	private static final Pattern SDO_PAT = Pattern
		.compile(SDO_GEOM_PAT_1 + SDO_GEOM_PAT_2 + SDO_GEOM_PAT_3 + SDO_GEOM_PAT_4, Pattern.CASE_INSENSITIVE);
	...
	static Geometry readGMLGeometry(File file) throws Exception {
	...
	static String writeGMLGeometry(Geometry geom) throws Exception {
	...
	static SDOGeometry loadFromFile(File f) throws IOException {
		...
		if (m.matches()) {
			...
			if (NULL.equalsIgnoreCase(m.group(6))) {
				...
				point = new double[] { NULL.equalsIgnoreCase(m.group(3)) ? null : Double.parseDouble(m.group(3)),
						NULL.equalsIgnoreCase(m.group(4)) ? null : Double.parseDouble(m.group(4)),
						NULL.equalsIgnoreCase(m.group(5)) ? null : Double.parseDouble(m.group(5)) };
			}
