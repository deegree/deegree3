...
/**
 * The <code>Tesselator</code> class is a {@link GLUtessellator} utility wrapper. Its main
 * purpose is the creation of a {@link RenderableQualityModel} out of a
 * {@link GeometryQualityModel} by triangulating (tesselating) all it's
...
public class Tesselator {
	...
	/**
	 * Create a tesselator which triangulates all {@link SimpleAccessGeometry} of a
	 * {@link GeometryQualityModel}.
	...
	/**
	 * Create a renderable quality model from the given 'original' geometry model, by
	 * tesselating (triangulating) its polygons.
	 * @param objectID identifying the given quality model, if an error occurs. May be
	 * <code>null</code>
	 * @param originalObject containing polygons
	...
	/**
	 * Create a renderable geometry from the given {@link SimpleAccessGeometry} by
	 * tesselating it.
	 * @param originalGeometry may not be null.
	 * @return the {@link RenderableGeometry} or <code>null</code> if the given geometry
	...
	/**
	 * Create a callback for the given geometry and register it with the given tesselator
	 * object.
	 * @param tess
	 * @param geom
	 */
	...
	/**
	 * Tesselate the given geometry with respect to the innerrings and texture
	 * coordinates.
	 * @param tess
	 */
	...
	private RenderableGeometry tesselatePolygon(GLUtessellator tess, GeometryCallBack callBack) {
		...
		// the current ring
		int currentRing = 0;
		// the current position of the ring in the 3d float array float[0]=x ; float[1]=y;
		// float[2]=z;
		int ringBegin = 0;
		...
		glu.gluTessBeginPolygon(tess, null);
		{
			do {
				if (hasRings) {
					// we've got innerrings, eclipse doesn't seem to understand, that the
					// innerrings can not be null
			...
			// begin/next contour are in the tesselate ring function, just close the last
			// contour
