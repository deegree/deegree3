...
/**
 *
 * The <code>TextureTile</code> define the parameters necessary to create, request and
 * render a texture to the DEM. Texture are created and rendered with following options:
 * <ul>
 ...
 * <li>gl.glTexParameteri( GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_LINEAR
 * );</li>
 * <li>gl.glTexParameteri( GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_LINEAR
 * );</li>
 * </ul>
 *
...
public class TextureTile {
	...
	// true if the gl context unpack value must be set to 1 (the texture width can not be
	// divided by 4 ) :-)
	...
	/**
	 * @param gl context to load this texture to.
	 * @param refID of the enabling fragment.
	...
	public int enable(GL gl, int refID) {
		synchronized (LOCK) {
			...
			// numReferences++;
			// System.out.println( "--------------Enabling textureTile " + toString() + "
			// numrefs: "
	 ...
	 * @param gl
	 * @param refID of the fragment which needed this texture tile.
	 */
		...
		if (textureID == null) {
			if (imageData != null) {
				if (imageData.capacity() != dataSize) {
					...
					// System.out.println( "capacity: " + imageData.capacity() + " should
					// be: " + 3 * tWidth * tHeight
					// );
					if (unpack) {
	...
	// TextureTile tt = new TextureTile( minX, minY, maxX, maxY, textureWidth,
	// textureHeight, (ByteBuffer) null,
	// false, false );
	...
	// tt = new TextureTile( minX, minY, maxX, maxY, textureWidth, textureHeight, null,
	// false, false );
	// System.out.println( tt.hashCode() );
	// }
