...
public class TextureTileRequest {
	...
	/**
	 * @param request to copy the values from.
	 */
	...
	/**
	 * Returns whether this {@link TextureRequest} supersedes another
	 * {@link TextureRequest}.
	 * <p>
	 * This is true iff:
	 * <ul>
	 * <li>the bbox of the other request lies completely inside the bbox of this request
	 * (or coincides with it)</li>
	 * <li>the meters per pixel of the other request is less than or equal to the meters
	 * per pixel of this request</li>
	 * </ul>
	 * </p>
	 * @param that
	...
	public boolean supersedes(TextureTileRequest that) {
		return this.minX <= that.minX && this.minY <= that.minY && this.maxX >= that.maxX && this.maxY >= that.maxY
				&& this.unitsPerPixel <= that.unitsPerPixel/*
															 * Math.abs(
															 * this.unitsPerPixel -
															 * that.unitsPerPixel ) <= EPS
															 */;
	}

	/**
	 * @param candidate which values are taken into consideration.
	 * @param epsilon is used to determine the tolerance of the scale factor.
	 * @return true if this request lies within the area of the given 'candiate' and the
	 * the resolution of this is larger or withing epsilon range of the given candidate.
	 */
	public boolean isFullfilled(TextureTileRequest candidate, double epsilon) {
		return this.minX >= candidate.minX && this.minY >= candidate.minY && this.maxX <= candidate.maxX
				&& this.maxY <= candidate.maxY && ((this.unitsPerPixel >= candidate.unitsPerPixel)
						|| (Math.abs(this.unitsPerPixel - candidate.unitsPerPixel) < epsilon));
	}
