...
public class ViewFrustumCrit implements LODCriterion {
	 ...
	 * @param viewParams specifies the visible space volume (viewer position, view
	 * direction, etc.)
	 * @param maxPixelError maximum tolerable screen space error in pixels (in the
	 * rendered image)
	 * @param zScale scaling factor applied to z values of the mesh geometry (and bounding
	 * boxes)
	 * @param maxTextureSize maximum texture size (all fragments of the extracted LOD must
	 * have a size that is small enough, so they can be textured regarding this value and
	 * the <code>finestTextureResolution</code> parameter
	 * @param textureManagers texture managers that will be used to provide the textures
	...
	/**
	 * Returns true, iff the region associated with the arc is inside the view frustum
	 * volume and the estimated screen projection error is greater than the maximum
	 * tolerable error.
	 * @param arc arc to be checked
	 * @return true, iff the arc's region is inside the view frustum and the estimated
	 * screen projection error is greater than the maximum tolerable error
	 */
	@Override
	public boolean needsRefinement(Arc arc) {
		...
		if (!isScreenSpaceErrorAcceptable(arc)) {
			// step 2: only refine if the region currently violates the screen-space
			// constraint
			return true;
		}
	...
	/**
	 * Checks whether all fragments denoted by the given {@link Arc} are texturable with
	 * respect to the max texture size and texture resolutions.
	 * @param arc
	 * @return true, if all are texturable, false otherwise (at least one is not
	 * texturable)
	 */
	...
	/**
	 * Checks whether the screen-space error (after perspective projection) introduced by
	 * the fragments of the {@link Arc} is acceptable.
	 * @param arc
