...
/**
 * The <code>RenderableTexturedGeometry</code> is a {@link RenderableGeometry} which has
 * texture coordinates assigned to each vertex as well. Currently only one texture is
 * supported.
 *
...
public class RenderableTexturedGeometry extends RenderableGeometry {
	...
	public RenderableTexturedGeometry(float[] vertices, int openGLType, float[] vertexNormals,
			SimpleGeometryStyle style, String texture, float[] textureCoordinates, boolean useDirectBuffers) {
	...
	/**
	 * @param coordPosition in the direct vertex buffer
	 * @param vertexCount the number of vertices
	 * @param openGLType the opengl type
	 * @param normalPosition position in the direct normal buffer
	 * @param style to be applied
	 * @param textureID of the texture to use.
	 * @param texturePosition in the direct texture buffer.
	 */
	...
	private void loadTextureCoordinates(float[] textureCoordinates) {
		...
		if (textureCoordinates.length / 2 != getVertexCount()) {
			throw new IllegalArgumentException("The number of texture coordinates (" + (textureCoordinates.length / 2)
