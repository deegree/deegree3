...
public class RasterAPITextureTileProvider implements TextureTileProvider {
	 ...
	 * @param raster the raster api raster.
	 *
	 */
	...
	private TextureTile getTextureTile(double minX, double minY, double maxX, double maxY) {
		...
		// RasterRect r = raster.getRasterReference().convertEnvelopeToRasterCRS(
		// subsetEnv );
		...
		// }
		// LOG.warn(
		// "The requested texture for the world coordinates will result in a texture with
		// odd (not even) texture ( "
		...
		// ByteBufferRasterData bbData = RasterDataFactory.createRasterData( rect.width,
		// rect.height,
		// raster.getRasterDataInfo().bandInfo,
		// rasterData.getDataType(),
		// InterleaveType.PIXEL, false );
		// bbData.setByteBuffer( pixelBuffer, new DataView( new RasterRect( 0, 0,
