...
public class JOGLUtils {
	 ...
	 * <p>
	 * NOTE: This method evaluates the value of <code>GL_MAX_TEXTURE_IMAGE_UNITS</code>.
	 * The number is usually not identical to <code>GL_MAX_TEXTURE_UNITS</code> which
	 * denotes the maximum number of texture units in fixed shader functions.
	 * </p>
	 * @param gl
	...
	/**
	 * Returns the value of the symbolic constant TEXTURE0...TEXTURE31 for a certain
	 * texture unit id.
	 * @param textureId id of the requested texture unit (0...31)
	...
	public static int getTextureUnitConst(int textureId) {
		...
		throw new IllegalArgumentException(
				"Invalid texture unit (=" + textureId + ") specified. OpenGL defines TEXTURE0...TEXTURE31.");
	}
	 ...
	 * @param gl
	 * @param GL_MATRIX_TYPE one of {@link GL#GL_MODELVIEW_MATRIX},
	 * {@link GL#GL_PROJECTION_MATRIX}
	 ...
	 * @param gl to get the modelview from.
	...
	/**
	 * Create an a float array from the given color object, which can be used for
	 * rendering with jogl.
	 * @param color to be converted.
	 ...
	 * @param color (rgba) to be converted into a float arra.
	 * @return the float array ready to be rendered.
	 */
	public static float[] convertColorIntAsFloats(int color) {
		return new float[] { ((color >> 24) & 0xFF) / 255f, ((color >> 16) & 0xFF) / 255f, ((color >> 8) & 0xFF) / 255f,
				(color & 0xFF) / 255f, };
	}

	/**
	 * Create an int value ([a]rgb) from the given color array (rgb[a]), the result can be
	 * used for buffered images.
	 * @param color to be converted may be of length 3 or 4, not <code>null</code>.
	...
	/**
	 * Create an int value (rgb) from the given color array (rgb), the result can be used
	 * for buffered images.
	 * @param color to be converted may be only be of length 3, not <code>null</code>.
	...
	/**
	 * Read the framebuffer's rgba values and place them into a {@link ByteBuffer}. If the
	 * resultImage was <code>null</code> or it's height or width are to small a new
	 * ByteBuffer is created.
	 * @param glContext to get the image from.
	 * @param imageBuffer to write the framebuffer to.
	 * @param viewPortX the x location in the framebuffer.
	 * @param viewPortY the y location in the framebuffer.
	 * @param width of read in the framebuffer.
	 * @param height of read in the framebuffer.
	 * @return the resultImage or a newly created BufferedImage holding the requested data
	...
	/**
	 * Read the framebuffer's rgb values and place them into a {@link BufferedImage}. If
	 * the resultImage was <code>null</code> or it's height or width are to small a new
	 * BufferedImage is created. The resultImage type is supposed to be
	 ...
	 * @param width of read in the framebuffer.
	 * @param height of read in the framebuffer.
	 * @param resultImage which will hold the result, may be <code>null</code>
	 * @return the resultImage or a newly created BufferedImage holding the requested data
	 * from the framebuffer.
	 */
	...
	/**
	 * Read the framebuffer's rgb values and place them into a {@link ByteBuffer}. If the
	 * resultImage was <code>null</code> or it's height or width are to small a new
	 * ByteBuffer is created.
	 * @param glContext to get the image from.
	 * @param imageBuffer to write the framebuffer to.
