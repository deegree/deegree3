...
/**
 * A {@link TextureTile} applied to a {@link RenderMeshFragment}, also wraps OpenGL
...
public class FragmentTexture {
	...
	/**
	 * @param fragment needed to generate the texture coordinates
	 * @param texture
	 ...
	 * The texture coordinates will be put into the given direct buffer.
	 ...
	 * @param gl to generate the buffer object to.
	 */
	public void enable(GL gl) {
		...
		// System.out.println( "Fragment bound to: " + fragment.getId() + "| gl texture
		// id: " + textureID );
	...
	public void unload() {
		if (glBufferObjectIds == null && textureID == -1) {
			...
			LOG.warn(
					"Trying to free a buffer which is still on the gpu, this should not happen, because no texture coordinates would be left, ignoring request, and hope for the best. ");
			if (LOG.isDebugEnabled()) {
				Thread.dumpStack();
			}
		}
	...
	/**
	 * Clears up all data in this fragment texture, this disables the texture and frees up
	 * all OpenGL references, as well as releasing the direct texture coordinates buffer.
	 * @param gl openGL context to which the texture was bounded to.
	 */
