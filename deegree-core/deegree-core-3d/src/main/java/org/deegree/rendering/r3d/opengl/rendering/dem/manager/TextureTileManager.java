...
/**
 * Manages the fetching (and caching) of {@link TextureTile} instances from
 * {@link TextureTileProvider}s.
 *
...
public class TextureTileManager {
	...
	/**
	 * @param unitsPerPixel
	 * @return the native resolution of the configured TextureProvider, based on the
	 * meters per pixel.
	 */
	public double getMatchingResolution(double unitsPerPixel) {
		TextureTileProvider match = getMatchingProvider(unitsPerPixel);
		if (match != null) {
			// System.out.println( "The match: " + match + " requested upp: " +
			// unitsPerPixel );
		...
		// match was null or the provider does not supply a texture for the given
		// resolution.
		return Double.NaN;
	}
	...
	public TextureRequest createTextureRequest(RenderContext glRenderContext, double[][] fragmentBBoxWorldCoordinates,
		...
		if (provider instanceof RasterAPITextureTileProvider) {
			// RasterGeoReference geoRef = ( (RasterAPITextureTileProvider) provider
			// ).getRasterReference();
		...
		// following values are note the half distance to the next pixel in world
		// coordinates.
