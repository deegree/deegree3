...
/**
 * Provides a texture created from geometries supplied by a Featurstore and filled with a
 * color or fillpattern, which columns are the geometries and which columns define the
 * color/texture is defined by SE-Style file. Texture rendering is done by using the
 * {@link Java2DRenderer}.
 *
...
public class StyledGeometryTTProvider implements TextureTileProvider {
	 ...
	 * @param offsetVector translation vector of the WPVS
	 * @param wpvsCRS crs of the WPVS
	 * @param featureStore to get the features / geometries from.
	 * @param style used to evaluate the given features.
	 * @param maxUnitsPerPixel for which this provider will become active.
	 * @param cacheDir to cache textures
	...
	public StyledGeometryTTProvider(double[] offsetVector, ICRS wpvsCRS, FeatureStore featureStore, Style style,
		...
		if (styleFT != null) {
			...
			if (sFT == null) {
				posFT = null;
				LOG.warn(
						"The defined syle featuretype did not exist in the feature store, trying to use the first feature type with geometries.");
			}
		}
	...
	public TextureTile getTextureTile(TextureTileRequest request) {
		...
		// boolean debug = Math.abs( minX - 2579520.0 ) < 0.001 && Math.abs( minY -
		// 5620912.0 ) < 0.001;
		// if ( debug ) {
		...
		if (imageBuffer == null) {
			...
			// if ( imageWidth % 2 != 0 ) {
			// double dW = ( resolution + ( resolution * ( imageWidth - iWidth ) ) ) *
			// 0.5;

			// System.out.println( "GEOM: Texturewidth " + imageWidth + " is not even with
			// resolution: " +
			// resolution
			// + ", updating world width : " + worldWidth + " to " + ( worldWidth + ( 2 *
			// dW ) )
			...
			// }
			// if ( imageHeight % 2 != 0 ) {
			// double dH = ( resolution + ( resolution * ( imageHeight - iHeight ) ) ) *
			// 0.5;
			// System.out.println( "GEOM: TextureHeight " + imageHeight + " is not even
			// with resolution: " +
			// resolution
			// + ", updating world height: " + worldHeight + " to " + ( worldHeight + ( 2
			// * dH ) )
			...
			try {
				...
				imageBuffer = ByteBuffer
					.wrap((byte[]) imageRaster.getDataElements(0, 0, imageWidth, imageHeight, null));
				imageBuffer.rewind();
				if (textureCache != null) {
	...
	private void addToCache(ByteBuffer imageBuffer, TextureTileRequest request, double resolution,
		...
		long sT = currentTimeMillis();
		RasterGeoReference gRef = new RasterGeoReference(CENTER, resolution, -resolution, tileEnvelope.getMin().get0(),
				tileEnvelope.getMax().get1(), tileEnvelope.getCoordinateSystem());
		RasterDataInfo rdi = new RasterDataInfo(
				new BandType[] { BandType.ALPHA, BandType.RED, BandType.GREEN, BandType.BLUE }, DataType.BYTE,
