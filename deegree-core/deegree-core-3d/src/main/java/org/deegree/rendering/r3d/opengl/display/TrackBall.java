...
/**
 * The <code>TrackBall</code> pretends that a ball encloses the 3d view. You roll this
 * ball with the mouse. For example, if you click on the center of the ball and move the
 * ball directly right, you rotate around y. Click on edge of ball and roll to get a z
 * rotation.
 *
 * The idea isn't too hard. Start with a vector from the first mouse click to the center
 * of the 3d view. Set the radius of the ball to the smaller dimension of the 3d view. As
 * you drag around, a second vector is determined from the surface to center of the ball.
 * Axis of rotation is cross-product of those two vectors, and the angle is the angle
 * between the vectors.
 *
 * This class was copied from a cpp file I once used in my Computer Graphic classes. I
 * don't know who the original author was, but since I only created the java
 * representation of the working code, I would like to thank him/her.
 *
...
public class TrackBall extends MouseAdapter {
	...
	/**
	 * Multiply the rotation vectors on top of the current context, the multiplication
	 * also uses the translation
	 * @param context to multiply the rotations upon.
	 * @param worldTranslation a float[3] which contains a translation vector (normally
	 * the center of the scene ), if null or length != 3 no translation will be done.
	 */
	...
	/**
	 * Initialize a new rotation by calculating the startposition on as well as the center
	 * and radius of the (imaginary) sphere.
	 * @param drawable to get the window height and width from.
	 * @param point
	 */
	private void start(GLAutoDrawable drawable, Point point) {
		...
		// Adjust the window width/height so that the ball won't be filling the whole
		// window,
		// if you solemnly want to rotate the z axis, you need to be outside of the sphere
		// 'all-the-time'
	...
	/**
	 * Calculate the current rotation while dragging, by identifying the current position
	 * relative to the start position and normalizing the rotation axis calculate using
	 * simple trigonometry.
	 * @param mousePosition while dragging
	 * @return <code>null</code> if the dragging distance was to small, otherwise the new
	 * temporary rotation axis since the last rotation.
	 */
	private float[] rollTo(Point mousePosition) {
		...
		// sin = ||(s x e)||/(||s|| ||e||)
		float lr = 0;
		float sinAng = lr = (float) Math.sqrt(rot[1] * rot[1] + rot[2] * rot[2] + rot[3] * rot[3]); // ||(s
																									// x
																									// e)||
		sinAng = sinAng * ls * le;
	 ...
	 * </code> if the identity rotation was found ( cos (0.5*angle) == 1 ) the rotation
	 * vector will be set to identity
	 */
	private void addToRotation() {
		...
		// here is non-identity rotation. cos of half-angle is non-zero, so sine is
		// non-zero
	 ...
	 * @param r the new rotation parameters
	 */
	...
	/**
	 * convert GL rotation to a quaternion. GL looks like: {ang, x, y, z} and quat looks
	 * like: {{v}, cos(angle/2)} where {v} is (x,y,z)/sin(angle/2)
	 * @param A The original opengl rotation
