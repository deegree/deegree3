...
/**
 * The <code>RenderableGeometry</code> class uses VertexArrays to render the coordinates
 * of it's geometry in an openGL context. For this to work the coordinates are expected to
 * be organized as defined by the given openGLType.
 * <p>
 * Normally you might want to use a set of geometries for a single object (for example to
 * create walls of a house). In this case the easiest way to create a
 * {@link RenderableQualityModel} (containing {@link RenderableGeometry}(ies)) is the
 * usage of the {@link Tesselator}. The {@link Tesselator} class can be used to create a
 * sole {@link RenderableGeometry} as well.
 * </p>
 *
...
public class RenderableGeometry implements RenderableQualityModelPart {
	...
	private void createColors() {
		// ambientColor = new float[] { 0.6f, 0.6f, 0.6f, 1 };// convertColorIntAsFloats(
		// style.getAmbientColor() );
	 ...
	 * @param geomBuffer for which the coord/normal Positions are valid for.
	 */
		...
		if (hasNormals) {
			if (vertexNormals.length % 3 != 0) {
				throw new IllegalArgumentException(
						"The number of vertex normals(" + (vertexNormals.length) + ") must be kongruent to 3.");
			}
	...
	/**
	 * @return a floatbuffer containing the normals or <code>null</code> if normals are
	 * not loaded.
	 */
