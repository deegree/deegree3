...
/**
 * The <code>TreeManager</code> will hold the bill board references depending on their
 * texture id.
 *
...
public class TreeRenderer extends RenderableManager<BillBoard> {
	...
	private static final FloatBuffer normalBuffer = BufferUtil.copyFloatBuffer(FloatBuffer.wrap(new float[] { 0, -1, 0, // ll
			0, -1, 0, // lr
			0, -1, 0, // ur
			0, -1, 0 }// ul
	));

	private static final FloatBuffer textureBuffer = BufferUtil
		.copyFloatBuffer(FloatBuffer.wrap(new float[] { 0, 1, 1, 1, 1, 0, 0, 0 }));
	...
	private class DistComparator implements Comparator<BillBoard> {
		...
		/**
		 * @param eye to compare this billboard to.
		 *
		 */
