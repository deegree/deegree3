...
/**
 * The <code>AllocatedHeapMemory</code> class provides methods for calculating the heap
...
public class AllocatedHeapMemory {
	...
	/**
	 * This will return the size of an instance of a new Object, if the asReference is
	 * true, the size of a reference will be added as well.
	 * @param asReference
	 ...
	 * @param s to check
	 * @param asReference true if the String is a reference inside another class (if the
	 * size of a reference should be added).
	 * @param internalised the resulting string will not be internalised, for example if a
	...
	/**
	 * @param o
	 * @param asReference true if the array is a reference inside another class (if the
	 * size of a reference should be added).
	...
	private static long sizeOfList(LinkedList<?> ll) {
		...
		if (ll != null) {
			...
			// take 16 (for the elementData class and the length) plus 4 *
			// elementData.length.
			// We then follow the
			// hierarchy up and discover the variable int modCount in the superclass
			// java.util.AbstractList, which will
	 ...
	 * @param l get the bytes for.
	 * @param asReference true if the list is a reference inside another class (if the
	 * size of a reference should be added)
	...
	/**
	 * @param b
	 * @param asReference true if the buffer is a reference inside another class (if the
	 * size of a reference should be added)
	...
	// * to get the size for.
	// * @param asReference
	// * true if the Envelope is a reference inside another class (if the size of a
	...
	// // an id String
	// // a crs
	// localSize = instanceAndReferenceSize( asReference ) + sizeOfPoint( env.getMin(),
	// true, false )
	...
	// * to check
	// * @param asReference
	// * true if the Point is a reference inside another class (if the size of a reference
	// should be added)
	// * @param withCRS
	...
	// // an id String
	// // a crs
	// localSize += instanceAndReferenceSize( asReference ) + sizeOfString( p.getId(),
	// true, true )
	...
	// * @param asReference
	// * @return 0 or 256 bytes, this method should check all the references and
	// components... not so trivial.
	// */
