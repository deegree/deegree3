...
/**
 * The <code>QTree</code> is a quadtree based organization of a scene containing
...
public class QTree<T> extends SpatialIndex<T> implements Serializable {
	...
	/**
	 * @param validDomain
	 * @param numberOfObjects each node will contain
	 */
		...
		return getEnvString() + ": "
				+ ((isLeaf()) ? " is a leaf with " + leafObjects.size() + " leafObjects."
						: (" is a node with: " + ((children[0] == null) ? "" : " ll,")
								+ ((children[1] == null) ? "" : " lr,") + ((children[2] == null) ? "" : " ul,")
								+ ((children[3] == null) ? "" : " ur,")));
	...
	private boolean removeObject(T object) {
		...
		// none of the totally covering objects matched the given object, try the
		// subtrees.
		if (!result) {
			if (isLeaf()) {
	...
	/**
	 * Try to remove the given object from the set of objects which cover this nodes whole
	 * envelope
	 * @param object to be removed
	 * @return true if the object was in the list of the objects covering this nodes'
	 * total area.
	 */
	...
	private Set<Entry<T>> validateChildrenSize() {
		...
		if (!isLeaf()) {
			for (int i = 0; i < children.length; ++i) {
				...
				if (n != null) {
					if (n.isLeaf()) {
						// if one of the other children wasn't a leaf, don't bother
						// getting the objects
						if (possibleMerge) {
							n.getObjects(n.envelope, childrenInLeaf);
						}
						// the child is a leaf but does not contain any leafObjects, no
		...
		for (int i = 0; i < objectList.size(); ++i) {
			...
			if (!checked) {
				...
				for (int j = i + 1; j < objectList.size(); ++j) {
					...
					// if min and max have distance 0, then just count as one because they
					// are equals, this might
	...
	private boolean hasDuplicateLocation(final float[] objectEnvelope) {
		for (Entry<T> obj : leafObjects) {
			...
			// if min and max have distance 0, then just count as one because they are
			// equals, this might
			// prevent a stack overflow
			if ((minD < SPLIT_CRITERIA_EPSILON && maxD < SPLIT_CRITERIA_EPSILON)) {
				return true;
	...
	/**
	 * @return true if the number of objects in the leaf without the duplicate envelopes
	 * are larger than the allowed number of objects in a leaf
	 */
	...
	/**
	 * @param son one of
	 * {@link QTree#LOWER_LEFT},{@link QTree#LOWER_RIGHT},{@link QTree#UP_LEFT},{@link QTree#UP_RIGHT}
	...
	/**
	 * @param envelope
	 * @param min index of the intersecting node the min point
	...
	/**
	 * @param env to get the leafObjects for.
	 * @return the leafObjects which intersect with this node and or it's children, or the
	 * empty list.
	 */
	@Override
