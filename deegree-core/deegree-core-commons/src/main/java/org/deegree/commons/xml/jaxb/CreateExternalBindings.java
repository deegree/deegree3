...
/**
 * The <code>CreateExternalBindings</code> is a little utitly class, which writes a jaxb
 * binding file by reading java files from a 'configuration' package and create a
...
public class CreateExternalBindings {
	...
	private void writeBindings(List<StringPair> classes, XMLStreamWriter writer, String referencedXSD, String namespace,
			String packageName) throws XMLStreamException {
		...
		for (StringPair cl : classes) {
			if (cl != null) {
				...
				if (index != -1) {
					...
					// If the class name doesn't have a Type in its name, it is probably a
					// schema element... *g*
	...
	private List<StringPair> findClasses(File sourceDir, String prefix) throws IOException {
	...
	private String matchSCDType(String className, String file) throws IOException {
		String type = "type";
	...
	/**
	 * @param args the arguments to the bindings creator, args[0] sourceDir, args[1]
	 * destination file, [Optional args[2]=namespace of configuration file,
	 * args[3]=relative link to target schema file].
	...
	public static void main(String[] args) throws XMLStreamException, IOException {
		...
		if (args != null) {
			...
			switch (args.length) {
				...
				case 1:
					System.out.println(
							"Not enough parameters given, please supply a source directory (param 1) and a target file( param 2).");
					break;
				case 2:
