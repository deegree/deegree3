...
public class BufferSerializer {
	...
	/**
	 * Writes a buffer to the given output stream by checking the instance of the buffer
	 * and writing the native values appropriately. Because of the ObjectOutputStream is
	 * not seekable, the serialized buffer will be corrupted if an {@link IOException}
	 * occurs. The 'size' integer will in this case not be set to -1 and will not contain
	 * the correct number of 'native-types' to read.
	 * @param buffer
	 * @param out
	 * @throws IOException
	 */
	public static void writeBufferToStream(Buffer buffer, DataOutputStream out) throws IOException {
		...
		try {
			...
			LOG.error(
					"An exception occurred while writing the buffer, the output will be corrupted and should be re-serialized.");
			throw ie;
		}
	}
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(DoubleBuffer buffer, DataOutputStream out) throws IOException {
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(IntBuffer buffer, DataOutputStream out) throws IOException {
		for (int i = 0; i < buffer.capacity(); ++i) {
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(LongBuffer buffer, DataOutputStream out) throws IOException {
		for (int i = 0; i < buffer.capacity(); ++i) {
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(CharBuffer buffer, DataOutputStream out) throws IOException {
		for (int i = 0; i < buffer.capacity(); ++i) {
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(FloatBuffer buffer, DataOutputStream out) throws IOException {
		for (int i = 0; i < buffer.capacity(); ++i) {
	 ...
	 * @throws IOException
	 */
	private static void writeBuffer(ByteBuffer buffer, DataOutputStream out) throws IOException {
		for (int i = 0; i < buffer.capacity(); ++i) {
	...
	/**
	 * Reads a number of floats from the stream and fills a direct, native FloatBuffer
	 * with them.
	 * <p>
	 * The callee has to ensure that this method is called in it's right time, so that the
	 * next read to the stream gives an int and the read(s) thereafter will result in the
	 * number of values denoted by the first int value. Using this class will result in a
	 * correct order. The following code will result in correct reading and writing of the
	 * buffers:<code>
	 *
	 ...
	 * @param in
	 * @param direct
	 * @return the floatBuffer of <code>null</code> if no indication of the number of
	 * floats to read has been made.
	 * @throws IOException
	 */
	...
	/**
	 * Reads a number of bytes from the stream and fills a direct, native ByteBuffer with
	 * them.
	 * <p>
	 * The callee has to ensure that this method is called in it's right time, so that the
	 * next read to the stream gives an int and the read(s) thereafter will result in the
