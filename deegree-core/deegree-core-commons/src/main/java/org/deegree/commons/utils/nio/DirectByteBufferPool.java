...
/**
 * The <code>DirectByteBufferPool</code> pools a number of direct 'native' bytebuffers so
 * they can be reused.
 *
...
public class DirectByteBufferPool {
	...
	// private final SortedMap<Integer, Set<PooledByteBuffer>> freeBuffers = new
	// TreeMap<Integer,
	 ...
	 * @param capacityLimit total capacity of this pool.
	 * @param name
	 */
	 ...
	 * @throws OutOfMemoryError if no PooledByteBuffer with the given capacity is
	 * available and allocating it from the system would exceed the assigned resources
	 */
	public synchronized PooledByteBuffer allocate(int capacity) throws OutOfMemoryError {
	 ...
	 * available and allocating it from the system would exceed the assigned resources
	 */
	public synchronized PooledByteBuffer allocate_(int capacity) throws OutOfMemoryError {
	...
	public synchronized void deallocate(PooledByteBuffer buffer) {
		if (buffer != null) {
			...
			if (!allBuffers.contains(buffer)) {
				...
				// System.out.println( name + ":deallocate: freebuffer capas after: " +
				// freeBuffers.size()
				// + ", number of freebuffers for capa: " + capacity + ", "
				// + freeBuffers.get( capacity ).size() );
				buffers.add(buffer);
				// System.out.println( name + ":deallocate: freebuffer capas before: " +
				// freeBuffers.size()
				// + ", number of freebuffers for capa: " + capacity + ", "
				// + freeBuffers.get( capacity ).size() );
