...
/**
 * The <code>SpatialIndex</code> defines basic methods for the adding, removing and
 * querying of a spatial index.
 *
...
public abstract class SpatialIndex<T> {
	...
	/**
	 * Query the spatial index with the given envelope and return all objects which
	 * intersect with the given boundingbox.
	 * @param envelope to intersect
	 ...
	 * @param listOfObjects to be inserted into the spatial index.
	 */
	...
	/**
	 * Test if two envelopes intersect, bbox must be defined as
	 * float[4]=min[0],min[1];max[0],max[1]
	 * @param box1 the first envelope
	 * @param box2 the second envelope
	 * @param maxOffset the offset within the bbox where the max point starts.
