 ...
 * <ul>
 * <li>Perform a task asynchronously (in an independent thread) optionally with a maximum
 * execution time.</li>
 * <li>Perform a task synchronously with a maximum execution time.</li>
 * <li>Perform several task synchronously (but in parallel threads) with a maximum
 * execution time.</li>
 * </ul>
 * <p>
 * The <code>Executor</code> class is realized as a singleton and uses a cached thread
 * pool internally to minimize overhead for acquiring the necessary {@link Thread}
...
public class Executor {
	 ...
	 * @param task task to be performed (specified in the {@link Callable#call()} method)
	 * @param finishedListener notified when the method call finishes (succesfully or
	 * abnormally), may be null
	 */
	 ...
	 * @param <T> type of return value
	 * @param task tasks to be performed (specified in the {@link Callable#call()} method)
	 * @param timeout maximum time allowed for execution in milliseconds
	 * @return result value of the called method
	 * @throws CancellationException if the execution time exceeds the specified timeout /
	 * thread has been cancelled
	 * @throws InterruptedException if interrupted while waiting, in which case unfinished
	 * tasks are cancelled
	 * @throws Throwable if the tasks throws an exception itself
	 */
	 ...
	 * <p>
	 * This method does not return before all tasks are finished (successfully or
	 * abnormally). For each given {@link Callable}, an independent thread is used. For
	 * each task, an {@link ExecutionFinishedEvent} is generated and returned.
	 * @param <T> the result type of the callables
	 * @param tasks tasks to be performed (specified in the {@link Callable#call()}
	 * methods)
	 ...
	 * @throws InterruptedException if the current thread was interrupted while waiting
	 */
	public <T> List<ExecutionFinishedEvent<T>> performSynchronously(List<Callable<T>> tasks, long timeout)
			throws InterruptedException {
