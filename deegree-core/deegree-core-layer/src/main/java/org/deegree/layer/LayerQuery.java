...
public class LayerQuery {
	...
	public LayerQuery(Envelope envelope, int width, int height, int x, int y, int featureCount, OperatorFilter filter,
			StyleRef style, Map<String, String> parameters, Map<String, List<?>> dimensions, MapOptionsMaps options,
	...
	public Envelope calcClickBox(int radius) {
		...
		int r2 = radius / 2;
		r2 = r2 <= 0 ? 1 : r2;
		return fac.createEnvelope(
				new double[] { envelope.getMin().get0() + (x - r2) * dw, envelope.getMax().get1() - (y + r2) * dh },
				new double[] { envelope.getMin().get0() + (x + r2) * dw, envelope.getMax().get1() - (y - r2) * dh },
	 ...
	 * @return the property (first) and the values (second) to filter for,
	 * <code>null</code> if at least one parameter is <code>null</code> or empty
	 */
