...
public class TextStroke implements Stroke {
	...
	private GeneralPath tryWordWise(Shape shape, double initialGap) {
		...
		// algorithm sketch:
		// try to match as many words on each line segment as possible (taking into
		// account gap length)
		// if a segment is encountered that is smaller than the current word length,
		// return false
		// (even if maybe the next segment would be fitting, the empty segment would just
		// not look nice, so better abort
		// here)
		// the list will contain StringOrGap objects (a choice between a String, a gap or
		// a end of line tag)
		// So if some gap[1]/word[1-*] combination fits the segment, all of them are added
		// to the list, as well as an
		// end of line tag. Then, the next segment is considered. If repeat is on, the
		// words list will never be empty
	...
	private boolean prepareWordsToRender(LinkedList<String> words, LinkedList<StringOrGap> wordsToRender, Shape shape,
		...
		while (words.size() > 0 && lengths.size() > 0) {
			...
			double segLength = lengths.poll() - font.getSize2D(); // at least it works for
																	// line angles < 90°
			...
			// the next segment will be considered, so add a end of line tag. If the last
			// StringOrGap was a gap, insert
	...
	private void newWords(LinkedList<String> words, LinkedList<String> wordsCopy, String word,
		...
		do { // do/while because at least one word has to be added
			...
			if (words.isEmpty() && linePlacement.repeat) {
				// in this case, the words list has to be filled again. Also the gap has
				// to be considered.
				...
				sog.string = newWord;
				wordsToRender.add(sog);
				justInserted = true; // set this flag so the adding at the end of the loop
										// won't happen again with
				// the same string.
			...
			if (!justInserted && (totalLength + vecLength >= segLength)) {
				// the normal case, one or more words have been fit, and the current word
				// does overflow the length
				// -> add the word
				StringOrGap sog = new StringOrGap();
				sog.string = word;
				wordsToRender.add(sog);
	...
	private void nextSegment(CharacterWiseState state, int length, double distance, GlyphVector glyphVector,
			double lastX, double lastY, double dx, double dy, double r, double angle, GeneralPath result) {
		while (state.currentChar < length && distance >= state.next) {
			...
			state.nextAdvance = state.currentChar < length - 1
					? glyphVector.getGlyphMetrics(state.currentChar + 1).getAdvance() * 0.5f : advance;
			AffineTransform t = new AffineTransform();
