...
/**
 * A path iterator which iterates over a path in the reverse direction. This is missing in
 * the java.awt.geom package, although it's quite simple to implement. After
 * initialization the original PathIterator is not used any longer.
 * <p>
 * There are several static convenience methods to create a reverse path iterator from a
 * shape directly:
 * <ul>
 * <li>{@link #getReversePathIterator(java.awt.Shape)} for reversing the standard path
 * iterator</li>
 * <li>{@link #getReversePathIterator(java.awt.Shape, double)} for reversing a flattened
 * path iterator</li>
 * <li>{@link #getReversePathIterator(java.awt.Shape, java.awt.geom.AffineTransform)} for
 * reversing a transformed path iterator</li>
 ...
 * <li>{@link #getReversePathIterator(java.awt.Shape, int)} for reversing the standard
 * path iterator while explicitely defining a winding rule</li>
 * <li>{@link #getReversePathIterator(java.awt.Shape, double, int)} for reversing a
 * flattened path iterator while explicitely defining a winding rule</li>
 * <li>{@link #getReversePathIterator(java.awt.Shape, java.awt.geom.AffineTransform, int)}
 * for reversing a transformed path iterator while explicitely defining a winding
 * rule</li>
 * <li>{@link #getReversePathIterator(java.awt.Shape, java.awt.geom.AffineTransform, double, int)}
 * for reversing a transformed flattened path iterator while explicitely defining a
 * winding rule</li>
 * </ul>
 *
...
public class ReversePathIterator implements PathIterator {
	...
	/**
	 * Get a reverse flattened path iterator for a shape, keeping the shape's winding
	 * rule.
	...
	/**
	 * Get a reverse transformed path iterator for a shape, keeping the shape's winding
	 * rule.
	...
	/**
	 * Get a reverse transformed flattened path iterator for a shape, keeping the shape's
	 * winding rule.
	 * @param shape shape for which a reverse transformed flattened path iterator is
	 * needed
	 * @param flatness flatness epsilon
	...
	/**
	 * Returns the winding rule for determining the interior of the path. This just
	 * returns the winding rule of the original path, which may or may not be what is
	 * wanted.
	 ...
	 * @return <code>true</code> if all the segments have been read; <code>false</code>
	 * otherwise.
	 */
	...
	/**
	 * Returns the coordinates and type of the current path segment in the iteration. The
	 * return value is the path-segment type: SEG_MOVETO, SEG_LINETO, SEG_QUADTO,
	 * SEG_CUBICTO, or SEG_CLOSE. A double array of length 6 must be passed in and can be
	 * used to store the coordinates of the point(s). Each point is stored as a pair of
	 * double x,y coordinates. SEG_MOVETO and SEG_LINETO types returns one point,
	 * SEG_QUADTO returns two points, SEG_CUBICTO returns 3 points and SEG_CLOSE does not
	 * return any points.
	 ...
	 * Returns the coordinates and type of the current path segment in the iteration. The
	 * return value is the path-segment type: SEG_MOVETO, SEG_LINETO, SEG_QUADTO,
	 * SEG_CUBICTO, or SEG_CLOSE. A float array of length 6 must be passed in and can be
	 * used to store the coordinates of the point(s). Each point is stored as a pair of
	 * float x,y coordinates. SEG_MOVETO and SEG_LINETO types returns one point,
	 * SEG_QUADTO returns two points, SEG_CUBICTO returns 3 points and SEG_CLOSE does not
	 * return any points.
	 * @param coords an array that holds the data returned from this method
	 * @return the path-segment type of the current path segment.
