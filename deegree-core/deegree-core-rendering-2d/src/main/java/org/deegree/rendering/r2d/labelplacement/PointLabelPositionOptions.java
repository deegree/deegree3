...
public class PointLabelPositionOptions {
	...
	/**
	 * Some constants, describing the 8 possible positions of the label and their
	 * qualities.
	 */
	 ...
	 * @param label The label to autoposition
	 * @param uomCalculator The {@link UomCalculator} from the RenderContext, used to
	 * calculate the correct displacement
	 */
	 ...
	 * @param index position index for the requested quality
	 *
	 */
	 ...
	 * @param selected A value between 0 and 7
	 */
	...
	/**
	 * Tests the currently selected label position for intersection with the current
	 * selection of another label position. If both labels are not rotated, an efficient
	 * AABB test is used, otherwise a polygon intersection is done
	 * @param labelPosOption Another {@link PointLabelPositionOptions} to intersect with
	 *
	 */
	boolean intersectsSelection(PointLabelPositionOptions labelPosOption) {
		...
		if (label.getStyling().rotation != 0 || labelPosOption.label.getStyling().rotation != 0) {
			...
			return ((selMinX < labelPosOption.getSelectedMaxX()) && (selMaxX > labelPosOption.getSelectedMinX())
					&& (selMinY < labelPosOption.getSelectedMaxY()) && (selMaxY > labelPosOption.getSelectedMinY()));
	}

	/**
	 * Tests if the bounding rectangle of all possible positions intersects with the
	 * bounding rectangle of another PointLabelPositionOptions-object If both labels are
	 * not rotated, an efficient AABB test is used, otherwise a polygon intersection is
	 * done
	 * @param labelPosOption Another {@link PointLabelPositionOptions} to intersect with
	 *
	 */
	boolean intersectsAny(PointLabelPositionOptions labelPosOption) {
		...
		if (label.getStyling().rotation != 0 || labelPosOption.label.getStyling().rotation != 0) {
			...
			return ((totalMinX < labelPosOption.getMaxX()) && (totalMaxX > labelPosOption.getMinX())
					&& (totalMinY < labelPosOption.getMaxY()) && (totalMaxY > labelPosOption.getMinY()));
	}
	...
	private Polygon getPolygon(double minX, double minY, double maxX, double maxY) {
		...
		// rotating the points if necessary. This is much faster than using JTS-methods
