...
class GeometryClipper {
	...
	/**
	 * Clips the passed geometry with the drawing area if the drawing area does not
	 * contain the passed geometry completely.
	 * @param geom the geometry to clip, must not be <code>null</code> and in the same CRS
	 * as the clipping area
	 * @return the clipped geometry or the original geometry if the geometry lays
	 * completely in the drawing area.
	 */
	...
	/**
	 * Calculates the points inside the geometry and inside the view port. First the
	 * passed geometry is clipped by the view port. A multipolygon may result. For each of
	 * the polygon in this multipolygon one interior point is created
	 * @param geom to create labels for, must not be <code>null</code> and in the same CRS
	 * as the viewPort
	...
	Geometry clipGeometry(final Geometry geom, Geometry clippingArea) {
		if (clippingArea != null && !clippingArea.contains(geom)) {
			try {
				...
				org.locationtech.jts.geom.Geometry jtsClipped = ((AbstractDefaultGeometry) clippedGeometry)
					.getJTSGeometry();
	...
	/**
	 * Check if the passed Geometry is a Polygon (or the first Geometry of a Collection)
	 * and the exterior Ring has CW orientation
	 *
	 * This helper is only a workaround to render polygons in CW/CCW order in the same
	 * manner clipped as unclipped
	 *
	 * TODO This should be resolved in a more universal way, see
	 * https://github.com/deegree/deegree3/issues/645
	 * @param jtsGeom JTS Geometry to be evaluated
	 * @return boolean true if (first) Geometry is Polygon with CW external ring, false
	 * otherwise
	 */
