...
public class Java2DRendererTest extends AbstractSimilarityTest {
	 ...
	 */
	@Test
	public void testPointStyling() throws Exception {
		...
		Java2DRenderer r = new Java2DRenderer(g, img.getWidth(), img.getHeight(),
				geomFac.createEnvelope(new double[] { 0, 0 }, new double[] { 5000d, 5000d }, mapcs));
		...
		texts.add(
				"third line: 32 pixel default square, inside a 16 pixel default square (upper left corner), two times");
		...
		texts.add(
				"third line: 32 pixel default square, outside a 16 pixel square (upper center at lower right corner of the large square) and a 16 pixel rotated square (lower right corner)");
	 ...
	 */
	@Test
	public void testLineStyling() throws Exception {
		BufferedImage img = new BufferedImage(1000, 1000, TYPE_INT_ARGB);
		long time = currentTimeMillis();
		Graphics2D g = img.createGraphics();
		GeometryFactory geomFac = new GeometryFactory();
	 ...
	 */
	@Test
	public void testLineStyling2() throws Exception {
		BufferedImage img = new BufferedImage(1000, 1000, TYPE_INT_ARGB);
		long time = currentTimeMillis();
		Graphics2D g = img.createGraphics();
		GeometryFactory geomFac = new GeometryFactory();
		...
		texts.add(
				"line 2: graphic filled lines with line width 0, 1, ..., 9, dashed with pattern 15, 15, 17, 5, ending butt");
		texts.add(
				"line 3: default style lines with line width 0, 1, ..., 9, dashed with pattern 15, 15, 17, 5, ending butt");
	 ...
	 */
	@Test
	public void testPolygonStyling() throws Exception {
		BufferedImage img = new BufferedImage(1000, 1000, TYPE_INT_ARGB);
	 ...
	 */
	@Test
	public void testTextStyling() throws Exception {
		BufferedImage img = new BufferedImage(1000, 1000, TYPE_INT_ARGB);
	 ...
	 */
	@Test
	public void testTextStyling2() throws Exception {
		BufferedImage img = new BufferedImage(1000, 1000, TYPE_INT_ARGB);
		...
		List<String> texts = new LinkedList<String>();
		texts.add(
				"Note: this text may not produce good tests every time (it depends on the length of the random line strings).");
	...
	@Test
	public void testTextStylingHalo() throws Exception {
	...
	@Test
	public void testTextStylingLabelHalo() throws Exception {
		BufferedImage img = new BufferedImage(200, 200, TYPE_INT_ARGB);
	...
	public void testPolygonStylingSmallClipping() throws Exception {
		...
		GeometryFactory geomFac = new GeometryFactory();
		Java2DRenderer r = new Java2DRenderer(g, img.getWidth(), img.getHeight(),
				geomFac.createEnvelope(new double[] { 0, 0 }, new double[] { 50d, 50d }, mapcs));
	...
	public void testPolygonStylingWithCirclesSmallClipping() throws Exception {
		...
		GeometryFactory geomFac = new GeometryFactory();
		Java2DRenderer r = new Java2DRenderer(g, img.getWidth(), img.getHeight(),
				geomFac.createEnvelope(new double[] { 0, 0 }, new double[] { 600d, 600d }, mapcs));
	...
	public void testPolygonStylingWithStrokeWithoutSize() throws Exception {
		...
		GeometryFactory geomFac = new GeometryFactory();
		Java2DRenderer r = new Java2DRenderer(g, img.getWidth(), img.getHeight(),
				geomFac.createEnvelope(new double[] { 0, 0 }, new double[] { 100d, 100d }, mapcs));
	...
	@Test
	public void testPolygonStylingPerpendicularOffset() throws Exception {
		BufferedImage img = new BufferedImage(100, 100, TYPE_INT_ARGB);
		long time = currentTimeMillis();
		Graphics2D g = img.createGraphics();
		GeometryFactory geomFac = new GeometryFactory();
		...
		List<String> texts = new LinkedList<String>();
		texts.add(
				"polygon: white rectangle with red triangle stroke and perpendicular offest of -4. Expected: triangles points to the INSIDE of the geometry!");
	 ...
	 * if one of the coordinates of a geometry has an invalid value (which is out of
	 * bounds) and the clipper does not clip the geometry
	 *
