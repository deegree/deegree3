...
/**
 * Encapsulates the properties of a process offered by a WPS instance (id, title,
 * abstract, input parameter types, output parameter types, etc.) and provides access to a
 * {@link ProcessExecution} context for executing it.
 *
 * @see WPSClient
...
public class Process implements Serializable {
	 ...
	 * @param client associated client instance, must not be <code>null</code>
	 * @param processInfo brief process info, must not be <code>null</code>
	 */
	 ...
	 * @return the abstract, can be <code>null</code> (if the process description does not
	 * define an abstract)
	 */
	 ...
	 * @return the descriptions for all input parameters, can be empty, but never
	 * <code>null</code>
	 ...
	 */
	public InputType[] getInputTypes() throws OWSExceptionReport, IOException {
	 ...
	 * @return the descriptions for all output parameters, can be empty, but never
	 * <code>null</code>
	 * @throws OWSExceptionReport if the server replied with an exception
	 * @throws IOException if a communication/network problem occured
	 */
	public OutputType[] getOutputTypes() throws OWSExceptionReport, IOException {
		List<OutputType> outputs = getProcessDetails().getOutputs();
		return outputs.toArray(new OutputType[outputs.size()]);
	}

	/**
	 * Returns whether the process supports storing of the response document
	 * (=asynchronous execution).
	 * @return true, if the process supports storing the response document, false
	 * otherwise
	 * @throws OWSExceptionReport if the server replied with an exception
	 * @throws IOException if a communication/network problem occured
	 */
	public boolean getStoreSupported() throws OWSExceptionReport, IOException {
		return getProcessDetails().getStoreSupported();
	}

	/**
	 * Returns whether the process supports polling of status information during
	 * asynchronous execution.
	 * @return true, if the process supports polling status information, false otherwise
	 * @throws OWSExceptionReport if the server replied with an exception
	 * @throws IOException if a communication/network problem occured
	 */
	public boolean getStatusSupported() throws OWSExceptionReport, IOException {
	 ...
	 * <p>
	 * This method will request the response using the WPS <code>ResponseDocument</code>
	 * mode. If you're unsure, this is most probably what you want, as it is the most
	 * flexible way of executing WPS processes.
	 * </p>
	...
	/**
	 * Prepares a new {@link RawProcessExecution} context that allows to execute the
	 * process using the WPS <code>RawOutput</code> mode.
	 * <p>
	 * If you're unsure, then you most probably want to use {@link #prepareExecution()},
	 * as the <code>RawOutput</code> mode is rather restricted: just a single (complex)
	 * output, no asynchronous execution, no storing of output.
	 * </p>
	 * @return new process execution context, never <code>null</code>
	 */
	 ...
	 * @throws IOException if a communication/network problem occured
	 */
	private ProcessDetails getProcessDetails() throws OWSExceptionReport, IOException {
		...
		if (processDetails == null) {
			...
			String finalURLStr = url.toExternalForm() + "?service=WPS&version="
					+ URLEncoder.encode(client.getServiceVersion(), "UTF-8") + "&request=DescribeProcess&identifier="
