...
/**
 * API-level client for accessing services that implement the
 * <a href="http://www.opengeospatial.org/standards/wps">WebProcessingService (WPS)
 * 1.0.0</a> protocol.
 *
 * <h4>Initialization</h4> In the initial step, one constructs a new {@link WPSClient}
 * instance by invoking the constructor with a URL to a WPS capabilities document. In most
 * cases, this will be a GetCapabilities request (including necessary parameters) to a WPS
 * service.
 *
 * <pre>
 * ...
 *   URL capabilitiesUrl = new URL( "http://...?service=WPS&version=1.0.0&request=GetCapabilities" );
 *   WPSClient wpsClient = new WPSClient( capabilitiesUrl );
 * ...
 * </pre>
 *
 * Afterwards, the {@link WPSClient} instance is bound to the specified service and allows
 * to access announced service metadata, process information as well as the execution of
 * processes.
 *
 * <h4>Accessing service metadata</h4> The method {@link #getMetadata()} allows to access
 * the metadata announced by the service, such as title, abstract, provider etc.
 *
 * <h4>Getting process information</h4> The method {@link #getProcesses()} allows to find
 * out about the processes offered by the service. Additionally (if one knows the
 * identifier of a process beforehand), one can use {@link #getProcess(String)} or
 * {@link #getProcess(String, String)} to retrieve a specific process. The {@link Process}
 * class allows to execute a process and offers methods to access detail information such
 * as title, abstract, input parameter types and output parameters types:
 *
 * <pre>
 * ...
 ...
 * </pre>
 *
 * <h4>Executing a process</h4> When executing a request, the method
 * {@link Process#prepareExecution()} must be used to create a {@link ProcessExecution}
 ...
 * </pre>
 *
 * <h4>Providing input</h4>Input is straightforwardly specified for the <b>literal and
 * bbox cases</b>: <br/>
 * <br/>
 * {@link ProcessExecution#addLiteralInput(String, String, String, String, String)}
 * respectively <br/>
 * <br/>
 * {@link ProcessExecution#addBBoxInput(String, String, double[], double[], String)}
 *
 * <pre>
 * ...
 *   execution.addLiteralInput( &quot;BufferDistance&quot;, null, &quot;0.1&quot;, &quot;double&quot;, &quot;unity&quot; );
 ...
 * </pre>
 *
 * For complex data however, the are methods that provide <b>XML and binary data
 * separately</b>. One can provide the <b>input as reference</b> by giving an URL and
 * whether it is external (using http, ftp, etc. protocols) or local (file protocol).
 * <br/>
 * {@link ProcessExecution#addXMLInput(String, String, URL, boolean, String, String, String)}
 * and respectively <br/>
 * <br/>
 * {@link ProcessExecution#addBinaryInput(String, String, URL, boolean, String, String)}
 *
 * <pre>
 * ...
 ...
 * </pre>
 *
 * Alternatively, one can simply provide the <b>input inline as stream</b>. In the case of
 * XML it will be a {@link XMLStreamReader}, while for the binary data expected will be an
 * {@link InputStream}.<br/>
 * <br/>
 * {@link ProcessExecution#addXMLInput(String, String, XMLStreamReader, String, String, String)}
 * respectively <br/>
 * <br/>
 * {@link ProcessExecution#addBinaryInput(String, String, java.io.InputStream, String, String)}
 *
 * <pre>
 * ...
 ...
 * </pre>
 *
 * <h4>Controlling output</h4>By omitting to set the outputs, the process will generate
 * the default ones, as specified in the process definition. However, the user can take
 * control on precisely what outputs he/she wants by using
 * {@link ProcessExecution#addOutput(String, String, String, boolean, String, String, String)}
 * and specifying the id of the respective wanted output. Among the parameters there is
 * the possibility of specifying to <b>retrieve the output as reference or inline</b>.
 *
 * <pre>
 * ...
 ...
 * </pre>
 *
 * <h4>Executing a process: raw output mode</h4> There is also the possibility of
 * selecting the WPS <code>RawOutput</code> mode, where the (single) output parameter is
 * not going to be wrapped in an ExecuteResponse XML document, but directly returned as a
 * resource. For this one must set {@link Process#prepareRawExecution()} to retrieve a
 * suitable execution context. The server shall respond with this sole output resource.
 *
 * <pre>
 * ...
 *   Process buffer = wpsClient.getProcess ("Buffer");
 ...
 * </pre>
 *
 * <h4>Executing a process: asynchronous mode</h4> Instead of using
 * {@link ProcessExecution#execute()}, one can also request asynchronous execution via the
 * {@link ProcessExecution#executeAsync()} method. In the latter case, the call will
 * return immediately, but the result is not necessarily available yet. In order to check
 * for completion, the {@link ProcessExecution#getState()} is available, that will poll
 * the server for the current status (if it wasn't finished anyway). Additionally, the
 * method {@link ProcessExecution#getPercentCompleted()} is available to check the
 ...
 * <li>The implementation is thread-safe, a single {@link WPSClient} instance can be
 * shared among multiple threads.</li>
 * </ul>
 *
 * <h4>TODOs</h4>
 * <ul>
 * <li>Implement input parameter passing for POST-references.</li>
 * <li>Cope with exceptions reports that are returned for <code>GetCapabilities</code> and
 * <code>DescribeProcess</code> requests.</li>
 * <li>Determine the correct XML encoding for HTTP-requests/responses, see
 * http://www.ietf.org/rfc/rfc2376.txt</li>
 * <li>Clean up exception handling.</li>
 * <li>Enable/document a way to set connection parameters (timeout, proxy settings,
 * ...)</li>
 * <li>Support metadata in multiple languages (as mandated by the WPS spec).</li>
 * <li>Check validity (cardinality, order) of input and output parameters in
 * {@link ProcessExecution}.</li>
 * </ul>
 *
 * @see Process
...
public class WPSClient {
	 ...
	 * @param capabilitiesURL url of a WPS capabilities document, usually this is a
	 * GetCapabilities request to a WPS service, must not be <code>null</code>
	...
	private WPS100CapabilitiesAdapter retrieveCapabilities(URL capabilitiesURL) throws IOException {
	 ...
	 * @return all processes offered by the service, may be empty, but never
	 * <code>null</code>
	 */
	 ...
	 * <p>
	 * NOTE: This is a convenience method that ignores the optional codespace that a
	 * process identifier may have. If a server actually offers two processes with the
	 * same identifier, but different codespace, let's say 'Buffer' (codespace:
	 * 'Sextante') and 'Buffer' (codespace: 'GRASS'), then it's not defined which of the
	 * ones will be returned, when this method is called with parameter 'Buffer'. To be on
	 * the safe side, use {@link #getProcess(String, String)}.
	 * </p>
	 * @param id process identifier, never <code>null</code>
	 * @return process instance, can be <code>null</code> (if no process with the
	 * specified identifier and code space is offered by the services)
	 */
	 ...
	 * @param id process identifier, never <code>null</code>
	 * @param idCodeSpace codespace of the process identifier, may be <code>null</code>
	...
	/**
	 * Returns the URL announced by the service for issuing <code>DescribeProcess</code>
	 * requests.
	 * @param post if set to true, the URL for POST requests will be returned, otherwise
	 * the URL for GET requests will be returned
	 * @return the <code>DescribeProcess</code> URL, may be <code>null</code> (if the
	 * server doesn't provide a binding for the specified request method)
	 */
	 ...
	 * @param post if set to true, the URL for POST requests will be returned, otherwise
	 * the URL for GET requests will be returned
	 * @return the <code>Execute</code> URL, may be <code>null</code> (if the server
	 * doesn't provide a binding for the specified request method)
	 */
