 ...
 * Catalogue Services Specification (CSW) 2.0.2</a> protocol.
 *
 * <h4>Initialization</h4> In the initial step, one constructs a new {@link CSWClient}
 * instance by invoking the constructor with a URL to a CSW capabilities document. This
 ...
 * </pre>
 *
 * Afterwards, the initialized {@link CSWClient} instance is bound to the specified
 * service and CSW protocol version. Now, it's possible to access records and perform
...
public class CSWClient extends AbstractOWSClient<CSWCapabilitiesAdapter> {
	 ...
	 * @param capaUrl url of a CSW capabilities document, usually this is a
	 ...
	 * @param capaUrl url of a CSW capabilities document, usually this is a
	 * <code>GetCapabilities</code> request to the service, must not be <code>null</code>
	 * @param connectionTimeout the timeout for get/post requests in milliseconds, 0 is
	 * interpreted as an infinite timeout (default)
	 * @throws OWSExceptionReport if the server replied with a service exception report
	 * @throws XMLStreamException
	 * @throws IOException if a communication/network problem occured
	 */
	public CSWClient(URL capaUrl, int connectionTimeout, int readTimeout)
	...
	@Override
	protected CSWCapabilitiesAdapter getCapabilitiesAdapter(OMElement rootEl, String version) throws IOException {
	...
	public GetRecordsResponse getIsoRecords(ResultType resultType, ReturnableElement elementSetName, Filter constraint)
		...
		GetRecords getRecords = new GetRecords(new Version(2, 0, 2), 10, 15, "application/xml",
				"http://www.isotc211.org/2005/gmd",
				Collections.singletonList(
						new QName(CommonNamespaces.ISOAP10GMDNS, "MD_Metadata", CommonNamespaces.ISOAP10GMD_PREFIX)),
	...
	public GetRecordsResponse getIsoRecords(int startPosition, int maxRecords, ResultType resultType,
			...
			throws IOException, XMLProcessingException, OWSExceptionReport, XMLStreamException {
		GetRecords getRecords = new GetRecords(new Version(2, 0, 2), startPosition, maxRecords, "application/xml",
				"http://www.isotc211.org/2005/gmd",
				Collections.singletonList(
						new QName(CommonNamespaces.ISOAP10GMDNS, "MD_Metadata", CommonNamespaces.ISOAP10GMD_PREFIX)),
				ResultType.results, ReturnableElement.full, null);
	...
	public GetRecordsResponse getIsoRecords(int startPosition, int maxRecords, Filter constraint)
		...
		GetRecords getRecords = new GetRecords(new Version(2, 0, 2), startPosition, maxRecords, "application/xml",
				...
				Collections.singletonList(
						new QName(CommonNamespaces.ISOAP10GMDNS, "MD_Metadata", CommonNamespaces.ISOAP10GMD_PREFIX)),
				ResultType.results, ReturnableElement.full, constraint);
		return this.getRecords(getRecords);
	}
	...
	private void moveToNextStartElement(XMLStreamReader xmlStream) throws XMLStreamException {
		xmlStream.next();
	...
	private void ckeckOperationSupported(String operationName) throws UnsupportedOperationException {
	...
	/**
	 * Cope with <code>OperationMetadata</code> sections that specify separate SOAP and
	 * XML endpoints.
	 *
	 * <pre>
