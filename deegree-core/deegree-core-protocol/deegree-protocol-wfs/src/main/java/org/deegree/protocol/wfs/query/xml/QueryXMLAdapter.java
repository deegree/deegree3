...
public class QueryXMLAdapter extends AbstractWFSRequestXMLAdapter {
	...
	/**
	 * Parses a <code>fes:AbstractQueryExpression</code> element (in the context of a WFS
	 * 2.0.0 XML request).
	 * @param queryEl element substitutable for <code>fes:AbstractQueryExpression</code>,
	 * must not be <code>null</code>
	 * @return parsed {@link Query}, never <code>null</code>
	 * @throws OWSException
	 */
	public Query parseAbstractQuery200(OMElement queryEl) throws OWSException {
	...
	private Query parseAdHocQuery200(OMElement queryEl) throws OWSException {
		...
		// <xsd:element ref="fes:AbstractProjectionClause" minOccurs="0"
		// maxOccurs="unbounded"/>
		...
		// <xsd:element name="SortBy" type="fes:SortByType"
	...
	private Query parseDynamicFeatureQueryTe100(OMElement queryEl) throws OWSException {
		...
		List<OMElement> timeSliceProjectionEls = getElements(queryEl,
				new XPath("fes-te:TimeSliceProjection", nsContext));
	...
	private TimeSliceProjection parseTimeSliceProjectionTe100(OMElement timeSliceProjectionEl) {
		...
		// OMElement relevantTimeEl = getElement( timeSliceProjectionEl, new XPath(
		// "fes-te:relevantTime", nsContext )
		// );

		Filter filter = null;
		OMElement timeSliceFilterEl = getElement(timeSliceProjectionEl, new XPath("fes-te:timeSliceFilter", nsContext));
	...
	private StoredQuery parseStoredQuery200(OMElement queryEl) {
		...
		// <xsd:element name="Parameter" type="wfs:ParameterType" minOccurs="0"
		// maxOccurs="unbounded"/>
