...
public class GetFeatureInfo extends RequestBase {
	...
	public GetFeatureInfo(List<LayerRef> layers, List<StyleRef> styles, List<String> queryLayers, int width, int height,
			int x, int y, Envelope envelope, ICRS crs, int featureCount, String infoFormat,
	...
	private double[] handleCommon(Map<String, String> map) throws OWSException {
		...
		this.layers = ls == null ? new LinkedList<LayerRef>()
				: new LinkedList<LayerRef>(map(ls.split(","), FROM_NAMES));
	...
	/**
	 * @return the live list of styles (this uses the query_layers parameter as source for
	 * layer names)
	 */
	 ...
	 */
	private void cleanUpLayers(List<String> qlayers) throws OWSException {
