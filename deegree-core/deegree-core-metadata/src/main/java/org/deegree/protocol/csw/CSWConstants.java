...
public final class CSWConstants {
	...
	/**
	 * Common namespace prefix for elements from the ISO AP specification for the types
	 * "Dataset", "DatasetCollection" and "Application"
	 */
	...
	/**
	 *
	 * Sections are informations about the service represented in the GetCapabilities
	 * operation <br>
	...
	public enum ResultType {
		...
		hits,
		/**
		 * returns one or more records from the result set up to the maximum number of
	...
	public enum ReturnableElement {
		...
		/**
		 * Brief representation of a record. This is the shortest view of a record by a
		 * specific profile.
		 */
		brief,

		/**
		 * Summary representation of a record. This view responses all the elements that
		 * should be queryable by a record-profile.
		 */
		summary,
		/**
		 * Full representation of a record. In that response there are all the elements
		 * represented that a record holds. Thus, there are elements presented that are
		 * not queryable regarding to the CSW specification.
		 */
		full;
	...
	/**
	 *
	 * Specifies in which filter mode the query has to be processed. Either there is a OGC
	 * XML filter encoding after the filterspecification document
	 * <a href="http://www.opengeospatial.org/standards/filter">OGC 04-095</a> or there is
	 * a common query language string (CqlText) which can be seen as an explicit typed
	...
	public enum TransactionType {
		...
		/**
		 * With the INSERT action of the transaction operation there can be inserted one
		 * or more records into the backend.
		 */
		INSERT,

		/**
		 * With the DELETE action of the transaction operation there can be deleted
		 * specific records defined by a filter expression.
		 */
		DELETE,

		/**
		 * With the UPDATE action of the transaction operation there can be updated one
		 * complete record or just properties of specific records defined by a filter
		 * expression.
		 */
		UPDATE
