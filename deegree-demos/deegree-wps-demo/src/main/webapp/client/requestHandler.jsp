...
<%
    ...
    if ( isXML ) {
        ...
        ( (PostMethod) http ).setRequestEntity( new StringRequestEntity( xml, "text/xml", "UTF-8" ) );
    } else {
        ...
        if ( isGET ) {
            ...
            http = new PostMethod( action );
            ( (PostMethod) http ).setRequestEntity( new StringRequestEntity( sb.toString(), "text/xml", "UTF-8" ) );
        }
    }
    ...
    try {
        ...
        if ( http.getStatusCode() == 401 ) {
            ...
            out.write( "" + http.getStatusCode() );
            out.write( "Http Authentication failed! Please provide valid username / password." );
            out.flush();
        }

        // HttpStatus check not needed, we want to see exceptionReports, etc
        //if ( http.getStatusCode() == HttpStatus.SC_OK ) {  
        else if ( http.getResponseHeader( "Content-Type" ) != null ) {
