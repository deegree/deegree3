...
public class InsertRowManager {
	 ...
	 * @param idGenMode feature id generation mode, must not be <code>null</code>
	 */
	 ...
	 * <p>
	 * Note that some or all of the corresponding table rows may actually not be inserted
	 * when this method returns. They may be delayed until their dependencies are
	 * inserted. This method also takes care of checking for rows that have been delayed
	 * and can be inserted now.
	 * </p>
	 * @param feature feature instance to be inserted, must not be <code>null</code>
	 * @param ftMapping mapping of the corresponding feature type, must not be
	 * <code>null</code>
	...
	private FeatureRow lookupFeatureRow(String fid) throws FeatureStoreException {
	...
	public void buildInsertRows(final TypedObjectNode particle, final Mapping mapping, final InsertRow row,
			List<InsertRow> additionalRows) throws FilterEvaluationException, FeatureStoreException {
		...
		for (TypedObjectNode value : values) {
			...
			if (mapping instanceof PrimitiveMapping) {
				...
				if (!(me instanceof DBField)) {
					...
					ParticleConverter<PrimitiveValue> converter = (ParticleConverter<PrimitiveValue>) fs
				...
				if (feature instanceof FeatureReference) {
					...
					if (!idGenMode.isSkipResolveReferences()
							&& (featureReference.isLocal() || featureReference.isResolved())
				...
				if (subFeatureRow != null) {
					...
					if (jc == null || jc.isEmpty()) {
						...
						if (!delayedRows.contains(subFeatureRow)) {
							// sub feature already inserted, propagate key values right
							// away
	...
	private KeyPropagation getKeyPropagation(FeatureMapping mapping, TableJoin join) throws FeatureStoreException {
		...
		if (mapping.getValueFtName() != null) {
			...
			if (getSchema().getFeatureType(ftName).isAbstract()) {
				...
				if (concreteSubtypes.length == 0) {
					String msg = "Error in mapping. Feature-particle mapping " + mapping
							+ " has an abstract value feature type ('" + ftName + "') with no concrete substitutions.";
			...
			// I hope this does not break anything. Use the table configured in the Join
			// mapping if the schema did not
	...
	private JoinRow buildJoinRow(InsertRow row, TableJoin join) throws FeatureStoreException {
	...
	/**
	 * Returns the number of currently delayed rows (rows that depend on some other row to
	 * be inserted first).
