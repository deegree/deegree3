...
public class MappedAppSchema extends GenericAppSchema {
	 ...
	 * include the GML base feature types (e.g. <code>gml:_Feature</code> or
	 ...
	 * @param xsModel the underlying GML schema infoset, may be <code>null</code>
	 * @param ftMappings relational mapping information for feature types, can be
	 * <code>null</code> (for BLOB-only mappings)
	 * @param bboxMapping BBOX mapping parameters, may be <code>null</code> (for
	 * relational-only mappings)
	 * @param blobMapping BLOB mapping parameters, may be <code>null</code> (for
	 * relational-only mappings)
	 * @param deleteCascadingByDB if <code>true</code>, the DB automatically deletes
	 * joined tables when a feature type table row is deleted, <code>false</code>
	 * otherwise
	 * @param relationalModel table detail information, may be <code>null</code>
	 ...
	 */
	public MappedAppSchema(FeatureType[] fts, Map<FeatureType, FeatureType> ftToSuperFt, Map<String, String> prefixToNs,
			GMLSchemaInfoSet xsModel, FeatureTypeMapping[] ftMappings, BBoxTableMapping bboxMapping,
			BlobMapping blobMapping, GeometryStorageParams geometryParams, boolean deleteCascadingByDB,
	 ...
	 * @return the corresponding mapping, may be <code>null</code> (if the feature type
	 * does not have a relational mapping)
	 */
	 ...
	 * @param ftName name of the feature type, must denote a non-abstract feature type
	 * that's part of the schema and not <code>null</code>
	 ...
	 * @param ftId id of the feature type
	 * @return name of the feature type, denotes a non-abstract feature type that's part
	 * of the schema
	 */
	 ...
	 * @return the BLOB mapping parameters, may be <code>null</code> (for RELATIONAL-only
	 * mappings)
	 */
	 ...
	 * / geometries by id).
	 * @return the name of the lookup table, can be <code>null</code> (no global lookup
	 * table, only per feature type)
	 */
	 ...
	 * @param featureOrGeomId id to be analyzed, must not be <code>null</code>
	 * @return id analysis, never <code>null</code>
	 * @throws IllegalArgumentException if the id does not denote a feature or geometry id
	 */
	public IdAnalysis analyzeId(String featureOrGeomId) throws IllegalArgumentException {
