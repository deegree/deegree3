...
/**
 * A {@link ValueReference} that's mapped to the relational model defined by a
...
public class MappedXPath {
	 ...
	 * @param propName
	 * @param aliasManager
	 * @param isSpatial if <code>true</code>, a spatial property is targeted (in this
	 * case, mapped property names are automatically extended to the nearest geometry
	 * child in the mapping configuration)
	 * @param handleStrict
	 * @throws UnmappableException if the propertyName can not be matched to the
	 * relational model
	 */
	public MappedXPath(SQLFeatureStore fs, FeatureTypeMapping ftMapping, ValueReference propName,
		...
		if (propName == null || propName.getAsText().isEmpty()) {
			...
			// the first step may be the name of the feature type or the name of a
			// property
	...
	private void map(FeatureMapping mapping, List<MappableStep> remaining) throws UnmappableException {
		...
		// TODO what do do with Href mappings here? Needs proper reference resolving code
		// for mapping here...
