...
/**
 * Creates {@link MappedAppSchema} instances from {@link AppSchema}s by inferring a
...
public class AppSchemaMapper {
	 ...
	 * @param createBlobMapping true, if BLOB mapping should be performed, false otherwise
	 * @param createRelationalMapping true, if relational mapping should be performed,
	 * false otherwise
	 * @param geometryParams parameters for storing geometries, must not be
	 * <code>null</code>
	 * @param maxLength max length of column names
	 * @param usePrefixedSQLIdentifiers <code>true</code> if the sql identifiers should be
	 * prefixed, <code>false</code> otherwise
	 * @param useIntegerFids <code>true</code> if the integer fids should be used,
	 * <code>false</code> for uuids
	 */
	 ...
	 * @param useIntegerFids <code>true</code> if the integer fids should be used,
	 * <code>false</code> for uuids
	 * @param allowedCycleDepth depth of the allowed cycles
	 */
	public AppSchemaMapper(AppSchema appSchema, boolean createBlobMapping, boolean createRelationalMapping,
			GeometryStorageParams geometryParams, int maxLength, boolean usePrefixedSQLIdentifiers,
	 ...
	 * <code>false</code> for uuids
	 * @param allowedCycleDepth depth of the allowed cycles
	 * @param referenceData describing the data stored in the features store
	 * @param useRefDataProps <code>true</code> if only properties defined in reference
	...
	private FeatureTypeMapping[] generateFtMappings(FeatureType[] fts) {
		return Arrays.stream(fts)
			.filter(ft -> referenceData != null ? referenceData.shouldFeatureTypeMapped(ft.getName()) : true)
			.map(ft -> generateFtMapping(ft))
			.toArray(FeatureTypeMapping[]::new);
	}
	...
	private List<Mapping> generatePropMapping(PropertyType pt, MappingContext mc, CycleAnalyser cycleAnalyser) {
		...
		try {
			if (elDecl != null && elDecl.getTypeDefinition() instanceof XSComplexTypeDefinition) {
				...
				List<XSElementDeclaration> substitutions = appSchema.getGMLSchema()
					.getSubstitutions(elDecl, null, true, true);
				for (XSElementDeclaration substitution : substitutions) {
					try {
						...
						XSComplexTypeDefinition typeDefinition = (XSComplexTypeDefinition) substitution
							.getTypeDefinition();
	...
	private List<Mapping> generateMapping(XSComplexTypeDefinition typeDef, MappingContext mc, ObjectPropertyType opt,
			CycleAnalyser cycleAnalyser) {
		List<Mapping> particles = new ArrayList<Mapping>();
	...
	private void createAndAddTimeProperty(MappingContext mc, String propertyName, List<Mapping> particles) {
		...
		MappingContext mappingContext = mcManager.mapOneToOneElement(mc, new QName(GML3_2_NS, propertyName));
		...
		PrimitiveMapping timePositionMapping = new PrimitiveMapping(path, false, dbField, primitiveType, null, null);
	...
	private List<Mapping> generateMapping(XSParticle particle, int maxOccurs, MappingContext mc,
		...
		// ObjectPropertyType pt = appSchema.getXSModel().getGMLPropertyDecl( elDecl,
		// elName, minOccurs, maxOccurs,
		// ptSubstitutions );
		// if ( pt != null ) {
	...
	private List<Mapping> generateMapping(XSElementDeclaration elDecl, int occurence, MappingContext mc,
		...
		// return mappings;
		// }
		// if ( eName.equals( new QName( "http://www.opengis.net/gml/3.2",
		// "TimeOrdinalEra" ) ) ) {
		...
		// }
		//
		// if ( eName.equals( new QName( "http://www.isotc211.org/2005/gmd",
