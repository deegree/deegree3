...
public class OracleDDLCreator extends DDLCreator {
    ...
    private static Logger LOG = getLogger( OracleDDLCreator.class );
    ...
    protected StringBuffer createJoinedTable( TableName fromTable, TableJoin jc, List<StringBuffer> ddls, FIDMapping fidMapping ) {
        ...
        ddls.add( new StringBuffer( "create sequence " ).append( jc.getToTable() ).append( "_id_seq start with 1 increment by 1 nomaxvalue" ) );
        ddls.add( new StringBuffer( "create or replace trigger " ).append( jc.getToTable() ).append( "_id_trigger before insert on " ).append( jc.getToTable() ).append( " for each row begin select " ).append( jc.getToTable() ).append( "_id_seq.nextval into :new.id from dual; end;" ) );
