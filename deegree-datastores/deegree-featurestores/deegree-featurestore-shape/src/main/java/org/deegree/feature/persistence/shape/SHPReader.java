...
public class SHPReader {
	...
	/**
	 * @param bbox
	 * @param ids if not null, the resulting list will only contain record numbers which
	 * are also contained in this set. If null, all matching records are returned.
	...
	public List<Pair<Integer, Long>> query(Envelope bbox, HashSet<Integer> ids) {
		...
		for (Long ptr : pointers) {
			...
			if (num == 0 && !recordNumStartsWith0 && rtree != null) {
				LOG.error(
						"PLEASE NOTE THIS: Detected that the shape file starts counting record numbers at 0 and not at 1 as specified!");
				LOG.error(
						"PLEASE NOTE THIS: This should not happen any more, and is a bug! Please report this along with the data!");
		...
		while (buffer.position() + 1 < buffer.capacity()) {
			...
			switch (type) {
				...
				case POINT: {
					...
					Pair<float[], Long> p = new Pair<float[], Long>(
							new float[] { (float) x, (float) y, (float) x, (float) y }, pos);
					list.add(p);
					break;
				}
				default: {
					Pair<float[], Long> p = new Pair<float[], Long>(new float[] { (float) buffer.getDouble(),
							(float) buffer.getDouble(), (float) buffer.getDouble(), (float) buffer.getDouble() }, pos);
					list.add(p);
					break;
				}
			}
	 ...
	 * @param ptr position of the entry
	...
	private Point readPointZ(ByteBuffer buffer) {
		return fac.createPoint(null,
				new double[] { buffer.getDouble(), buffer.getDouble(), buffer.getDouble(), buffer.getDouble() }, crs);
	}
