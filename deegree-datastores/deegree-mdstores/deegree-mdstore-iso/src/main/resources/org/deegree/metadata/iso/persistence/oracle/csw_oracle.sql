CREATE TABLE IDXTB_MAIN ( 
	...
	abstract CLOB NOT NULL, -- can occure multiple times, considering multi languages
	anytext CLOB NOT NULL,
...
ALTER TABLE IDXTB_OperatesOnData ADD CONSTRAINT FK_IDXTB_OpOnData_IDXTB_MAIN FOREIGN KEY (fk_main) REFERENCES IDXTB_MAIN (ID) ON DELETE CASCADE;

-- set index for alpha numeric columns 
-- IDXTB_MAIN
CREATE INDEX title_idx ON IDXTB_MAIN (title);
CREATE INDEX abstract_idx ON IDXTB_MAIN (abstract) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
CREATE INDEX anytext_idx ON IDXTB_MAIN (anytext) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
...
CREATE INDEX lineage_idx ON IDXTB_MAIN (lineage) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
...
CREATE INDEX ConditionAppToAcc_idx ON IDXTB_Constraint (ConditionAppToAcc) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
CREATE INDEX AccessConstraints_idx ON IDXTB_Constraint (AccessConstraints);
CREATE INDEX OtherConstraints_idx ON IDXTB_Constraint (OtherConstraints) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
...
CREATE INDEX keywords_idx ON IDXTB_KEYWORD (keywords) INDEXTYPE IS CTXSYS.CONTEXT PARAMETERS ('DATASTORE CTXSYS.DEFAULT_DATASTORE');
...
delete from USER_SDO_GEOM_METADATA where TABLE_NAME = 'IDXTB_MAIN' AND COLUMN_NAME = 'BBOX';
INSERT INTO USER_SDO_GEOM_METADATA (TABLE_NAME, COLUMN_NAME, DIMINFO, SRID) 
  VALUES ('IDXTB_MAIN', 'BBOX', 
    MDSYS.SDO_DIM_ARRAY 
    (
...
CREATE INDEX SPX_GEOMETRIE_BBOX on IDXTB_MAIN(BBOX) INDEXTYPE is MDSYS.SPATIAL_INDEX;
