...
public class EOPropertyNameMapper implements PropertyNameMapper {
     ...
     *            return type names as specified in the query
     * @return query type to be returned, never <code>null</code>
     * @throws MetadataStoreException
     *             if the specified return type names do not correspond to the query types
     */
    ...
    public PropertyNameMapping getMapping( ValueReference propName, TableAliasManager aliasManager )
        ...
        int stepNo = 1;
        // check that every step in the location path is a name step on the child or attribute axis (and nothing else)
        for ( Object o : steps ) {
    ...
    private SlotMapping getSlot( ValueReference propName, NameStep slotStep )
        ...
        if ( expr == null || ( !( expr instanceof EqualityExpr ) )
             || !( (EqualityExpr) expr ).getOperator().equals( "=" ) ) {
            String msg = "Unable to map PropertyName '" + propName.getAsText()
                         + "'. Slot steps must specify a single name predicate (../rim:Slot[@name=...]).";
            throw new UnmappableException( msg );
        }
