...
class CoverageDimensionHandler {
	...
	RangeSet getDimensionFilter(Map<String, List<?>> dims, List<String> headers) throws OWSException {
		...
		for (String name : layerDims.keySet()) {
			...
			if (dims.get(name) == null) {
				...
				if (name.equals("elevation")) {
					headers
						.add("99 Default value used: elevation=" + vals.get(0) + " " + (units == null ? "m" : units));
				}
				else {
	...
	private void handleDimensionValue(Dimension<?> dim, Object o, List<Interval<?, ?>> intervals,
			List<SingleValue<?>> singleValues, List<String> headers, String name) throws OWSException {
		...
		if (o instanceof DimensionInterval<?, ?, ?>) {
			...
			// repair min/max values to extent min/max values if applicable (raster API
			// fails if boundary values are not
			// correct)
			final String min;
