...
public class WpsPrinterBean implements Serializable {
    ...
    private void addProcess( List<SelectItem> templates, Process p ) {
        if ( p != null ) {
            final String label = p.getId().getCode()
                                 + ( p.getTitle() != null ? ( "-" + p.getTitle().getString() ) : "" );
            ...
            if ( wpsClient != null ) {
                ...
                if ( process != null ) {
                    ...
                    try {
                        ...
                        FacesMessage fm = MessageUtils.getFacesMessage( FacesMessage.SEVERITY_ERROR,
                                                                        "WPSPrinterBean.error.updateTemplate",
                                                                        e.getMessage() );
                        FacesContext.getCurrentInstance().addMessage( null, fm );
                    }
                }
    ...
    public void renderMetaInfo( ComponentSystemEvent event )
                            throws AbortProcessingException {
                ...
                try {
                    ...
                    for ( InputType inputType : inputTypes ) {
                        if ( inputType.getType() == Type.LITERAL ) {
                            ...
                            String id = null;
                            if ( "http://www.deegree.org/processprovider/map".equals( comp.getDefaultFormat().getSchema() ) ) {
                            ...
                            } else if ( "http://www.deegree.org/processprovider/table".equals( comp.getDefaultFormat().getSchema() ) ) {
