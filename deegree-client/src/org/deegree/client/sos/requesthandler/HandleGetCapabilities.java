...
/**
 * 
 * Requesthandler-class for GetCapabilities-request. Initializes a new
 * StorageGetCapabilities-object with contents of the GetCapabilities-response
 * called by a http-post-method.
 * 
...
public class HandleGetCapabilities {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(HandleGetCapabilities.class);
	...
	/**
	 * Public constructor initializes the XMLAdapter-object with the given
	 * request-String (http-get request).<br>
	...
	private List<Pair<String, String>> parseServiceIdentification() {
		...
		xpath = new XPath("//ows:ServiceIdentification/descendant::*",
				nsContext);
	...
	private List<Pair<String, String>> parseServiceProvider() {
		...
		for (OMElement element : nodes) {
			...
			if (value != null && !value.trim().equals("")) {
				...
				pair.second = value;
				result.add(pair);
			} else if (name.equals("ProviderSite")
					|| name.equals("OnlineResource")) {
				Pair<String, String> pair = new Pair<String, String>();
				pair.first = name;
				pair.second = element.getAttributeValue(new QName(
						CommonNamespaces.XLNNS, "href"));
				result.add(pair);
			}
		}
		return result;
	...
	private List<Operation> parseOperationsMetadata() {
		...
		List<OMElement> operations = xml.getElements(xml.getRootElement(),
				xpath);
		for (OMElement operationsElement : operations) {
			Operation operation = new Operation();
			String operationName = operationsElement.getAttribute(
					new QName("name")).getAttributeValue();
			...
			for (OMElement metadataElement : nodes) {
				...
				if (metadataElement.getLocalName().equals("Get")) {
					pair.first = "Get";
					pair.second = metadataElement.getAttributeValue(new QName(
							CommonNamespaces.XLNNS, "href"));
					http.add(pair);
			...
			for (OMElement parameterElement : nodes) {
				...
				String parameterName = parameterElement.getAttribute(
						new QName("name")).getAttributeValue();
				parameter.setName(parameterName);
				xpath = new XPath("ows:AllowedValues/descendant::*", nsContext);
				List<OMElement> owsValues = xml.getElements(parameterElement,
						xpath);
				List<String> parameterValues = new ArrayList<String>();
				for (OMElement owsValue : owsValues) {
					if (owsValue.getText() != null
							&& !owsValue.getText().trim().equals("")) {
	...
	private Filter_Capabilities parseFilter_Capabilities() {
		...
		for (OMElement temporal_Capability : nodes) {
			if (temporal_Capability.getLocalName().equals("TemporalOperands")) {
				xpath = new XPath("child::*", nsContext);
				List<OMElement> temporalOperands = xml.getElements(
						temporal_Capability, xpath);
				operands = new ArrayList<String>();
				for (OMElement temporalOperand : temporalOperands) {
					if (temporalOperand.getLocalName()
							.equals("TemporalOperand")) {
						operands.add(temporalOperand.getText());
					}
				}
				result.setTemporalOperands(operands);
			} else if (temporal_Capability.getLocalName().equals(
					"TemporalOperators")) {
				xpath = new XPath("child::*", nsContext);
				List<OMElement> temporalOperators = xml.getElements(
						temporal_Capability, xpath);
				operators = new ArrayList<Operator>();
				for (OMElement temporalOperatorElement : temporalOperators) {
					Iterator<OMAttribute> attributes;
					for (attributes = temporalOperatorElement
							.getAllAttributes(); attributes.hasNext();) {
						OMAttribute attribute = attributes.next();
						if (attribute.getLocalName().equals("name")) {
							Operator temporalOperator = new Operator();
							temporalOperator.setName(attribute
									.getAttributeValue());
							xpath = new XPath("child::*/child::*", nsContext);
							List<OMElement> temporalOperands = xml.getElements(
									temporalOperatorElement, xpath);
		...
		for (OMElement scalar_Capability : nodes) {
			...
			} else if (scalar_Capability.getLocalName().equals(
					"ComparisonOperators")) {
				List<String> comparisonOperators = new ArrayList<String>();
				xpath = new XPath("child::*", nsContext);
				List<OMElement> comparisonOperatorsElements = xml.getElements(
						scalar_Capability, xpath);
	...
	private List<ObservationOffering> parseOfferings() {
		...
		for (OMElement observationOffering : nodes) {
			...
			offering.setId(observationOffering.getAttributeValue(new QName(
					CommonNamespaces.GMLNS, "id")));
			...
			xpath = new XPath("child::*", nsContext);
			List<OMElement> children = xml.getElements(observationOffering,
					xpath);
			for (OMElement child : children) {
				String childName = child.getLocalName();
				if (childName.equals("metaDataProperty")
					...
					for (attributeIterator = element.getAllAttributes(); attributeIterator
							.hasNext();) {
					...
					List<OMAttribute> attributes = new ArrayList<OMAttribute>();
					for (attributesIterator = child.getAllAttributes(); attributesIterator
							.hasNext();) {
					...
					Iterator<OMAttribute> attributes;
					for (attributes = child.getAllAttributes(); attributes
							.hasNext();) {
						OMAttribute attribute = attributes.next();
