...
public class CRSExporterBase {
	...
	protected void export(IProjection projection, XMLStreamWriter xmlWriter) throws XMLStreamException {
		if (projection != null) {
			...
			if ("transverseMercator".equalsIgnoreCase(implName)) {
				...
				xmlWriter.writeAttribute("inDegrees", "true");
				xmlWriter
					.writeCharacters(Double.toString(((IStereographicAzimuthal) projection).getTrueScaleLatitude()));
				xmlWriter.writeEndElement();
			}
			xmlWriter.writeEndElement();
			xmlWriter.writeEndElement();
	...
	protected void exportAbstractCRS(ICRS crs, XMLStreamWriter xmlWriter) throws XMLStreamException {
		if (crs != null) {
			...
			StringBuilder axisOrder = new StringBuilder(4); // maxOccurs of Axis = 3 in
															// the schema
	...
	/**
	 * Export a list of transformations from the crs element to xml with respect to the
	 * crs-definitions schema layout.
	 * @param transformations to be exported.
	 * @param xmlWriter to export to.
	 * @throws XMLStreamException
	 */
