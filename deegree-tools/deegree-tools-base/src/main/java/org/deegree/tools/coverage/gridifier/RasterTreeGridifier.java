...
public class RasterTreeGridifier {
    ...
    private RasterTreeGridifier( String rtbBaseDir, float minX, float minY, float maxX, float maxY, String outputDir,
                                 int tileHeight, int tileWidth, long maxBlobSize, boolean recursive,
    ...
    private void generateCells( RasterLevel level )
    ...
    private void writeMetaInfoFile( File file, RasterGeoReference env, int columns, int rows )
                            throws IOException {
    ...
    private static long blobSize( String blobsize ) {
        ...
        if ( "k".equals( unit ) ) {
            ...
            throw new IllegalArgumentException( "Unknown blobsize unit: " + unit
                                                + " (try k(ilo), m(ega), g(iga) byte)." );
        }
    ...
    private class Worker implements Runnable {
        ...
        public void run() {
            ...
            Set<TileFile> tileFiles = tileIndex.getTiles( geomFactory.createEnvelope( croppedMinX, croppedMinY,
                                                                                      croppedMaxX, croppedMaxY, null ),
                                                          metersPerPixel );
    ...
    private class BlobWriterThread implements Runnable {
        ...
        private SortedMap<Integer, ByteBuffer> fifo = Collections.synchronizedSortedMap( new TreeMap<Integer, ByteBuffer>() );
        ...
        private void storeCell( ByteBuffer buffer )
                                throws IOException {
