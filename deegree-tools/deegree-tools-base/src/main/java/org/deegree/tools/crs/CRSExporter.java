...
public class CRSExporter extends CRSExporterBase {
	 ...
	 * @param crsToExport
	 * @param xmlWriter to write the definitions to.
	 * @throws XMLStreamException if an error occurred while exporting
	 */
	@Override
					...
					if (crs != null) {
						...
						switch (type) {
							...
							case PROJECTED: {
								...
								if (id != null && (id.contains("Snyder-StereoGraphic") || id.contains("9820") /*
																												 * LaAzEq
																												 */
										|| id.contains("9809") /* StAl */ || id.contains("9802") /*
																									 * LaCoCo
																									 */
										|| id.contains("9804") /* Merc */
										|| id.contains("9807") /* tmerc */)) {
						...
						if (rs != null && rs.next()) {
							...
							if (newId != 0) {
								...
								BufferedReader reader = new BufferedReader(new InputStreamReader(
										new ByteArrayInputStream(remark), Charset.forName("LATIN1")));
			...
			if (Math.abs(areaOfUseBBox[0] + 180) < 1E-8 && Math.abs(areaOfUseBBox[1] + 90) < 1E-8
				...
				if (epsgCode != -1) {
					...
					try {
						connection = connProvider.getConnection();
						PreparedStatement ps = connection.prepareStatement(
								"SELECT b.area_of_use, b.area_west_bound_lon, b.area_south_bound_lat,b.area_east_bound_lon,b.area_north_bound_lat from epsg_coordinatereferencesystem as a JOIN epsg_area as b on a.area_of_use_code=b.area_code where a.coord_ref_sys_code=?");
						ps.setInt(1, epsgCode);
						ResultSet rs = ps.executeQuery();
						if (rs != null && rs.next()) {
							// no bbox defined
	...
	private void updateProjectionId(IProjectedCRS proj, IProjection projection) {
		...
		if (db_id != null) {
			...
			if (epsgCode != -1) {
				...
				LOG.debug(
						"No epsg code found for projected crs, setting the projected crs value as the default value.");
			}
		}
		else {
	 ...
	 */
	protected void exportEllipsoids(XMLStreamWriter xmlWriter, Set<IEllipsoid> ellipsoids) throws XMLStreamException {
	...
	protected void exportCoordinateSystems(XMLStreamWriter xmlWriter, Set<ICompoundCRS> compounds,
			Set<IProjectedCRS> projecteds, Set<IGeographicCRS> geographics, Set<IGeocentricCRS> geocentrics)
			throws XMLStreamException {
	...
	private void writeDefinitionStart(XMLStreamWriter xmlWriter, String name) throws XMLStreamException {
		...
		// "schemaLocation",
		// "http://ICompoundCRS.deegree.org/crs
		// Y:/WORKSPACE/D3_CORE/RESOURCES/SCHEMA/CRS/0.3.0/crsdefinition.xsd" );
	 ...
	 * @param ntv2 to be exported
	 * @param xmlWriter to export to.
	 * @throws XMLStreamException
	 */
	 ...
	 * @param pm PrimeMeridian to be exported
	 * @param xmlWriter to export to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param compoundCRS to be exported
	 * @param xmlWriter to export the geographic CRS to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param projectedCRS to be exported
	 * @param xmlWriter to export the projected CRS to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param projection to be exported
	 * @param xmlWriter to export the projection to.
	 * @throws XMLStreamException
	 */
	@Override
		...
		if (projection != null) {
			...
			if ("lambertConformalConic".equalsIgnoreCase(implName)) {
				...
				xmlWriter.writeCharacters(
						Double.toString(toDegrees(((IStereographicAzimuthal) projection).getTrueScaleLatitude())));
				xmlWriter.writeEndElement();
			}
			xmlWriter.writeEndElement();
		}
	...
	/**
	 * Export the geocentric/geographic CRS to it's appropriate deegree-crs-definitions
	 * form.
	 * @param geoGraphicCRS to be exported
	 * @param xmlWriter to export the geographic CRS to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param geocentricCRS to be exported
	 * @param xmlWriter to export the geocentric CRS to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param axis to be exported.
	 * @param elName the name of the element, either 'Axis' or 'heightAxis'
	 * @param xmlWriter to export to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param datum to be exported
	 * @param xmlWriter to export the datum to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param ellipsoid to be exported
	 * @param xmlWriter to export the ellipsoid to.
	 * @throws XMLStreamException
	 */
	@Override
	 ...
	 * @param identifiable object to be exported.
	 * @param xmlWriter to export to
	 * @throws XMLStreamException
	 */
	@Override
