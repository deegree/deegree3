...
/**
 * This class simplifies a 3D-Object. Two different simplifications will be performed. The
 * first one create a simplified version of the original geometry by calculating and
 * simplifying the convex hull of its two-dimensional projection. The second one
 * calculates a transformation matrix to move, rotated und stretch a box of size 1,1,1
...
public class WorldObjectSimplifier {
	 ...
	 * <li>4. extrude polygon represented by convex hull to height of the original
	 * geometry (first simplification result)
	 * <li>5. calculate 3D bounding box (currently not supported)
	 * </ul>
	 * @param wro to get the qualitylevel from.
	 * @param levelOfInterest the simplifications should be created from.
	 * @param targetQualityLevel to which the generalisation should be saved.
	 *
	 */
	public void createSimplified3DObject(WorldRenderableObject wro, int levelOfInterest, int targetQualityLevel) {
		...
		if (rqm == null || rqm.getPrototypeReference() != null) {
			throw new IllegalArgumentException("The given QualityModel: " + levelOfInterest
	...
	private RenderableQualityModel createPrototypeReference(RegressionForm form) {
		...
		return new RenderableQualityModel(
				new PrototypeReference("box", angle, form.getCentroid(), width, height, depth));
	...
	private RenderableQualityModelPart createHorizontalGeometry(Coordinate[] coordinates, float zValue) {
		...
		// RenderableGeometry rg = new RenderableGeometry( coords, GL.GL_POLYGON, normals,
		// false );
	...
	/**
	 *
	 * The <code>RegressionForm</code> class calculates a linear regression approximation
	 * over a couple of points in the xy plane. See
	...
	private class RegressionForm {
		...
		/**
		 * @param coordinates to do a regresssion analysis for
		 */
		...
		/**
		 * Calculate the width and the depth of the given points by applying the
		 * regression form
		 * @param coordinates to be used.
		 */
		...
		/**
		 * Calculate a rotation angle (in degrees) by using the slope of the linear
		 * regression
