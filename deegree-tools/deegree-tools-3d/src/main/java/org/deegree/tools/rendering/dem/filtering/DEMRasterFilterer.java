...
public class DEMRasterFilterer {
        ...
        for ( int row = 30; row < rows; ++row ) {
            ...
            for ( int col = 0; col < columns; ++col ) {
                ...
                RasterRect outputRect = new RasterRect( ( ( col * tileWidth ) - kernelHalf ),
                                                        ( ( row * tileHeight ) - kernelHalf ), tileWidth
                                                                                               + this.kernelSize,
            ...
            if ( row + 1 < rows ) {
                ...
                LOG.info( "Filtering row: {}, took approximately: {} seconds, estimated remaining time: {} seconds "
                          + ( ( remain > 60 ) ? "( {} minutes)." : "." ), new Object[] { ( row + 1 ), rPT, remain,
                                                                                        Math.round( remain / 60d ) } );
            }
            System.gc();
            RasterCache.dispose();
        }
    ...
    private void outputTiles()
        ...
        if ( !raster.isSimpleRaster() ) {
            ...
            int percentage = 0;
            System.out.println( "Writing " + totalTiles + " raster files to " + outputDir + " with file type: "
                                + outputType );
    ...
    private static void startFilterer( CommandLine line )
        ...
        try {
            ...
            throw new IllegalArgumentException( "The Standard deviation correction must be a number: "
    ...
    private static void addFilterOptions( Options options ) {
        ...
        option = new Option(
                             "stdCorr",
                             OPT_STANDARD_DEVIATION_CORRECTION,
                             true,
