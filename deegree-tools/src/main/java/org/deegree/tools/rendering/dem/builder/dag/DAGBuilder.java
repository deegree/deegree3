...
public class DAGBuilder {
    ...
    public DAGBuilder(int levels, PatchManager patchManager) {
        ...
        for (Node node : lastRealLevel.nodes) {
            ...
            if (node.locationCode3 == -1) {
                ...
                arc.locationCodes = new int[] { node.locationCode1, node.locationCode2,
                        node.locationCode3, node.locationCode4 };
            }
        }
        ...
        // generate consistent patch numbering (patches on the same arc must have subsequent ids)
    ...
    public void writeBlob(Blob targetBlob) throws SQLException {
        ...
        System.out.print("Writing arcs segment [" + arcsSegmentStart + "-"
                + (arcsSegmentStart + arcsSegmentSize - 1) + "]...");
        ...
        for (Node sourceNode : level.nodes) {
            ...
            if (sourceNode.locationCode3 != -1) {
                if (isDiamond(sourceNode.locationCode3, sourceNode.locationCode4)) {
                    ...
                    arc.locationCodes = new int[] { sourceNode.locationCode3,
                            sourceNode.locationCode4 };
                } else {
                    // 2 Kanten mit jeweils 1 Patch
    ...
    private Arc addArc(Node sourceNode, Node destinationNode) {
        ...
        if (destinationNode.lowestIncomingArc == -1
     ...
     *
     * @param node
     *                the modification node
     * @param locationCode
     *                triangle location code introduced by the node
     * @return
     */
    ...
    /**
     * Return the modification node that contains the triangle with the given location code.
     *
     * @param locationCode
     *                triangle location code
     * @return
     */
    ...
    public void printArrays() {
        ...
        System.out.println(nodes.length + " nodes, " + arcs.length + " arcs, " + patches.length
                + " patches.");
    }
    ...
    public void printStats() {
        ...
        for (int i = 1; i < dagLevels.length; i++) {
            ...
            System.out.println("Level " + i + " has " + numNodes + " node(s) and " + arcsFromParent
                    + " arc(s) to parent level.");
