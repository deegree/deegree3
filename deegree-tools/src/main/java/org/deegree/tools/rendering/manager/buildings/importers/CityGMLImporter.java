...
public class CityGMLImporter implements ModelImporter {
    ...
    public CityGMLImporter( String schemaLocation, float[] translation, SimpleGeometryStyle defaultStyle,
        ...
        ApplicationSchemaXSDDecoder adapter = null;
        try {
        ...
        if ( buildingType == null ) {
            throw new IllegalArgumentException( "Could not create a featuretype: " + NS_BRACE
                                                + "Building, probably your schema at location: " + schemaLoc
                                                + " is invalid. Or try the opengis switch." );
        }
    ...
    private FeatureCollection readGML( String fileName )
                            throws IOException, XMLStreamException, FactoryConfigurationError, XMLParsingException,
     ...
     * @param min
     *            will contain the minimum of all geometries of the given geometry
     * @param max
    ...
    /**
     * Extract the coordinates from the given {@link PolygonPatch} rings and add them to the given
     ...
     *            to use.
     * @param index
     *            pointing to the current vertex
     */
    ...
    private String getID( Feature building ) {
        ...
        if ( result == null || "".equals( result.trim() ) ) {
            ...
            LOG.warn( "Created id: " + result + " for building with envelope: " + building.getEnvelope()
                      + " and name: " + building.getName() + " because it did not supply a gml:id." );
        }
        return result;
    }
            ...
            if ( BUILDING_QNAME.equals( f.getName() ) ) {
                ...
                if ( wro != null ) {
                    if ( !bMap.containsKey( wro.getId() ) ) {
                        ...
                        LOG.warn( "Duplicate building with id: " + wro.getId()
                                  + " using first building with envelope: " + wro.getBbox().toString() );
                    }
                }
            } else {
