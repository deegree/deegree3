    ...
    public static void main( String[] args )
        ...
        if ( args.length == 0 ) {
            ...
            if ( "-update".equals( args[0] ) ) {
                buildTestSuite( getInstance().getClass() );
            } else {
                ToolInfo tool = getInstance().findTool( args[0] );
                if ( tool != null ) {
                    if ( args.length > 1 ) {
                        ...
                        for ( Method m : methods ) {
                            if ( m != null ) {
                                if ( "main".equals( m.getName() )
                                     && ( ( ( m.getModifiers() & Modifier.PUBLIC ) == Modifier.PUBLIC ) && ( ( m.getModifiers() & Modifier.STATIC ) == Modifier.STATIC ) )
