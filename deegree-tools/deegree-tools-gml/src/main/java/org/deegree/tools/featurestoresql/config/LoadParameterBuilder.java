...
public class LoadParameterBuilder {
		...
		if (depth == null || depth.isEmpty()) {
			...
			try {
				...
				throw new IllegalArgumentException(
						"Invalid value of parameter cycledepth: " + depth + ". Must be an integer value.");
			}
		}
		return this;
	}
	...
	public LoadParameterBuilder setListOfPropertiesWithPrimitiveHref(String pathToFileWithPropertiesWithPrimitiveHref) {
		if (pathToFileWithPropertiesWithPrimitiveHref != null) {
			...
			List<QName> propertiesWithPrimitiveHref = propertyNameParser
				.parsePropertiesWithPrimitiveHref(pathToFileWithPropertiesWithPrimitiveHref);
	...
	public LoadParameterBuilder setReferenceData(String referenceData) {
		if (referenceData != null && !referenceData.isEmpty()) {
			try {
				...
				throw new IllegalArgumentException(
						"Invalid value of parameter referenceData: " + referenceData + ". Could not be read.");
			}
	...
	public LoadParameterBuilder setuseRefDataProps(String useRefDataProps) {
		if (useRefDataProps != null && !useRefDataProps.isEmpty()) {
			if (!"true".equalsIgnoreCase(useRefDataProps) && !"false".equalsIgnoreCase(useRefDataProps)) {
				throw new IllegalArgumentException(
						"Invalid value of parameter useRefDataProps: " + useRefDataProps + ". Must be true or false");
			}
			loadParameter.setUseRefDataProps(Boolean.parseBoolean(useRefDataProps));
		}
		return this;
	}
