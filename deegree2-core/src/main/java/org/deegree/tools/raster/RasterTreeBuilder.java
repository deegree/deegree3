...
public class RasterTreeBuilder {
     ...
     * @param dummy
     */
    public RasterTreeBuilder( List<String> imageFiles, String outputDir, String baseName, String outputFormat,
                              double maxTileSize, String srs, String interpolation, WorldFile.TYPE worldFileType,
                              float quality, String bgColor, int depth, double resolution, float offset,
                              float scaleFactor, boolean dummy ) {
    ...
    private TiledImage loadImage( String imageSource )
        ...
        if ( ti == null ) {
            System.out.println( "cache size: " + imgCache.getSize() );
    ...
    private void paintImagesOnTile( Tile tile )
        ...
        if ( dummy ) {
            ...
            for ( int i = 0; i < imageFiles.size(); i++ ) {
                ...
                File file = new File( imageFiles.get( i ) );
                if ( imageFilesErrors.get( imageFiles.get( i ) ) == null && file.exists() && !file.isDirectory() ) {

                    WorldFile wf = imageFilesEnvs.get( i );
                    if ( wf == null ) {
                    ...
                    if ( wf.getEnvelope().intersects( tileEnv ) ) {
                        TiledImage bi = loadImage( imageFiles.get( i ) );
                        gcc++;
                        try {
                            System.out.println( "drawImage" );
                            drawImage( out, data, bi, tile, wf, minimumRes, interpolation, imageFilesErrors,
                                       outputFormat, bitDepth, offset, scaleFactor );
                            paint = true;
                        } catch ( Exception e ) {
                            e.printStackTrace();
                            imageFilesErrors.put( imageFiles.get( i ), e.getMessage() );
     ...
     */
    public static void drawImage( BufferedImage out, float[][] data, final TiledImage image, Tile tile, WorldFile wf,
        ...
        if ( x2 - x1 >= 0 && y2 - y1 >= 0 && x1 + x2 - x1 < image.getWidth() && y1 + y2 - y1 < image.getHeight()
            ...
            // + image.getHeight() );
            BufferedImage img = image.getSubImage( x1, y1, w, h ).getAsBufferedImage();
    ...
    public static void main( String[] args )
        ...
        RasterTreeBuilder rtb = new RasterTreeBuilder( imageFiles, outDir, baseName, outputFormat, maxTileSize, srs,
                                                       interpolation, worldFileType, quality, backgroundColor, depth,
                                                       resolution, offset, scaleFactor, dummy );
        ...
        for ( int i = 0; i < level; i++ ) {
            ...
            if ( i < level - 1 ) {
                ...
                rtb = new RasterTreeBuilder( imageFiles, outDir, baseName, outputFormat, maxTileSize, srs,
                                             interpolation, WorldFile.TYPE.CENTER, quality, backgroundColor, depth,
                                             resolution, offset, scaleFactor );
            }
