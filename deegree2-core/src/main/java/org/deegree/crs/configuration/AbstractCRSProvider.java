...
public abstract class AbstractCRSProvider<T> implements CRSProvider {
    ...
    private static ILogger LOG = LoggerFactory.getLogger( AbstractCRSProvider.class );

    private Map<String, Identifiable> cachedIdentifiables = new HashMap<String, Identifiable>( 42124 );
    ...
    public <K extends CRSResource<T>> AbstractCRSProvider( Properties properties, Class<K> subType,
        ...
        if ( className == null || "".equals( className.trim() ) ) {
            if ( defaultResolver != null ) {
                LOG.logWarning( "Found no configured CRS-Resource to use, trying default: "
            ...
            try {
                ...
                if ( resolver == null ) {
                    ...
                    if ( defaultResolver != null ) {
                        LOG.logInfo( "Trying to instantiate the default resolver: "
                                     + defaultResolver.getClass().getCanonicalName() );
                        resolver = defaultResolver;
                    } else {
        ...
        if ( id != null && !"".equals( id.trim() ) ) {
            ...
            if ( cachedIdentifiables.containsKey( id ) ) {
                ...
                if ( !( r instanceof CoordinateSystem ) ) {
                    LOG.logError( "Found Identifiable: " + r.getIdAndName()
    ...
    public void setResolver( CRSResource<T> newResolver ) {
        if ( LOG.isDebug() ) {
            LOG.logDebug( "The following resolver was set: " + newResolver + ", it will replace the old resolver: "
                          + this.resolver );
        }
     ...
     * @return the {@link Identifiable} of the first matching id or <code>null</code> if it was not found.
     */
     ...
     * @return the {@link Identifiable} of the first matching id or <code>null</code> if it was not found.
     */
    public <V extends Identifiable> V getCachedIdentifiable( Class<V> expectedType, String[] ids ) {
        ...
        for ( int i = 0; i < ids.length && result == null; i++ ) {
            ...
            if ( LOG.isDebug() ) {
                LOG.logDebug( "Searched for id: " + ids[i] + " resulted in: "
     ...
     * @param id
     *            to search the cache for
     * @return the {@link Identifiable} or <code>null</code> if it was not found or the wrong type was found.
     */
    @SuppressWarnings("unchecked")
    public <V extends Identifiable> V getCachedIdentifiable( Class<V> expectedType, String id ) {
        if ( id == null ) {
            return null;
        }
        V result = null;
        try {
            ...
            LOG.logError( "Given id is not of type: " + expectedType.getCanonicalName() + " found following error: "
                          + cce.getLocalizedMessage() );
        }
        if ( LOG.isDebug() ) {
            LOG.logDebug( "Searched for id: " + id + " resulted in: "
                          + ( ( result == null ) ? "null" : result.getIdentifier() ) );
        }
        return result;
    }
     ...
     */
    public <V extends Identifiable> V addIdToCache( V identifiable, boolean update ) {
                ...
                for ( String idString : identifiable.getIdentifiers() ) {
                    if ( idString != null && !"".equals( idString.trim() ) ) {
                        if ( cachedIdentifiables.containsKey( idString ) && cachedIdentifiables.get( idString ) != null ) {
                            if ( update ) {
                                LOG.logDebug( "Updating cache with new identifiable: " + idString );
                                cachedIdentifiables.put( idString, identifiable );
                            }
                        } else {
                            LOG.logDebug( "Adding new identifiable to cache: " + idString );
                            cachedIdentifiables.put( idString, identifiable );
                        }
                    } else {
                        LOG.logWarning( "Not adding the null string id to the cache of identifiable: "
