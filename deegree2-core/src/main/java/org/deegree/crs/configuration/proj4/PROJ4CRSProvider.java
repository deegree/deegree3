...
public class PROJ4CRSProvider extends AbstractCRSProvider<Map<String, String>> {
    ...
    private static ILogger LOG = LoggerFactory.getLogger( PROJ4CRSProvider.class );
    ...
    public List<CoordinateSystem> getAvailableCRSs()
        ...
        if ( LOG.isDebug() ) {
            LOG.logDebug( "Found following keys: " + keys );
        }
        ...
        for ( String key : keys ) {
            try {
                ...
                LOG.logInfo( Messages.getMessage( "CRS_CONFIG_PROJ4_NOT_ADDING_CRS", key, e.getMessage() ) );
            }
        }
    ...
    private Ellipsoid createEllipsoid( Map<String, String> params )
        ...
        if ( tmpValue != null && !"".equals( tmpValue.trim() ) ) {
            LOG.logDebug( "Creating predefined ellipsoid: " + tmpValue );
            result = getPredefinedEllipsoid( tmpValue );
        } else {
            // if no ellipsoid is defined maybe a sphere
            tmpValue = params.remove( "R" );
            if ( tmpValue != null && !"".equals( tmpValue.trim() ) ) {
                LOG.logDebug( "Found a Radius instead of an ellipsoid, the projection uses a sphere!" );
                        ...
                        if ( tmpValue != null && !"".equals( tmpValue.trim() ) ) {
                            ...
                            if ( tmpValue != null && !"".equals( tmpValue.trim() ) ) {
                                ...
                                if ( Math.abs( flattening ) > 0.000001 ) {
                                    ...
                                    LOG.logDebug( "The given flattening: " + flattening
            ...
            if ( !Double.isNaN( eccentricitySquared ) ) {
                ...
                LOG.logDebug( "Only a semimajor defined, assuming a sphere (instead of an ellipsoid) is to be created." );
    ...
    /**
     * Converts angle information to radians. With a little help from com.jhlabs.map.MapMath. For
     * negative angles, d should be negative, m & s positive.
     * 
     * @param d
     *            days
     * @param m
    ...
    /**
     * Converts angle information to degrees. With a little help from com.jhlabs.map.MapMath. For
     * negative angles, d should be negative, m & s positive.
     * 
     * @param d
     *            days
    ...
    public Identifiable getIdentifiable( String id )
        ...
        if ( result == null ) {
            throw new UnsupportedOperationException(
                                                     "The retrieval of an arbitrary Identifiable Object is currently not supported by the proj 4 provider." );
        }
        return result;
    }
    ...
    protected CoordinateSystem parseCoordinateSystem( Map<String, String> crsDefinition )
        ...
        if ( crsType == null || "".equals( crsType.trim() ) ) {
            LOG.logDebug( "The given params contain: " + crsDefinition );
    ...
    public List<Transformation> getTransformations() {
        throw new UnsupportedOperationException(
                                                 "Parsing of transformations is not applicable for proj4 configuration files yet." );
    }
}
