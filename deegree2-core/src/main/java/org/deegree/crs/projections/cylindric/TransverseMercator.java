...
public class TransverseMercator extends CylindricalProjection {
    ...
    private static final long serialVersionUID = -8648170171776619756L;
     ...
     */
    public TransverseMercator( int zone, boolean northernHemisphere, GeographicCRS geographicCRS, Unit units ) {
        this( zone, northernHemisphere, geographicCRS, units, new Identifiable( "EPSG::9807" ) );
    }
    ...
    public Point2d doProjection( double lambda, double phi )
                            throws ProjectionException {
        // LOG.logDebug( "Projection, incoming points lambda: " + lambda + " phi: " + phi );
        LOG.logDebug( "Projection, incoming points lambda: " + Math.toDegrees( lambda ) + " phi: "
                      + Math.toDegrees( phi ) );
        Point2d result = new Point2d( 0, 0 );
