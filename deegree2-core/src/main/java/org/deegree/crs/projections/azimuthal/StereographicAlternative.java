...
public class StereographicAlternative extends AzimuthalProjection {
    ...
    private final static ILogger LOG = LoggerFactory.getLogger( StereographicAlternative.class );

    private static final long serialVersionUID = -444957611911311171L;
    ...
    public StereographicAlternative( GeographicCRS geographicCRS, double falseNorthing, double falseEasting,
        ...
        this( geographicCRS, falseNorthing, falseEasting, naturalOrigin, units, scale, new Identifiable( "EPSG::9809" ) );
    }
    ...
    public Point2d doInverseProjection( double x, double y )
        ...
        if ( LOG.isDebug() ) {
            LOG.logDebug( "(sterea incoming inv proj) x:" + x + ", y:" + y );
        }
    ...
    public Point2d doProjection( double lambda, double phi )
                            throws ProjectionException {
        if ( LOG.isDebug() ) {
            LOG.logDebug( "(sterea incoming proj) lam:" + Math.toDegrees( lambda ) + ", phi:" + Math.toDegrees( phi ) );
        }
        Point2d result = new Point2d();
