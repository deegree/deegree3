...
public class GeocentricTransform extends CRSTransformation {
    ...
    private static final long serialVersionUID = -5306062355334449324L;
    ...
    private void calcParams() {
        this.hasHeight = ( getSourceCRS().getType() == CoordinateSystem.COMPOUND_CRS );
     ...
     * <p>
     * The method used here is derived from "An Improved Algorithm for Geocentric to Geodetic Coordinate Conversion", by
     * Ralph Toms, Feb 1996 UCRL-JC-123138.
     * 
     * @param srcPts
    ...
    protected void toGeographic( List<Point3d> srcPts ) {
        for ( Point3d p : srcPts ) {
            if ( LOG.isDebug() ) {
                LOG.logDebug( "(incoming geocentric) x:" + p.x + ", y:" + p.y + ", z:" + p.z );
            }
            ...
            if ( LOG.isDebug() ) {
                LOG.logDebug( "(outgoing) lam:" + Math.toDegrees( p.x ) + ", phi:" + Math.toDegrees( p.y ) );
            }
        }
    }
    ...
    protected void toGeoCentric( List<Point3d> srcPts ) {
        for ( Point3d p : srcPts ) {
            if ( LOG.isDebug() ) {
                LOG.logDebug( "(incoming geographic) lam:" + Math.toDegrees( p.x ) + ", phi:" + Math.toDegrees( p.y ) );
            }
            ...
            if ( LOG.isDebug() ) {
                LOG.logDebug( "(outgoing) x:" + p.x + ", y:" + p.y + ", z:" + p.z );
            }
        }
    }
