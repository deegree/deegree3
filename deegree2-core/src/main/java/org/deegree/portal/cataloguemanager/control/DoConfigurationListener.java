...
public class DoConfigurationListener extends AbstractListener {
    ...
    private static final String[] sqlOracle = new String[] { "oracle/drop_harvester.sql",
                                                            "oracle/drop_searchTables.sql", 
                                                            "oracle/drop_database.sql",
                                                            "oracle/create_database.sql",
                                                            "oracle/create_harvester.sql",
                                                            "oracle/create_searchTables.sql",
                                                            "oracle/create_spatial_index.sql",
                                                            "oracle/create_spatial_index.sql"};    

    private static final String[] sqlPostgis = new String[] { "postgis/drop_normal_index.sql",
                                                             "postgis/drop_spatial_index.sql",
                                                             "postgis/drop_harvester.sql",
                                                             "postgis/drop_searchTables.sql",
                                                             "postgis/drop_database.sql",
                                                             "postgis/create_database.sql",
                                                             "postgis/create_harvester.sql",
                                                             "postgis/create_searchTables.sql",
                                                             "postgis/create_normal_index.sql",
                                                             "postgis/create_spatial_index.sql" };
     ...
     * @throws IOException 
     */
    private void doMetadataEditorConfiguration( String cswurl ) throws IOException {
    ...
    private void doCSWConfiguration( String driver, String url, String user, String password, String cswurl, String db,
        ...
        String ftFile = "WEB-INF/conf/csw/featuretypes/csw_postgres.xsd";
        ftFile = event.getAbsolutePath( ftFile );
        File tmp = new File(ftFile);
        if ( tmp.exists() ) {
            tmp.delete();
        }
        ftFile = "WEB-INF/conf/csw/featuretypes/csw_oracle.xsd";
        ftFile = event.getAbsolutePath( ftFile );
        tmp = new File(ftFile);
        if ( tmp.exists() ) {
            tmp.delete();
        ...
        if ( db.equalsIgnoreCase( "postgres" ) ) {
            ftFile = "WEB-INF/conf/csw/featuretypes/csw_postgres.xsd.ignore";
        } else if ( db.equalsIgnoreCase( "oracle" ) ) {
            ftFile = "WEB-INF/conf/csw/featuretypes/csw_oracle.xsd.ignore";
        }
     ...
     * @throws IOException 
     */
    private void adaptProperties( String driver, String url, String user, String password, String cswurl ) throws IOException {
        File file = new File( event.getAbsolutePath( "./WEB-INF/classes/org/deegree/ogcwebservices/csw/csw202.properties" ) );
        try {            
    ...
    private void createTables( String driver, String url, String user, String password, String[] sql )
        ...
        if ( "oracle".equalsIgnoreCase( dbVendor ) ) {
            // special for creating oracle trigger
            createOracleTrigger( driver, url, user, password );
        }
    }

    private void createOracleTrigger( String driver, String url, String user, String password )
                            throws Exception, IOException {
        BufferedReader br = null;
        try {
            String s = event.getAbsolutePath( "WEB-INF/scripts/sql/oracle/create_trigger.sql"  );   
            LOG.logDebug( "load ", s );
            br = new BufferedReader( new FileReader( s ) );
        } catch ( Exception e ) {
            throw new Exception( "can not open database creation script: create_trigger.sql" );
        }
        String thisLine, sqlQuery;
        Statement stmt = null;
        DBConnectionPool pool = DBConnectionPool.getInstance();
