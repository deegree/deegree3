...
public class CoordsPortletAction extends JspPortletAction {

    @Override
    protected void buildNormalContext( Portlet portlet, RunData rundata )
                            throws Exception {
        try {
            CoordsPortletPerform crsPerform = new CoordsPortletPerform( rundata.getRequest(), portlet,
                                                                        rundata.getServletContext() );
            crsPerform.buildNormalContext();
        } catch ( Exception e ) {
            e.printStackTrace();
        }
