...
public class LoadHierarchyListListener extends AbstractGazetteerListener {
    ...
    private static ILogger LOG = LoggerFactory.getLogger( LoadHierarchyListListener.class );
    ...
    public void actionPerformed( WebEvent event, ResponseHandler responseHandler )
        ...
        try {
            hierarchies = loadHierarchiesDefinitions( modules[0].getParameter() );
        } catch ( Exception e ) {
            LOG.logError( e );
            responseHandler.writeAndClose( "ERROR: can not load hierarchy list" );
            return;
        }
        String[] hierarchyNames = new String[hierarchies.size()];
        for ( int i = 0; i < hierarchyNames.length; i++ ) {
            hierarchyNames[i] = hierarchies.get( i ).getName();
        }
        responseHandler.writeAndClose( false, hierarchyNames );
    }
