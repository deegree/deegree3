 ...
 */
public class LoadHierarchyListener extends AbstractGazetteerListener {

    private static ILogger LOG = LoggerFactory.getLogger( LoadHierarchyListener.class );
    ...
    public void actionPerformed( WebEvent event, ResponseHandler responseHandler )
        ...
        int index = Integer.parseInt( (String)event.getParameter().get( "hierarchy" ) );

        Hierarchy hierarchy;
        try {
            hierarchy = loadHierarchy( (String) parameters[index].getValue() );
        } catch ( Exception e ) {
            LOG.logError( e );
            responseHandler.writeAndClose( "ERROR: can not load hierarchy for index: " + index );
            return;
        }
        HierarchyNode node = hierarchy.getRoot();
        List<HierarchyNodeBean> beans = new ArrayList<HierarchyNodeBean>();
        while ( node != null ) {
            String name = node.getName();
            boolean sf = node.supportFreeSearch();
            beans.add( new HierarchyNodeBean( name, sf ) );
