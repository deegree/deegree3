...
public class MySQLDataLoader {
    ...
    private static final ILogger LOG = LoggerFactory.getLogger( MySQLDataLoader.class );
        ...
        try {
            ...
            while ( rs.next() ) {
                ...
                for ( int i = 0; i < ccnt; i++ ) {
                    ...
                    if ( name.equalsIgnoreCase( datasource.getGeometryFieldName() ) && value != null ) {
                        byte[] wkb = (byte[]) value;
                        org.postgis.Geometry pgGeometry = new BinaryParser().parse( wkb );
                        value = PGgeometryAdapter.wrap( pgGeometry, crs );
                    }
    ...
    private static PreparedStatement createPreparedStatement( DatabaseDataSource datasource, Envelope envelope,
        ...
        LOG.logDebug( "Envelope in WKT: ", WKTAdapter.export( surface ).toString() );

        stmt.setString( 1, WKTAdapter.export( surface ).toString() );
        return stmt;
    }
