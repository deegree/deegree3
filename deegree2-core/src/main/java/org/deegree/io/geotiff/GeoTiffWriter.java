...
public class GeoTiffWriter {
    ...
    private static final ILogger LOG = LoggerFactory.getLogger( GeoTiffWriter.class );
     ...
     * @param crs
     */
    public GeoTiffWriter( BufferedImage image, Envelope envelope, double resx, double resy, CoordinateSystem crs ) {
     ...
     */
    public GeoTiffWriter( BufferedImage image, Envelope envelope, double resx, double resy, CoordinateSystem crs,
                          double offset, double scaleFactor ) {
     ...
     *
     */
    private void setCoordinateSystem( CoordinateSystem crs ) {
        org.deegree.crs.coordinatesystems.CoordinateSystem crs2 = crs.getCRS();
        if ( crs2 != null ) {
            String[] identifiers = crs2.getIdentifiers();
            int epsg = -1;
            for ( String id : identifiers ) {
                LOG.logDebug( "trying to find EPSG code: " + id );
            ...
            if ( epsg != -1 ) {
                ...
                if ( crs2.getType() == org.deegree.crs.coordinatesystems.CoordinateSystem.GEOGRAPHIC_CRS ) {
                    addKeyToGeoKeyDirectoryTag( GeoTiffKey.GTModelTypeGeoKey,
                                                new int[] { 0, 1, ValueModelTypeGeographic } );
                    ...
                    LOG.logWarning( "can't save coordinate system. coordinate type " + crs2.getType()
                                    + " not supported" );
                }
            }
        }
