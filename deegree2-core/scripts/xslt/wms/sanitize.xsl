...
<stylesheet xmlns="http://www.w3.org/1999/XSL/Transform" xmlns:ll="http://www.lat-lon.de" xmlns:gml="http://www.opengis.net/gml" version="1.0">
  ...
  <template match="FeatureInfoResponse/FIELDS" mode="sanitize">
    <gml:featureMember>
      <ll:feature>
        <for-each select="@*">
          <variable name="attName"><value-of select="local-name(.)" /></variable>
  ...
  <template match="featureInfo/query_layer/object" mode="sanitize">
    <gml:featureMember>
      ...
      <element name="ll:{$layerName}">
        <for-each select="*">
          <variable name="propName"><value-of select="local-name(.)" /></variable>
