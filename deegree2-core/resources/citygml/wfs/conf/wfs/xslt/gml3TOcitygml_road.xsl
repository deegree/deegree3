...
xmlns:citygml="http://www.citygml.org/citygml/1/0/0" exclude-result-prefixes="wfs fo xsl ogc">
	...
	<xsl:template name="ROAD">
		<citygml:Road>
			...
			<xsl:apply-templates select="citygml:Feature/citygml:road/citygml:Road"/>
		</citygml:Road>
	</xsl:template>

	<xsl:template match="citygml:Road" name="RoadTemplate">
		...
		<xsl:for-each select="../../citygml:linkedGeometry/citygml:LINK_FEAT_GEOM">
			<xsl:if test="citygml:lod = 0">
				<citygml:lod0Network>
					<xsl:call-template name="roadGEOM">
	...
	<xsl:template name="roadGEOM">
		<xsl:param name="LOD">0</xsl:param>
		
		<xsl:choose>
			<xsl:when test="$LOD = 0">
				<xsl:for-each select="citygml:curveComposite/citygml:CurveComposite">
