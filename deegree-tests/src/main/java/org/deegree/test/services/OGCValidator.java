 ...
 *
 * <p>
 * This class offers some convenience for the validation process of OGC service requests. See {@link ValidatorTemplate}
 * for an example. Subclasses should call the {@link #setSchemaDoc(String)} and {@link #setNSContext(NamespaceContext)}
 * methods within an init-method (use junit's {@link BeforeClass} annotation).<br />
 *
 * The unit tests should then call {@link #getAsserterForValidDoc(String)} to send the request and validate the
 * response. This method returns an {@link XPathAsserter} that can be used to check the content of the repsonse.
 * {@link #getAsserterForValidDoc(String)} will also check the HTTP response (code 200, content-type text/xml). You can
 * alter this behaviour with your own {@link HTTPResponseValidator}.
 *
 *
...
public abstract class OGCValidator {
    ...
    /**
     * Create a XPathAsserter for the given URL. This method further checks if the document is valid and if the HTTP
     * response is OK (status=200) and text/xml.
     *
     * @param url
     *            the request URL
    ...
    /**
     * Create a XPathAsserter for the given URL. This method further checks if the document is valid and checks the HTTP
     * response with the given validator.
     *
     * @param url
     *            the request URL
     * @param check
     *            an {@link HTTPResponseValidator}
     * @return an XPathAsserter
     */
    ...
    /**
     * Create a XPathAsserter for the given URL and POST content. This method further checks if the document is valid
     * and if the HTTP response is OK (status=200) and text/xml.
     *
     * @param url
     *            the request URL
     * @param post
     *            a file with the post content
     * @return an XPathAsserter
     */
    ...
    /**
     * Create a XPathAsserter for the given URL and POST content. This method further checks if the document is valid
     * and checks the HTTP response with the given validator.
     *
     * @param url
     *            the request URL
     * @param post
