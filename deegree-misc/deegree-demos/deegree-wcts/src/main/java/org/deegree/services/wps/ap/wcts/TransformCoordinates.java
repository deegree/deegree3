...
public class TransformCoordinates implements ExceptionAwareProcesslet {
    ...
    public void init() {
        ...
        // initialize the default crs provider.
        CRSProvider provider = CRSConfiguration.getInstance().getProvider();
        if ( provider == null ) {
            throw new RuntimeException(
                                        "Could not instantiate the default crs provider, TransformCoordinate can not be used." );
        }
        LOG.info( "Using crs provider: " + provider.getClass().getCanonicalName()
                  + " to transform incoming geometries." );
    ...
    private InputParams evaluateInput( ProcessletInputs in )
        ...
        case 5:
            ...
            break;
        case 6:
            CoordinateSystem tarCRS = getCRS( tCrs );
            requestedTransform = CRSRegistry.getTransformation( null, transId );
            if ( requestedTransform == null || tarCRS == null ) {
                ...
                if ( requestedTransform == null ) {
                    sb.append( IN_TRANSFORM ).append( " (" ).append( transId ).append(
                                                                                       ") references an unknown transformation." );
                }
            } else {
                transformer = new XMLTransformer( targetCRS );
            }
     ...
     */
    private CoordinateSystem getCRS( String crs ) {
        CoordinateSystem result = null;
        try {
            result = new CRS( crs ).getWrappedCRS();
        } catch ( UnknownCRSException e ) {
            if ( LOG.isDebugEnabled() ) {
    ...
    private class InputParams {
        final CoordinateSystem sourceCRS;

        final XMLTransformer transformer;

        final Transformation defaultTransform;

        InputParams( CoordinateSystem sourceCRS, XMLTransformer transformer, Transformation defaultTransform ) {
