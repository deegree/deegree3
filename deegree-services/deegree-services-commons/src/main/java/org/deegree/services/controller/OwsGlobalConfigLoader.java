...
public class OwsGlobalConfigLoader implements Initializable {
    ...
    public void init( Workspace workspace ) {
        ...
        try {
            ...
            setMetadataConfig( (DeegreeServicesMetadataType) ( (JAXBElement<?>) JAXBUtils.unmarshall( METADATA_JAXB_PACKAGE,
                                                                                                      METADATA_CONFIG_SCHEMA,
                                                                                                      new DURL(
                                                                                                                mdurl.toExternalForm() ).openStream(),
                                                                                                      workspace ) ).getValue() );
        } catch ( Exception e ) {
        ...
        if ( !main.exists() ) {
            ...
            try {
                setMainConfig( (DeegreeServiceControllerType) ( (JAXBElement<?>) JAXBUtils.unmarshall( CONTROLLER_JAXB_PACKAGE,
                                                                                                       CONTROLLER_CONFIG_SCHEMA,
                                                                                                       new DURL(
                                                                                                                 main.toURI().toURL().toExternalForm() ).openStream(),
                                                                                                       workspace ) ).getValue() );
            } catch ( Exception e ) {
                setMainConfig( new DeegreeServiceControllerType() );
     ...
     *            the metadataConfig to set, may be <code>null</code>
     */
    ...
    /**
     * @param mainConfig
     *            the mainConfig to set, may be <code>null</code>
     */
    ...
    /**
     * @param logOnlySuccessful
     *            the logOnlySuccessful to set
     */
