...
public class SecureProxy extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// working around unwanted deegree 2 ant artefacts... an URL normalization would
		// be nice
		...
		try {
			...
			if (loggedIn && serviceRights) {
				...
				while (iter.hasMoreElements()) {
					...
					// by re-exporting the XML and omitting user/password the length may
					// change...
					// not removing it will cause the post call below to hang
					// indefinitely!
		...
		try {
			...
			if (loggedIn && serviceRights) {
				...
				if (req.equalsIgnoreCase("GetCapabilities") || req.equalsIgnoreCase("GetFeature")
					...
					reader.next();
					successful = copyXML(reader, outFac.createXMLStreamWriter(out), request.getRequestURL().toString());
				}
				else {
					// TODO determine from content type if it was successful, for WFS this
				...
				if (requestLogger != null && successful) {
					requestLogger.logKVP(proxiedUrl + "?" + request.getRequestURL(), toQueryString(normalizedKVPParams),
							startTime, System.currentTimeMillis(), creds);
				}
			}
			else {
				writeUnauthorized(response, loggedIn);
	...
	boolean copyXML(XMLStreamReader reader, XMLStreamWriter writer, String serverUrl) throws XMLStreamException {
				...
				case START_ELEMENT: {
					...
					for (int i = 0; i < reader.getAttributeCount(); i++) {
						...
						if (localName.equals("schemaLocation")) {
							String link = value.replace(":80", ""); // again,
																	// normalization would
																	// be nice
							...
							if (nsURI.equals("http://www.w3.org/1999/xlink")) {
								...
								if (link.startsWith(proxiedUrl)) {
									link = link.replace(proxiedUrl, serverUrl);
									// next two to work around buggy servers with broken
									// endpoints such as a misconfigured
