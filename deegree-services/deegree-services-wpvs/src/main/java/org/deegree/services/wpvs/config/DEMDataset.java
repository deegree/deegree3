...
public class DEMDataset extends Dataset<TerrainRenderingManager> {
	...
	/**
	 * @param numberOfDEMFragmentsCached defines the number of dem fragments to be cached
	 * on the gpu.
	 * @param directMeshfragmentPoolSize the size (in MB) of the pool used for allocating
	 * direct byte buffers for reading mesh fragments, used in the
	 * {@link MeshFragmentDataReader}.
	 * @param ambientColor
	 * @param diffuseColor
	 * @param specularColor
	 * @param shininess
	...
	public DEMDataset(int numberOfDEMFragmentsCached, int directMeshfragmentPoolSize, float[] ambientColor,
		...
		// this.directMeshfragmentPool = new DirectByteBufferPool(
		// directMeshfragmentPoolSize * 1024 * 1024,
		// "fragment_data" );
		this.workspace = workspace;
	}

	/**
	 * Analyzes the {@link ElevationDataset} from the {@link DatasetDefinitions}, fills
	 * the renderers with data from the defined modelbackends and builds up a the
	 * constraint vectors for retrieval of the appropriate renderers.
	 * @param sceneEnvelope
	 * @param toLocalCRS
	 * @param configAdapter
	 * @param dsd
	...
	public Envelope fillFromDatasetDefinitions(Envelope sceneEnvelope, double[] toLocalCRS,
		...
		if (!demDatsets.isEmpty()) {
			...
			LOG.info(
					"No elevation model dataset has been configured, no buildings, trees and prototypes will be available.");
		}
		...
		if (demDataset != null) {
			...
			if (mrModel != null) {
				...
				TerrainRenderingManager result = new TerrainRenderingManager(fragmentManager,
						demDataset.getMaxPixelError(), 1, ambientColor, diffuseColor, specularColor, shininess);
