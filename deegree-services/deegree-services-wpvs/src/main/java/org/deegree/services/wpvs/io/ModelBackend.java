...
/**
 * The <code>ModelBackend</code> provides methods for connections to the wpvs model in a
 * database.
 *
...
public abstract class ModelBackend<G> implements RenderableStore {
	...
	/**
	 * Retrieve some information on the number of vertices and texture coordinates used in
	 * the backend.
	 * @param type
	 ...
	 * @param baseCRS the crs of the WPVS scene
	 * @return the list of prototypes or the empty list if an error occurred, never
	 * <code>null</code>.
	 */
	 ...
	 * @param bm to add the building to
	 * @param baseCRS the crs of the WPVS scene
	 */
	 ...
	 * @param objectType
	 * @param sqlWhere to select the objects to retrieve
	 * @return the list of deserialized object or the emtpy list if no results matched.
	 * @throws IOException
	 */
	public abstract List<Object> getDeSerializedObjectsForSQL(Type objectType, String sqlWhere) throws IOException;
	 ...
	 * @throws IOException
	 */
	public abstract Object getDeSerializedObjectForUUID(Type objectType, String uuid) throws IOException;
	...
	/**
	 * @param geometry
	 * @param dimension of the result geometry
	 ...
	 * @param objectType defining the type of object.
	 * @param sqlWhere
	 * @param qualityLevel
	 ...
	 * @param <P> type of the positionable
	 * @param objects
	 * @param objectType defining the type of the dataModel
	 ...
	 * @param id to be used to initialize the if it contains
	 * org.deegree.services.wpvs.io.file.FileBackend, the file backend will be
	 * initialized.
	 * @param fileURL of the file backend files
	 * @return a ModelBackendInstance associated with the given driver.
	 * @throws DatasourceException if the driver was not found on the classpath, or could
	 * not be initialized.
	 * @throws UnsupportedOperationException if the given driver has no implementing
	 * backend.
	 */
	public static ModelBackend<?> getInstance(String id, Workspace workspace)
		...
		if (!(renderableStore instanceof ModelBackend<?>)) {
			throw new DatasourceException("The given id ( " + id + ") instantiates a: "
					+ renderableStore.getClass().getCanonicalName() + " this cannot be cast to Modelbackend.");
		}
		...
		// } catch ( IOException e ) {
		// throw new DatasourceException( "Filebackend could not be loaded because: " +
		...
		// throw new DatasourceException( "Given id: " + d + " was not known to the
	 ...
	 * @throws IOException if flushing was not successful
	 *
	 */
	...
	/**
	 * @param wpvsTranslationVector the 3 dimensional wpvsTranslationVector to set
	 */
