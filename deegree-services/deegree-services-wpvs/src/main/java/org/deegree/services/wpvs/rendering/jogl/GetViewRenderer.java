...
public class GetViewRenderer implements GLEventListener {
	 ...
	 * @param demRenderer
	 * @param colormap to render instead of a texture.
	 * @param buildingRenderers containing requested buildings
	 * @param treeRenderers containing requested trees.
	 * @param copyrightID the texture id of the copyright.
	 * @param copyrightScale the id of the copyright image
	 * @param sceneLatitude the latitude of the requested scene, necessary to calculate
	 * the light position.
	 *
	 */
	public GetViewRenderer(GetView request, RenderContext glRenderContext, PooledByteBuffer imageBuffer,
			TerrainRenderingManager demRenderer, Colormap colormap, List<TextureManager> textureManagers,
			List<RenderableManager<?>> renderableRenderers, String copyrightID, double copyrightScale,
	...
	private void init(GL gl) {
		...
		gl.glLightfv(GL.GL_LIGHT0, GL.GL_POSITION,
				new float[] { light_position[0], light_position[1], light_position[2], 0 }, 0);
	...
	private void renderCopyright(RenderContext context) {
		...
		if (copyImage != null) {
			...
			// // if the width of the texture is larger then the height, use the scalediff
			// to calculate the texwidth.
