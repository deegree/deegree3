...
/**
 * The <code>ModelDatasetWrapper</code> loads the entities and the billboards from the
 * configured backends.
 *
...
public class RenderableDataset extends Dataset<RenderableManager<?>> {
	...
	/**
	 * Analyzes the ModelDataset from the {@link DatasetDefinitions}, fills the renderers
	 * with data from the defined {@link RenderableStore} and builds up a the constraint
	 * vectors for retrieval of the appropriate renderers.
	 * @param sceneEnvelope
	 * @param toLocalCRS
	 * @param configAdapter
	 * @param dsd
	 ...
	 * @param result to add the information to
	 * @param backend to get the information from.
	 */
	...
	private Envelope initEntities(Envelope sceneEnvelope, double[] toLocalCRS,
		...
		/**
		 * assuming each building has 10 geometries (in average), each geometry has 6
		 * vertices (2 triangles ) and each vertex has 3 ordinates 10*6*3 = 180
		 */
		...
		BuildingRenderer allBuildings = new BuildingRenderer(dsEnvelope, numberOfObjectsInLeaf, geometryBuffer,
				configuredBuildingsDS.getMaxPixelError(), createLevels(configuredBuildingsDS.getSwitchLevels()));
	...
	private static Envelope createDefaultEnvelope(double[] translationVector, ICRS coordinateSystem) {
		return geomFac.createEnvelope(new double[] { 0, 0, 0 },
				new double[] { DEFAULT_SPAN, DEFAULT_SPAN, DEFAULT_SPAN }, coordinateSystem);
	}
	...
	private Envelope mergeGlobalWithScene(double[] toLocalCRS, Envelope dsEnvelope, Envelope sceneEnvelope) {
		if (dsEnvelope != null && (Math.abs(dsEnvelope.getSpan0() - RenderableDataset.DEFAULT_SPAN) > 1E-8)) {
			...
			// convert the global dataset (in wpvs world coordinates) to real world
			// coordinates.
