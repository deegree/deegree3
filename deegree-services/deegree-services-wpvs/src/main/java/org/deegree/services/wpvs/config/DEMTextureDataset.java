...
/**
 * The <code>TextureDatasetWrapper</code> extracts data from jaxb configuration elements
 * and creates texture managers, from them. Following hierarchy is used: a
 * {@link TextureManager} holds a {@link TextureTileManager} which can hold one or more
 * {@link TextureTileProvider}.
 *
...
public class DEMTextureDataset extends Dataset<TextureManager> {
	 ...
	 * @param maxTexturesInGPU the number of textures in gpu cache
	 * @param maxCachedTextureTiles the number of texture tiles in cache.
	...
	private Envelope handleTextureDataset(DEMTextureDatasetConfig textureDataset, Envelope sceneEnvelope,
			double[] toLocalCRS, ResourceLocation<?> location) throws IOException {
		...
		// levelSource.getCapabilitiesDocumentLocation().getLocation() );
		// int maxWidth = -1;
		// int maxHeight = -1;
		...
		// // TODO remove hard code crs reference (But note: 'EPSG' must be in uppercase,
		// which differs
		// // from
		// // getDefaultCRS())
		//
		// // rb: don't get the envelope, because wms's tend to have wrong or falsely
		// specified
		// // boundingboxes.
		// try {
		...
		// requestTimeout );
		// } catch ( Exception e ) {
		// LOG.warn( "Could not create wms dataset from " + capabilitiesURL + " because: "
		// + e );
		// }
		// } else {
		...
		if (!tileProviders.isEmpty()) {
			TextureTileManager tileManager = new TextureTileManager(
					tileProviders.toArray(new TextureTileProvider[tileProviders.size()]), maxCachedTextureTiles);
			...
			// adding this constraint will result in never finding it in the scene,
			// because it needs to be converted to
	...
	private Envelope fillFromStyledGeometries(StyledGeometryProvider styledGeometryProvider,
			List<TextureTileProvider> tileProviders, Envelope sceneEnvelope, double[] toLocalCRS,
		...
		try {
			...
			style = SymbologyParser.INSTANCE
				.parse(XMLInputFactory.newInstance().createXMLStreamReader(styleFile.toExternalForm(), styleStream));
		}
		catch (Exception e) {
		...
		org.deegree.services.jaxb.wpvs.StyledGeometryProvider.TextureCacheDir rasterCache = styledGeometryProvider
			.getTextureCacheDir();
