...
class WmtsBuilder {
    ...
    WmtsBuilder( ResourceMetadata<OWS> md, Workspace workspace ) {
        DeegreeWMTS conf;
        try {
            conf = (DeegreeWMTS) JAXBUtils.unmarshall( CONFIG_JAXB_PACKAGE,
                                                       ( (OWSProvider) md.getProvider() ).getSchema(),
                                                       md.getLocation().getAsStream(), workspace );
