...
* ``EnableResponsePaging``: By default, WFS 2.0.0 does not support response paging. By specifying ``true`` here, you can explicitly enable response paging. Currently @next and @previous URLs bases on the original GetFeature request in KVP encoding.
