...
To enable support for the Manage Stored Queries conformance class for WFS 2.0.0 it is required to create a directory storedqueries/managed in your workspace. In this directory the stored queries created with ``CreateStoredQuery`` requests are stored. They are loaded by the start of deegree automatically. It is not recommend to put the StoredQueries configured in the WFS config with the StoredQuery element into this folder. If the directory is missing the ``CreateStoredQuery`` requests returns an exception.  
