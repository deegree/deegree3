...
Please note that if you use external tools to seed the tile store, you need to make sure the resulting structure is compatible. The ``00`` directory corresponds to the *last* tile matrix of the referenced tile matrix set, ``01`` to the next to last tile matrix and so on.
