...
By default the sort order of the features is specified by the underlying database.
This can be changed by configuring the default sort order. Only simple properties
can be used for this option. It is possible to define multiple columns and
if sorting should be ascending or descending per column.
The default sort order is overwritten by requests, e.g. SORTBY in WFS request.
...
|_sortOrder_ |0..1 |String |sort type (ASC, DESC), default: ASC
|===
