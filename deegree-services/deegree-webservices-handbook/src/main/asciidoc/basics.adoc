...
image:images/workspace-configuration-dependencies.png[Workspace
...
In order to use a different folder for deegree's configuration files,
...
Usually, you don't need to care about the three files that are located
...
When multiple deegree webservices instances run on a single machine,
...
You may wonder why the `+main.xml+` and `+metadata.xml+` files are not
...
The configuration format has to match the workspace subdirectory, e.g.
...
xml/workspace_dependencies.xml
...
The required dependencies are specific to every type of resource and are
...
resource,scaledwidth=50.0%]

Tip

When a resource is being deactivated, the suffix of the corresponding
...
configuration,scaledwidth=50.0%]
...
Alternatively, you can approach the resources question bottom-up. Let's
...
In case you are able to understand XML schema, you can also use the
...
log,scaledwidth=50.0%]

Tip

If you deployed the WAR version, the location of the deegree log depends
on your web application container. For Tomcat, you will find it in file
`+catalina.out+` in the `+log/+` directory.

Tip

More logging can be activated by adjusting file `+log4j.properties+` in
