...
public class GetCoverage100XMLAdapter extends WCSRequest100XMLAdapter {
	...
	private RangeSet parseRangeSubset() throws OWSException {
		OMElement range = getElement(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":rangeSubset", wcsNSContext));
		RangeSet parsedRange = null;

		if (range != null) {
			List<OMElement> axisSubset = getRequiredElements(rootElement, new XPath(
					"/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":rangeSubset/" + WCS_PREFIX + ":axisSubset",
					wcsNSContext));
				...
				if (axis != null) {
					...
					for (OMElement rangeElem : ranges) {
						...
						if ("interval".equals(localName)) {
							...
							SingleValue<?> min = parseTypedLiteral(
									getElement(rangeElem, new XPath(WCS_PREFIX + ":min", nsContext)));
							SingleValue<?> max = parseTypedLiteral(
									getElement(rangeElem, new XPath(WCS_PREFIX + ":max", nsContext)));
							...
							SingleValue<?> spacing = parseTypedLiteral(
									getElement(rangeElem, new XPath(WCS_PREFIX + ":res", nsContext)));
							...
							if (single.type == ValueType.Void || single.type == ValueType.String) {
								// and now for the check if the value of the single value
								// contains a '/', than it is
								...
								if (val != null) {
									if (val.contains("/")) {
										// aha, an interval
										LOG.warn(
												"A single value uses interval semantic, parsing the single value as an interval.");
										if (intervals == null) {
											intervals = new ArrayList<Interval<?, ?>>();
										}
										List<Interval<?, ?>> parseIntervals = GetCoverage100KVPAdapter
											.parseIntervals(val, single.type);
	...
	private void checkDomainSubset() throws OWSException {
		OMElement domainSubset = getElement(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":domainSubset", wcsNSContext));

		checkRequiredElement("domainSubset", domainSubset);

		domainSubset = getElement(rootElement, new XPath(
				"/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":domainSubset/" + WCS_PREFIX + ":spatialSubset",
				wcsNSContext));
		OMElement temporalSubset = getElement(rootElement, new XPath(
				"/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":domainSubset/" + WCS_PREFIX + ":temporalSubset",
				wcsNSContext));
	...
	private String getInterpolation() {
		return getNodeAsString(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":interpolationMethod", wcsNSContext),
				InterpolationType.NEAREST_NEIGHBOR.name());
	}

	private String parseOutputFormat() throws OWSException {
		String format = getNodeAsString(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":output/" + WCS_PREFIX + ":format",
						wcsNSContext),
				null);
	...
	private String parseOutputCRS(String defaultCRS) {
		return getNodeAsString(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":output/" + WCS_PREFIX + ":crs",
						wcsNSContext),
				defaultCRS);
	}
	...
	private OMElement parseEnvelopeElement() throws OWSException {
		OMElement envElem = getElement(rootElement, new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX
				+ ":domainSubset/" + WCS_PREFIX + ":spatialSubset/gml:Envelope", wcsNSContext));
	...
	private String parseCoverageName() throws OWSException {
		String coverage = getNodeAsString(rootElement,
				new XPath("/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":sourceCoverage", wcsNSContext), null);
		checkRequiredString("sourceCoverage", coverage);
		return coverage;
	}

	private Grid parseGrid(Envelope requestEnvelope, String outputCRS) throws OWSException {
		...
		String path = "/" + WCS_PREFIX + ":GetCoverage/" + WCS_PREFIX + ":domainSubset/" + WCS_PREFIX
				+ ":spatialSubset/";
		OMElement gridElem = getElement(rootElement,
				new XPath(path + "gml:Grid | " + path + "gml:RectifiedGrid", wcsNSContext));
		checkRequiredElement("Grid", gridElem);

		// int dimension = getRequiredNodeAsInteger( gridElem, new XPath( "@dimension",
		// wcsNSContext ) );
		...
		if ("RectifiedGrid".equals(gridElem.getLocalName())) {
			...
			// double[] origPoint = parseNums( "gml:pos", getElement( origin, new XPath(
			// "gml:pos", wcsNSContext ) ) );
	...
	private double[] parseNums(String elemName, OMElement element) throws OWSException {
		if (element == null) {
	...
	private double[] parseNumsFromString(String elemName, String numbers) throws OWSException {
		try {
