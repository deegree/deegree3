...
public class WCServiceBuilder {
	...
	public WCService buildService() {
		...
		// if the list of outputformats is not defined by the config, use all available
		// formats.
	...
	private WCSCoverage extractCoverage(Coverage coverage) throws ServiceInitException {
		String id = coverage.getCoverageStoreId();
		org.deegree.coverage.Coverage cov = getServiceWorkspace().getNewWorkspace()
			.getResource(CoverageProvider.class, id);
		if (cov == null) {
	...
	private SimpleCoverage buildCoverage(Coverage coverage, AbstractRaster raster) {
		CoverageOptions options = buildOptions(coverage.getNativeFormat(), coverage.getSupportOptions());
		RangeSet rs = RangeSetBuilder.createBandRangeSetFromRaster("generated",
				"Automatically generated dataset, created from the native raster types.", raster);
