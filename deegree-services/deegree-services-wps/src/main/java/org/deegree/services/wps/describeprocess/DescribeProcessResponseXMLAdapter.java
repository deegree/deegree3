...
/**
 * Responsible for the generation of WPS ProcessDescription documents (responses to
...
public class DescribeProcessResponseXMLAdapter extends XMLAdapter {
	...
	/**
	 * Exports the given {@link ProcessDefinition}s as a WPS 1.0.0 compliant
	 * <code>wps:ProcessDescriptions</code> element.
	 * @param writer
	 * @param processes
	...
	public static void export100(XMLStreamWriter writer, List<WPSProcess> processes,
			Map<ProcessDefinition, String> processDefToWSDLUrl) throws XMLStreamException {
	...
	private static void exportDescription100(XMLStreamWriter writer, WPSProcess process, String wsdlURL)
		...
		if (processDef.getInputParameters() != null) {
			...
			for (JAXBElement<? extends ProcessletInputDefinition> input : processDef.getInputParameters()
				.getProcessInput()) {
		...
		for (JAXBElement<? extends ProcessletOutputDefinition> output : processDef.getOutputParameters()
			.getProcessOutput()) {
	...
	private static void exportInput100(XMLStreamWriter writer, ProcessletInputDefinition input)
		...
		if (input instanceof LiteralInputDefinition) {
			...
			if (literalData.getDefaultUOM() != null) {
				org.deegree.process.jaxb.java.LiteralInputDefinition.DefaultUOM defaultUOM = literalData
					.getDefaultUOM();
