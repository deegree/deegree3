...
public class SpringConnectionProviderBuilder implements ResourceBuilder<ConnectionProvider> {
    ...
    public SpringConnectionProviderBuilder( final SpringConnectionProviderMetadata metadata, final Workspace workspace,
    ...
    public ConnectionProvider build() {
        ...
        LOG.debug( "Building SpringConnectionProvider (applicationContextHolder: {}, dataSourceName: {})",
                   applicationContextHolderId, dataSourceName );
        ...
        if ( dataSourceName != null ) {
            try {
                ...
                throw new ResourceInitException( "Couldn't fetch bean with type DataSource and name '" + dataSourceName
                                                 + "' from ApplicationContext", e );
            }
        } else {
            ...
            if ( dataSources.size() == 1 ) {
                ...
                if ( dataSources.containsKey( "dataSource" ) ) {
                    ...
                    throw new ResourceInitException(
                                                     "Multiple beans with type DataSource are found in ApplicationContext, non of them are named 'dataSource'. Suggestion: add bean name to configuration" );
                }
            }
        }
