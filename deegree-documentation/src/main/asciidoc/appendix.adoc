...
|deegree.sqldialect.consider-all-geometry-columns |java.lang.Boolean |true |Enables the considerations of all geometry properties of a feature type for GetFeature requests with bbox parameter and without property name (all SQL Dialects), as well as the calculation of the bbox cache (PostgreSQL only).
